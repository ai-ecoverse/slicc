...
Native desktop capture is a **separate** tray follower (`Models/ComputerTrayFollower.swift`, runtime `sliccstart-computer`). It **always dials** when a leader join URL is set, advertises `capabilities.computer`, and never `exec`. First `computer.native.capture` / `.input` lazily requests Screen Recording / Accessibility (`ComputerPermissions`); JPEG frames honour maxWidth/fps and are chunked like `computer.frame`. Widgets must not import this path (or WebRTC). Tests stub SCStream/CGEvent.
