...
- **Build** — SPM cannot emit `.appex` bundles, so `packages/swift-launcher/project.yml` (XcodeGen) defines the extension target; `assemble-app.mjs` runs `xcodebuild` and copies the product into `Contents/PlugIns/`.
- **Credentials** — when `leaderJoinUrl` is set, `FileProviderCoordinator` writes the join URL into the team-prefixed app group (`S8LB56P782.com.slicc.sliccstart.fileprovider`) + keychain for the sandboxed appex. Tray id is `SyncedTraySession.identifier(forJoinUrl:)`.
- **Domain lifecycle** — `FileProviderDomainLifecycle` (shared) registers `slicc-vfs`, re-adds domains stuck at `userEnabled=false` from before `NSExtensionFileProviderSupportsEnumeration`, and withdraws on clean quit (not on update/detach). Settings → Startup toggles Finder integration (`fileProvider.finderEnabled`).
- **Signing** — appex is App Sandbox with its own entitlements; `sign-and-package.sh` signs it innermost-first before the outer app. User enablement is still required once in System Settings → Login Items & Extensions → File Provider.
