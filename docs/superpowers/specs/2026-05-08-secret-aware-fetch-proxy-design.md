...
- **API change**: `saveOAuthAccount` becomes **async** — was `function saveOAuthAccount(opts): void` (line 430), becomes `async function saveOAuthAccount(opts): Promise<void>`. Internally awaits the sync round-trip and caches the returned `maskedValue` before resolving. All call sites (github.ts:517, 549; adobe.ts:356, 406, 528) must add `await`.
