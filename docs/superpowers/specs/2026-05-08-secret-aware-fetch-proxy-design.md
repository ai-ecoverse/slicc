...
- **Sandboxing CLI agent JS execution** — no longer relevant. The kernel-realm system (`packages/webapp/src/kernel/realm/`) that landed in main closes this gap by running `.jsh` / `node -e` in a `DedicatedWorker` (standalone) or sandbox iframe (extension), neither of which has access to the page's `localStorage`. Our spec inherits this isolation for free. The original "JS-sandbox workstream" follow-up the threat-model section used to mention is therefore obsolete.
...
The agent has **active code-execution surfaces** distinct from its prompt context. Today's `.jsh` / `node -e` / `python3 -c` execution goes through the kernel-realm system (`packages/webapp/src/kernel/realm/`):

- Standalone: JS realm = `DedicatedWorker` via `js-realm-worker.ts` (no `localStorage` in Worker globals).
- Extension: JS realm = sandbox iframe via `realm-iframe.ts` (sandboxed origin, no access to extension main-scope storage).

In both cases, agent-controlled JS code does **not** have direct access to `slicc_accounts` in localStorage — the realm isolation closes that avenue incidentally. (The kernel-worker itself has a `localStorage` shim via `page-storage-sync.ts`, but realms are separate, and the shim's contents only matter for trusted-runtime code paths like `oauth-token`, which our Phase 4 design routes through masked output.)

What remains: shell-command implementations themselves run in the kernel-host (kernel-worker in standalone, offscreen document in extension) and DO have access to the localStorage shim / real chrome.storage. These are **trusted runtime code**, not agent-controlled. The risk would be a future shell command that exposes localStorage contents to agent stdout — there's no such command today, but a code-review discipline is needed to prevent one being added unintentionally. Worth a brief note in `docs/secrets.md` for future contributors.

The earlier framing of this section (claiming agent JS in CLI page context could read localStorage) was based on the pre-kernel-worker architecture and is now obsolete. The realm isolation that landed on main closed that hole.
...
**Why sessionId exists at all (preserving Lars's documented property).** `mask()` keys HMAC-SHA256 with `sessionId + secretName`. Both `secret-masking.ts` and `docs/architecture.md:464` document the intent: _"deterministic within a session (the agent can compare values) but differ across sessions (no lookup-table attacks)."_ The realistic threat: the agent **routinely echoes masked values** in normal operation (env vars, shell stdout, git plumbing). Those masks flow into the model's conversation context → LLM provider's logs. **If the HMAC key is publicly derivable** (no sessionId, or a sessionId derived from public env like hostname / `chrome.runtime.id` / file path) **an attacker with leaked mask + known secret name can enumerate candidate values** — feasible for low-entropy secrets. A random sessionId (per-process or persisted) is the only construction that resists this. Dropping sessionId or replacing it with a public-derivable identifier is a security regression for the most realistic threat surface. Persistence preserves the property while letting cached masks survive restart. (See the archaeology in PR review history: this design point was carefully considered against an alternative-of-dropping-sessionId proposal and the alternative was rejected.)
...
| `node-command.ts`                 | several | **Audit + careful migration.** The dispatcher into the kernel-realm system (`realm-factory.ts`, `js-realm-shared.ts`) is where `node -e` user code actually runs post-merge. Verify the realm's `fetch` shim points at SecureFetch upstream of the realm port. The legacy `~line 475` sandbox-iframe-handler reference no longer applies — `node-command.ts` got refactored as part of the kernel-realm work that landed on main. Mostly read-and-confirm.                                                                                                                                                                                                                                                             |
| `kernel/realm/js-realm-shared.ts` | n       | **Migrate the realm's `fetch` shim AND `fsBridge.fetchToFile`.** Post-kernel-realm merge, `.jsh` / `node -e` execute inside a `DedicatedWorker` (standalone) or sandbox iframe (extension) — see `realm-factory.ts`. The realm receives `exec` / `fetch` shims via an RPC port (`js-realm-shared.ts:11`). The fetch RPC is implemented upstream of the realm; route that upstream end through SecureFetch so user `.jsh` scripts using masked `$TOKEN` get the unmask. Same for `fsBridge.fetchToFile` (`js-realm-shared.ts:134`). **Replaces the earlier `jsh-executor.ts` entry** — the AsyncFunction construction moved out of `jsh-executor.ts` and into the realm system in the kernel refactor. ~20 LoC + tests. |
...
- `docs/secrets.md`:
  ...
  - **Threat-model addendum**: a short note that agent-controlled JS execution (`.jsh`, `node -e`, `python3 -c`) runs in kernel-realms (`DedicatedWorker` standalone, sandbox iframe extension) without direct access to the page's `localStorage`, so secrets in `slicc_accounts` are not reachable from agent code paths. Trusted runtime code in the kernel-host has localStorage access via the page-storage-sync shim; new shell commands must not echo localStorage contents to agent output — a code-review discipline, not an architectural enforcement.
