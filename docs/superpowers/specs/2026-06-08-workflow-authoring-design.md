...
Still open: 1. exact cross-type precedence rule (`.jsh` wins, or by mtime?) — pick one in planning. 2. whether to also discover skill-bundled `*.workflow.js` under `/workspace/skills/*/`.
