...
- **Re-sync trigger (name the watcher + the plumbing):** the load-bearing watcher is `WasmShellHeadless`'s registration watcher (`wasm-shell-headless.ts:281`), whose predicate is currently `path.endsWith('.jsh')`. Extend it to also fire on `*.workflow.js` under `/workspace/.workflows/` and `/workspace/skills/*/.workflows/`. The watcher covers VFS edits automatically; `workflow save` additionally calls the sync directly so the new command appears immediately. **Plumbing:** `syncJshCommands()` lives on `WasmShellHeadless` (`:~442`) but `createWorkflowCommand` only receives `getParentJid` today (`shell/supplemental-commands/index.ts:~177`) — so add a `syncScriptCommands` callback to `SupplementalCommandsConfig` and thread it into `createWorkflowCommand` for `save` to invoke. (`ScriptCatalog`'s own cache-invalidation watcher at `script-catalog.ts:82` already watches _all_ paths, so only the shell's re-sync predicate needs widening; the sync only ever **registers** names — it never needs to unregister, per the dispatch-time model.)
- **Skill-bundled workflows are always `<skill>:<name>`** — unconditionally (CC's `plugin:skill` model). `<skill>` is the skill directory basename (`skills/discover.ts:~25`); `<name>` is the workflow filename stem. **Caveat (verified):** discovery uses the raw directory basename, and a real VFS dir _could_ contain a `:` even though install-managed/dropped skills are validated against `VALID_SKILL_NAME = /^[A-Za-z0-9][A-Za-z0-9._-]*$/` (`skills/install-from-drop.ts:31`). So workflow discovery must **apply that same charset check to the skill segment** and skip (with a logged warning) any skill dir whose name contains a reserved char. Given a validated segment, `:` cannot appear in either `<skill>` or `<name>`, so `<skill>:<name>` is collision-free with built-ins, `.jsh`, saved workflows, and other skills' workflows (modulo unique skill names — a skill-system invariant), and skill workflows do **not** enter the bare-name contest. (This replaces the earlier hyphen idea, which was a lossy flatten: `weekly-audit`+`report` vs `weekly`+`audit-report`.)
- **`which`/`commands` visibility:** today these read the `.jsh` list (`SupplementalCommandsConfig.getJshCommands`, `which-command.ts`, `help-command.ts`). **Gotcha (verified):** registered names (jsh _and_ workflows) are added to `builtinCommandNames` (`wasm-shell-headless.ts:~789`) and surfaced via `getRegisteredCommands` (`:~399`); `getFilteredJshCommands` (`:824`) _excludes_ anything in `builtinCommandNames`; and `which` resolves any registered name to `/usr/bin/<name>` **before** consulting scripts (`which-command.ts:~48`). So raw `getRegisteredCommands` conflates static built-ins with dynamic `.jsh`/workflow names and would mislabel a workflow as a plain `/usr/bin` built-in. SP3 therefore needs a **separate dynamic script-command registry** distinct from the static built-in set: `getWorkflowCommands` reads the workflow discovery map (not the filtered jsh list), and `which`/`commands` render the **live** discovery/precedence state — labeling each name "workflow" or "workflow (shadowed)" rather than echoing the raw registered-name set.
- **`args` coercion:** a single trailing argument is parsed as JSON when valid, else passed as a string; multiple positional args → a string array; no arg → `undefined`. Worked examples: `wf 123` → `args === 123` (number — `JSON.parse('123')`); `wf '"123"'` → `args === "123"` (string); `wf '{"a":1}'` → `args === {a:1}`; `wf abc` → `args === "abc"` (unparseable JSON falls back to string); `wf a b` → `args === ["a","b"]`. **Note:** this is intentionally _more lenient_ than `workflow run --args`, which strictly `JSON.parse`s and errors on bad JSON (`workflow-command.ts:~76`). Both feed the same `args` global (`workflow-prelude.ts:~37`), so they are wire-compatible; the bare-command form just doesn't force authors to quote a JSON string for a simple scalar. The runner's own flags (`--wait`) are consumed before coercion; use `--` to pass a literal flag-like positional (`wf -- --not-a-flag`). `--wait` honored.
...
| `workflow save`               | `shell/supplemental-commands/workflow-command.ts` (modify)                   | Persist a run's `source` to `/workspace/.workflows/<name>.workflow.js` (reject-at-save on name collision; `--force` to overwrite); trigger the script-command sync (registers the newly discovered name — never a full rebuild). Requires a new `syncScriptCommands` callback plumbed in (see §3 Re-sync).                                                                                                                                  |
...
- **Run saved:** invoking the discovered command runs the script through the run manager (non-blocking by default); a trailing JSON arg arrives as `args`; non-JSON arg arrives as a string; multiple args arrive as a string array.
- **Run saved `--wait`:** `<saved-workflow> --wait …` runs inline and returns the full result (the `runWait` path), confirming the bare command honors `--wait`.
