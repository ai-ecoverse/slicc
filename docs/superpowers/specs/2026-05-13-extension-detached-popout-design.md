...
The current SW at `service-worker.ts:44` unconditionally executes `chrome.sidePanel.setPanelBehavior({ openPanelOnActionClick: true })` at top level. This runs on every SW cold start, including after MV3 eviction with a detached tab still alive. The Chrome side-panel API persists both `setPanelBehavior` and `setOptions` settings — but the SW's top-level call OVERWRITES the persisted `openPanelOnActionClick` every time the SW wakes, while `setOptions({ enabled: false })` is never touched by the existing code and so survives untouched. The net effect after eviction is a half-applied lock: `openPanelOnActionClick` reverted to `true` (so `action.onClicked` no longer fires and the toolbar-icon-while-detached flow is dead code) while the panel is still disabled (so clicking the icon may attempt to open a disabled side panel — "nothing happens" UX).
...
Three defense layers, labeled here in order of **preference of reliance** (Layer 1 is preferred because it ends the problem; Layer 3 is the final guarantee). `enterDetachedActiveState(offscreenClient, layout)` calls all three, but **the execution order in code is different from the preference numbering**: close → Layer 3 (lock) → Layer 2 (overlay). This is intentional — see the Components table row for `mainExtension detached-active handler` and the implementation in the plan. Locking before painting the overlay closes the brief window where a still-visible send button could leak traffic if `window.close()` doesn't take effect.
...
| SW evicted while detached tab is open                                                       | The top-level lock setting at boot is replaced by `reconcileDetachedLockOnBoot()` (see "Startup reconciliation"). On every SW wake — top-level boot, `onStartup`, `onInstalled` — the SW reads `chrome.storage.session.detachedTabId`, verifies the tab still exists, and re-applies `setPanelBehavior` + `setOptions`. Both APIs are documented by Chrome to persist their settings, but the existing SW's top-level `chrome.sidePanel.setPanelBehavior({ openPanelOnActionClick: true })` at `service-worker.ts:44` overwrites the persisted behavior on every cold start. That is the empirical source of the asymmetry — without reconciliation, eviction silently reverts `openPanelOnActionClick` to `true` while `enabled: false` survives untouched. The reconciler replaces that top-level call so both lock settings are now managed by the same code path.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `chrome.sidePanel.setOptions({ enabled: false })` UX risk                                   | Disabling the side panel globally can affect Chrome's side-panel entry-point UI (the panel may not appear in the "Open in side panel" menu while disabled). For our flow this is the correct behavior. **Persistence asymmetry (in practice):** Chrome documents both `setOptions` and `setPanelBehavior` as persistent, but the existing SW's top-level `setPanelBehavior` call at `service-worker.ts:44` overwrites that setting on every cold start while nothing in the codebase touches the persisted `enabled` flag. The reconciler replaces the top-level call and always re-applies both, eliminating the practical asymmetry. If product wants per-tab scoping, `setOptions({ tabId, enabled: false })` is per-tab; that does NOT meet the "global lock" requirement and would require iterating every open tab. Global is the chosen tradeoff.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
