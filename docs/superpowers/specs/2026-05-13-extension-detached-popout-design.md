...
**Layer 1 — Side panel close.** Call `window.close()` on the panel. On current Chrome target versions this typically works from inside a side panel, but historical behavior has varied. The SW also attempts `chrome.sidePanel.close({ windowId })` (Chrome 141+ — current QA target Chrome 146+ has it) immediately after the `detached-active` broadcast for the side panel surface specifically. Manual QA verifies which path actually closes the panel on the targeted Chrome build.

**Layer 2 — UI hard-disable.** Regardless of whether the close actually happens, call `layout.showDetachedActiveOverlay()` (new Layout method) which renders a non-dismissible overlay reading "Detached in another tab — close this window or click to dismiss." The chat composer and all action affordances are visually disabled by the overlay even if their underlying handlers remain wired up.
...
Placing the lock at `send()` rather than at the public `sendMessage` / `createAgentHandle` surface means every outbound path — chat send, scoop create/select, settings writes, terminal commands, tray join, mount commands — is automatically covered without enumeration. This is the load-bearing guarantee: even if the user clicks through a stuck UI overlay, no user-action message reaches the offscreen agent.
...
SW: chrome.action.onClicked fires
  ...
  → // Recovery: no detached tab actually exists.
    ...
    // chrome.sidePanel.open requires a user gesture; sequencing it
    // immediately after the chrome.tabs.get reject without awaiting
    // the cleanup keeps gesture preservation simple.
```
