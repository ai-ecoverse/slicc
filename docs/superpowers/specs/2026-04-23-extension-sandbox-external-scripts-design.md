...
**Fetch story**: The sandbox has a null origin, so `fetch('https://...')` only works if the CDN sends permissive CORS headers. For reliability, add a small parent relay: sandbox sends `postMessage({ type: 'sprinkle-fetch-script', id, url })` to the parent (side panel), parent calls `fetch(url)`, returns `postMessage({ type: 'sprinkle-fetch-script-response', id, url, text })`. The `id` field correlates requests to responses when multiple scripts load concurrently. Same pattern as the existing VFS read/write relay (`sprinkle-readfile` / `sprinkle-readfile-response`, which also uses request IDs). This guarantees the fetch works regardless of CORS policy. The relay includes a timeout to prevent leaked listeners if the parent never responds.
