...
**Review round 2 (post-implementation, PR #2214):** four review agents flagged, all addressed on-branch — (1) MUST-FIX: the ResizeObserver initial-notification skip swallowed the first _real_ resize for an element mounted hidden (silent 1×1 canvas); fixed to skip the burst, not the wake. (2) MUST-ADD coverage: burst-expiry on an animated field, context-restore resume (+static re-stop), failed-restore CSS degrade, reduced-motion one-frame-per-wake, mode-switch synchronous single draw, hidden→visible reveal. (3) Hardenings: clear `#contextLost` on the failed-restore degrade; clamp `#energy` non-negative in `pulse()`. (4) Doc accuracy: WebRTC exempts timer throttling only (freezing is a separate launch-flag/`autoDiscardable` opt-out), `FRAME_EPSILON_MS` fixes pacing not the average, DPR budget is per-mode. Out-of-scope pre-existing issues filed as follow-up issues (mode-switch link-failure renders stale program; context-lost-with-no-restore unlogged; disconnect→reconnect CSS-degrade via reused force-lost context).
