...
**Boy-scout complexity gate — `host.ts` AND `lick-formatting.ts` are on the debt lists and SP2 MUST touch both (concrete, not conditional).** The `check-touched-exemptions` CI step fails if a PR modifies any file on biome.json's `overrides` debt lists without (a) bringing **every** function in that file under the caps (cognitive ≤ 25, lines ≤ 150, `skipIifes`) AND (b) removing that file's debt-list entry in the same PR. biome.json has TWO override blocks — a function-size block (`noExcessiveLinesPerFunction: off`) and a cognitive-complexity block (`noExcessiveCognitiveComplexity: off`) — each with its own file list. **Authoritative check: `node packages/dev-tools/tools/check-touched-exemptions.mjs origin/main` (not a manual grep — the lists span two blocks and a grep can miss the second).** Verified for this feature:

- **`packages/webapp/src/kernel/host.ts` is on BOTH lists** (function-size ~`biome.json:289`, cognitive ~`biome.json:361`). SP2 edits it in **Task 1** (`defaultLickEventHandler` workflow name/id branch) and **Task 6** (publish the run manager).
- **`packages/webapp/src/scoops/lick-formatting.ts` is on the cognitive list** (~`biome.json:376`). SP2 edits it in **Task 1** (the `formatLickEventForCone` workflow branch), which pushes its already-high complexity further over. (An earlier note in this plan wrongly claimed it was NOT listed — that was a grep that only scanned the first override block; the gate is authoritative and flags it. A codex review correctly caught this.)
- So this PR must de-debt **both** files (extract behavior-preserving helpers so every function is under the caps) and delete **all three** entries (host.ts ×2 blocks + lick-formatting.ts ×1). **Treat this as required work, not a final check** — see Task 10 Step 3.
- **NOT on either list (no de-debt needed):** `lick-manager.ts`, `workflow-command.ts`, `workflow-prelude.ts`, and all new files (`workflow-run-manager.ts`, `__wf_progress` command). `lick-ws-bridge.ts` is on the lists but SP2 does NOT touch it — leave it.
- Re-verify after edits: `check-touched-exemptions.mjs origin/main` must print `OK (… 0 still on any debt list)`.
...
- [ ] **Step 3: De-debt `host.ts` AND `lick-formatting.ts` (REQUIRED — SP2 touches both) + verify the gate.** SP2 modifies `host.ts` (Task 1 lick branch + Task 6 publish) and `lick-formatting.ts` (Task 1 `formatLickEventForCone` branch), both on biome.json debt lists. The `check-touched-exemptions` gate requires this PR to:
  1. Remove the three debt entries from `biome.json`: `host.ts` from BOTH the `noExcessiveLinesPerFunction` block (~`:289`) and the `noExcessiveCognitiveComplexity` block (~`:361`), and `lick-formatting.ts` from the cognitive block (~`:376`). Leave `lick-ws-bridge.ts` (not touched by SP2).
  2. Run `npx biome lint packages/webapp/src/kernel/host.ts packages/webapp/src/scoops/lick-formatting.ts` to list the over-cap functions (this feature: `defaultLickEventHandler` cog 65, `createKernelHost` cog 42 / 377 lines, `formatLickEventForCone` cog 97).
  3. Extract behavior-preserving helpers until each function is under the caps (cognitive ≤ 25, lines ≤ 150 `skipIifes`). For the lick formatters/handlers: a per-channel helper + a label `Record`/switch dispatch. For `createKernelHost`: module-level boot-phase helpers threading state via params/returns (preserve exact ordering + fire-and-forget + dispose order). **This is kernel/lick code — extraction ONLY, no logic change; run the lick + kernel + manager tests after.**
  4. Run `node packages/dev-tools/tools/check-touched-exemptions.mjs origin/main` → must print `OK (… 0 still on any debt list)`; `npx biome check` both files → clean; `npx tsc --noEmit -p tsconfig.json` → EXIT 0.

  (No other touched file is on the lists — see the boy-scout note up top.)
...
- **Boy-scout gate (codex re-review + gate-confirmed):** `host.ts` (both override blocks) AND `lick-formatting.ts` (cognitive block) are on biome's debt lists, and SP2 touches both (Tasks 1 + 6) → this PR MUST de-debt both and remove all three entries (Task 10 Step 3, concrete). `lick-formatting.ts` being listed was confirmed by `check-touched-exemptions` (an earlier manual-grep note in this plan missed it — the second override block isn't caught by a naive grep; the gate is authoritative). `lick-ws-bridge.ts` is listed but untouched — leave it.
