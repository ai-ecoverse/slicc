...
| **host**                                    | `host-command.ts`          | Print the current tray status — leader (`status: leader`, `join_url`, connected followers) or follower (`status: follower (connected)`, `join_url`); the standalone worker reads follower state via the `slicc.followerTrayStatus` shim. `host join <join-url>` follows another browser's tray; `host lead [worker-url]` becomes a leader (defaults to the production hub `https://www.sliccy.ai` when no URL is given); `host reset` recycles the leader session; `host leave` exits the tray (or switches to leader on `--leader <url>`)                                                                                                                                                                                                                                                                                | `host`, `host join <join-url>`, `host lead [worker-url]`, `host reset`, `host leave`, `host leave --leader <worker-url>`                                                                                                                                                                                                                        |
...
| **serve**                                   | `serve-command.ts`         | Mint a worker-hosted preview URL for a VFS app directory, broadcast `preview.open` to all connected followers, and open the URL in the leader's tab. Two mint paths: in-realm `getPreviewMinter()` (extension agent + panel terminal) or panel-RPC `tray-open-preview` (standalone). Requires an active leader tray.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `[--entry <relative-path>] [--bridge \| --no-bridge] [--stop <token>] [--list] <directory>` — defaults to `index.html`; `--bridge` opts into leader-managed live updates (Cherry-attached followers default-on); `--no-bridge` forces off; `--stop` and `--list` parse but are deferred to a follow-up. `--project` is an obsolete no-op alias. |
...
# direct one-off file opens pre-Phase-3 SW deletion)
...
the one-time device/mount gesture caveat, tray membership (lead/join), and lick-back (the
inbound browser→brain channel for answering the chat panel) — see
`.claude/skills/slicc-cup/SKILL.md`; for the lick-back handler role specifically see
`.claude/skills/slicc-lickback-handler/SKILL.md`.
