...
| **secret**                                  | `secret-command.ts`        | Manage secrets (API keys, tokens) with domain-scoped injection. CLI: prints `~/.slicc/secrets.env` / Keychain edit instructions. Extension: writes to `chrome.storage.local`. | `list`, `set <name> [<value>] --domain <patterns>`, `delete <name>`, `test <name> <url>`                                                    |
