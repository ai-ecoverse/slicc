...
Handles are **shared** across the `usb` shell command, the realm `usb` global (`require('sliccy:usb')`), and `slicc.usb` in sprinkles — the same `usb1` object. Interface claims are **exclusive** per `(handle, interfaceNumber)`: a second `claim` from another consumer is refused with a message naming the current holder (or queued with `--wait` / `{ wait: true }`). `close` and `reset` refuse while another consumer holds a claim; pass `--force` / `{ force: true }` to displace them. Forced displacement logs a warning and emits `claim-lost` then `disconnect` on the USB bridge so the victim is told rather than left inferring from a failed transfer. `usb clear-halt` remains the targeted alternative to `reset`.
