...
# S3 (AWS) — first store creds (persisted; each --persist raises a sudo prompt), then mount
secret set s3.aws.access_key_id <id> --domain "*.amazonaws.com" --persist
secret set s3.aws.secret_access_key <key> --domain "*.amazonaws.com" --persist
...
secret set s3.r2.access_key_id <id> --domain "*.r2.cloudflarestorage.com" --persist
secret set s3.r2.secret_access_key <key> --domain "*.r2.cloudflarestorage.com" --persist
secret set s3.r2.endpoint https://<account>.r2.cloudflarestorage.com --domain "*.r2.cloudflarestorage.com" --persist
...
# List secrets (names + domains + SESSION/SAVED type, never values)
secret list

# Check if a secret would be injected for a URL
secret test GITHUB_TOKEN https://api.github.com/repos/foo/bar

# Set a session-only secret (in-memory, no approval, never persisted)
secret set API_KEY sk-… --domain api.example.com

# Read the masked value, or peek at the first/last chars of the real value
secret get API_KEY
secret peek API_KEY

# Persist or rescope — each raises a native sudo prompt
secret set API_KEY sk-… --domain api.example.com --persist
secret scope API_KEY --domain "api.example.com,*.example.com"
```
