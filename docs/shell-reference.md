...
**Webhook delivery receipts (#2524):** a POST is answered from the leader's disposition, not from "we forwarded it". `202 {"ok":true,"accepted":true}` means a unit was woken or the webhook's own `--filter` dropped the event; `404 WEBHOOK_NOT_REGISTERED` and `422 WEBHOOK_TARGET_UNRESOLVED` are the drops that used to be reported as success. The tray worker asks for the disposition with a `deliveryId` on `webhook.event` and waits briefly for the leader's `webhook.delivery`; the node-server asks over the lick bridge as a request. A leader too old to answer keeps the pre-#2524 receipt.
