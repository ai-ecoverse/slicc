...
| **vpod**                                    | `vpod-command.ts`          | Run Linux commands in a WebAssembly sandbox (capsule-run/vpod — a real 64-bit kernel + userland, not a shell reimplementation). Install-gated: `ipk add @capsule-run/vpod@<pinned>` provides the whole SDK from the VFS `node_modules`, imported through the preview service worker so its chunked ESM graph, worker spawning, and wasm fetch resolve natively (nothing bundled, no CDN fallback). `vpod start` boots a named background pod (ProcessManager-tracked, so `ps` / `kill` work); `vpod run` executes a command and returns its complete stdout/stderr/exit code — no TTY, no streaming; `vpod run` auto-boots the default pod. First boot pulls the default snapshot (git, curl, node, python3 baked in) from the vpod registry into origin-private storage. Guest networking needs cross-origin isolation (COOP/COEP), which SLICC does not serve — pods are offline; `vpod net` reports capabilities and per-port reachability. One command at a time per pod (busy guard); flags are only recognized before the first command word so guest args pass through intact.                                                                                                                                                                                                                                                                                  | `vpod run uname -a`, `vpod run --timeout 30 sh -c "..."`, `vpod start --name builder --snapshot node-20`, `vpod ls`, `vpod net --port 443`, `vpod stop`                                                                                                                                                                      |
