...
| **hid**                                     | `hid-command.ts`           | WebHID access from the shell. Same handle-registry + panel-RPC bridge as `usb`. `hid request` needs a real user gesture; `hid watch` streams input reports over a page→worker event channel until Ctrl+C. Chromium-only; unavailable in the cloud / hosted-leader float.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `hid list`, `hid request [--vid 0x.. --pid 0x.. --usage-page N --usage N]`, `hid open\|close <handle>`, `hid send <handle> <report-id>`, `hid feature-send <handle> <report-id>`, `hid feature-get <handle> <report-id> <length>`, `hid watch <handle>`, `--raw` |
| **esptool**                                 | `esptool-command.ts`       | Flash ESP32 / ESP8266 chips via esptool-js, layered on the `serial` handle namespace. Without `--port` the Web Serial picker opens (needs a user gesture). esptool-js loads lazily via dynamic `import()` (CSP-safe). Chromium-only; unavailable in the cloud / hosted-leader float.                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `esptool chip_id`, `esptool read_mac`, `esptool erase_flash`, `esptool write_flash <addr> <file>...`, `--port <handle>`, `--baud N`, `--vid 0x..`, `--pid 0x..`, `--erase`                                                                                       |
...
The `hid request` chooser requires a real user gesture (see [Gesture bridge](#gesture-bridge-usb--serial--hid)). `hid watch` subscribes to a device's input reports over a page→worker event channel (`hid-input-report`), accumulating them as hex lines until SIGINT (Ctrl+C), then printing them. `feature-get` hex-dumps by default; `--raw` emits bytes. Report payloads are capped at 4 MiB.

### Subcommands

| Form                                            | Behavior                                                          |
| ----------------------------------------------- | ----------------------------------------------------------------- |
| `hid list`                                      | List currently-granted devices as `handle  vid:pid  name [open]`. |
| `hid request [filter flags]`                    | Open the device picker (needs a user gesture); prints the handle. |
| `hid open\|close <handle>`                      | Open or close a device.                                           |
| `hid send <handle> <report-id>`                 | Send an output report; payload read from stdin.                   |
| `hid feature-send <handle> <report-id>`         | Send a feature report; payload read from stdin.                   |
| `hid feature-get <handle> <report-id> <length>` | Receive a feature report (hex by default, `--raw` for bytes).     |
| `hid watch <handle>`                            | Stream input reports as hex lines until Ctrl+C.                   |
...
| `--usage-page N` `--usage N` | `request`     | Filter the picker by HID usage page / usage. |
| `--raw`                      | `feature-get` | Emit raw bytes instead of a hex dump.        |

### Examples

```bash
# Grant a keyboard, open it, watch input reports until Ctrl+C
hid request --vid 0x046d              # prints e.g. "hid1  0x046d:0xc31c  USB Keyboard"
hid open hid1
hid watch hid1                        # streams "<reportId> <hex bytes>" lines

# Send an output report and round-trip a feature report
printf '\x00\xff' | hid send hid1 0
hid feature-get hid1 3 8 --raw > feature.bin
hid close hid1
```
