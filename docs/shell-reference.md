...
authenticated call, `GET /user` for GitHub — and reports `ACCEPTED`, `REJECTED`,
or `UNKNOWN` (the check itself could not run, which says nothing about the
token). Providers without the hook say so rather than guessing.

When silent renewal is declined, the upstream session has ended and no retry can
recover it; the only fix is `oauth-token <id> --force-login`, which opens a
consent window and therefore needs a human. Both `--renew` and `--check` name
that command in their failure output and exit **3**, the reserved
"a human must complete an interactive login" code, so an unattended caller can
branch on it — stop and ask the user — instead of parsing prose or retrying.
Exit 1 keeps its meaning: the command could not run, or its outcome is unknown
(bad usage, unknown provider, network failure).
