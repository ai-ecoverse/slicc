...
A **computer** is any screen the agent can screenshot and poke. Phase 1 backends: `v86:<name>` (registered by `v86 start`), `tab:<targetId>` (`computer add tab`), `jsh:<name>` (`sliccy:computer.register` from a `.jsh` / `jshd` unit). `screen` / `ssh` / `url` are later. Registered computers also appear as cards after browser tabs in the Browser overlay (live lightbox + bash-row frames).
