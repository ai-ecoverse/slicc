...
| **ffmpeg**                                  | `ffmpeg-command.ts`        | ffmpeg.wasm — audio/video transcoding and processing. On a cross-origin-isolated runtime the loader prefers the multi-threaded `@ffmpeg/core-mt` when installed (`ipk add -g @ffmpeg/core-mt@<pinned>`); `@ffmpeg/core` stays the universal fallback.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `ffmpeg -i input.mp4 -vcodec libx264 output.mp4`                                                                                                                                                                                                                                                                             |
...
- **Tab selection**: `--tab <targetId>` is explicit. Without it, `curlwright` uses the tab already on the request URL's origin — the one holding the session — or the only open tab. Anything ambiguous is an error listing the candidates, never a guess. `--frame <frameId>` runs in a child frame of that tab.
...
- **Supported**: `-X`, `-H` (repeatable), `-d` / `--data-raw` / `--data-binary` / `--data-ascii` / `--data-urlencode` / `--json`, `-F` / `--form-string`, `-G`, `-u`, `-e`, `-r`, `-o`, `-O`, `-i`, `-I`, `-D`, `-w`, `-s`, `-S`, `-v`, `-f`, `-m`, `-L`. `@file` arguments read from the VFS.
...
- **`-w` variables**: `%{url_effective}`, `%{http_code}`, `%{content_type}`, `%{size_download}`, `%{size_header}`, `%{size_upload}`, `%{method}`, `%{num_redirects}`, `%{time_total}`, `%{exitcode}`, `%{errormsg}`, `%{json}`, plus `%header{Name}`.
