...
| **biome**                                   | `biome-command.ts`         | Biome `check`, `lint`, and `format` over ipk-installed `@biomejs/wasm-web` and `@biomejs/js-api`; diagnostics are rendered as plain text. Walks directories filtered to known extensions, including `.jsh`/`.bsh`; those AsyncFunction bodies are wrapped before parsing and safely unwrapped after formatting. Discovers and applies the nearest `biome.json`/`biome.jsonc`; see [Biome wrapper behavior](#biome-wrapper-behavior).                                                                                                                                                                                                                                                                                                                                                                                                                               | `biome check <path>`, `biome lint <path>`, `biome format <path>`, `--write`, `--check` (format only), `--config-path <file>`, `--stdin-file-path <path>`                                                                                                                                                                                        |
