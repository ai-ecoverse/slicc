...
| **mktemp**                                                  | `mktemp-command.ts`        | Create a uniquely named temporary file (or directory with `-d`) and print its name. GNU surface: `-d`, `-u`, `-q`, `-t`, `-p DIR` / `--tmpdir[=DIR]`, `--suffix=SUFF`, `TEMPLATE` expansion, `--version`; an unrecognized option is an error, never ignored. The default directory is `$TMPDIR` (see above), so a unit gets a path it may actually write instead of discovering later that it may not. Files land at mode 0600, directories at 0700. **Overlay, not a home:** ported from [just-bash#377](https://github.com/vercel-labs/just-bash/pull/377) flag-for-flag so the eventual swap is invisible; delete it when that lands — `mktemp-builtin-tripwire.test.ts` fails the moment it does and carries the removal steps.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `mktemp`, `mktemp -d`, `mktemp -p "$TMPDIR" buildXXXXXX`, `mktemp --suffix=.json dataXXXXXX`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
