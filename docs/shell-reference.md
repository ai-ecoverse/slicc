...
WASM enters only for specific runtime-heavy commands, which fetch and cache their binaries on demand: `python3` (Pyodide), `sqlite3` (sql.js), the `node -e` / `javascript` sandbox (QuickJS), `convert` (ImageMagick), and `ffmpeg`. The `AlmostBashShell` / `AlmostBashShellHeadless` classes cover the whole shell, not just the WASM-backed commands.

`esbuild` and `biome` are thin built-in commands that are INERT unless their backing packages are installed via `ipk add` — the heavy wasm + JS API live in the user's VFS `node_modules`, never bundled with the app and never fetched from a CDN. `ipk add esbuild-wasm` enables `esbuild --version` / `--transform` / `--bundle` (bare-specifier resolution walks the same `node_modules` tree the realm uses); `ipk add @biomejs/wasm-web @biomejs/js-api` enables `biome --version` / `biome check` / `biome format`. Running either without its package exits non-zero with a guidance error pointing at the exact `ipk add` line; there is zero network traffic on the not-installed path. (`ipx esbuild` / `ipx biome` still work and stay equivalent for users who prefer the explicit npm-style invocation.)
...
| **tsc**                                     | `tsc-command.ts`           | Thin built-in surface that drives the `typescript` package loaded from VFS `node_modules` via the shared `getTypeScript()` ipk loader. Inert until `ipk add typescript`; a missing package exits non-zero with the canonical `ipk add` hint (zero network, no CDN). Walks up from `ctx.cwd` to merge nearest `tsconfig.json`'s `compilerOptions` over `ES2022`/`ESNext` defaults. No cross-file program-level type checking.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `tsc [files...]`, `--noEmit`, `--outDir`, stdin → stdout                                                                                                                                                                                                                                                                     |
| **test**                                    | `test-command.ts`          | Test runner over the [tst](https://github.com/dy/tst) library. Discovers `*.test.{js,ts}` in `ctx.cwd`, TS-transpiles each via the shared `getTypeScript()` ipk loader (run `ipk add typescript` first), runs each file in its own realm. Reporters: `tap` (default), `--reporter=spec` → tst `pretty`. Fork mode disabled.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `test [glob]`, `--reporter=<tap\|spec>`                                                                                                                                                                                                                                                                                      |
| **hf**                                      | `hf-command.ts`            | Download model repos from the Hugging Face Hub into the VFS so the on-device ML engines can read them via the preview SW. Wave 7 split: libraries (transformers / onnxruntime-web / kokoro-js) come from `ipk add`; weights come from `hf download`. Defaults `--to` to `/workspace/models/<repo>/` (matches the transformers.js `localModelPath`). Existing files at the destination are skipped unless `--force`. Routes through the captured `SecureFetch`, same proxy seam as `ipk` / `upskill`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `hf download <repo> [files...] [--to <dir>] [--revision <rev>] [--force]`                                                                                                                                                                                                                                                    |
...
| **say**                                     | `say-command.ts`           | macOS `say`-equivalent — speak the text. Uses the on-device Kokoro-82M voice once its library + weights have been staged into VFS (`ipk add kokoro-js` plus `hf download onnx-community/Kokoro-82M-v1.0-ONNX`; the warmup chains automatically after the whisper download — English text or an explicit kokoro voice id like `-v af_heart`), the Web Speech API otherwise. `allowRemoteModels` is `false` so missing weights surface a clean engine-unavailable path rather than a CDN fetch. `--list` shows kokoro voices first (marked `[kokoro]`) when the engine is warm.                                                                                                                                                                                                                                                                                                                                                                                                                         | `say "Hello"`, `-v <voice>`, `-l <lang>`, `--list`                                                                                                                                                                                                                                                                           |
| **hear**                                    | `hear-command.ts`          | macOS [`hear`](https://sveinbjorn.org/hear)-equivalent — speech recognition: listen on the microphone until the speaker pauses (or `-T` caps it) and print the transcript; `-i` transcribes an audio file instead. Two engines: the browser's built-in recognizer answers immediately; the enhanced on-device whisper-tiny model takes over once its library + weights are staged in VFS (`ipk add @huggingface/transformers onnxruntime-web` plus `hf download onnx-community/whisper-tiny` into `/workspace/models/`). The preview SW streams everything from OPFS — `allowRemoteModels` is `false`, no Cache Storage / CDN fallback. `--warmup` triggers the load, `--status` shows progress + ETA against the staged files. `-i` always uses the enhanced engine and errors with the `hf download` guidance if weights are missing. Worker float bridges over the `hear-*` panel-RPC ops; `-d` applies to the enhanced capture path only (the built-in recognizer always uses the default input). | `hear`, `-i <file>`, `-l <lang>`, `-T <seconds>`, `-d <deviceId>`, `--engine builtin\|enhanced`, `--devices`, `--status`, `--warmup`                                                                                                                                                                                         |
