...
| **convert / magick**                        | `convert-command.ts`       | Image conversion, transforms, filmstrip/grid composition, and timestamp labels (ImageMagick style)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `convert input.jpg -resize 800x600 output.jpg`; multiple inputs with `+append` / `-append`; nested `\\( ... \\)` groups; labels with `-gravity`, `-fill`, `-undercolor`, `-pointsize`, and `-annotate +X+Y TEXT`                                                                                                             |
| **playwright-cli / playwright / puppeteer** | `playwright-command.ts`    | Browser automation shell CLI. `frames --tab=<targetId>` lists frame IDs, which are distinct from tab target IDs; `eval` and `snapshot` accept `--tab=<targetId> --frame=<frameId>` for frame-scoped evaluation or a frame-only accessibility subtree.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `snapshot [--frame=<frameId>]`, `eval [--frame=<frameId>]`, `frames`, `click <ref>`, `cookie-set`, `tab-list`                                                                                                                                                                                                                |
...
| **upskill**                                 | `upskill-command.ts`       | Install skills from GitHub, the Tessl registry, or browse.sh; suggest skills for open browser tabs; update installed skills from the `.upskill` provenance record. Never overwrites or deletes an existing dotfile in a skill directory (protects `scripts/.config` credentials and `.upskill` itself).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `upskill owner/repo`, `upskill tessl:<name>`, `upskill browse:<host>/<task>`, `upskill search "query"`, `upskill tabs [--json]`, `upskill update [<skill>] [--dry-run]`, `upskill outdated`                                                                                                                                  |
...
### Install provenance (`.upskill`) and updates

Every install writes `<skill>/.upskill` — a JSON record of the source kind (`github` / `tessl` / `browse.sh`), `owner`/`repo`, repo-relative `path`, the requested `ref`, the resolved commit `sha`, and an install timestamp. `upskill info <name>` surfaces it as `Installed from:` / `Installed commit:` (the pre-existing `Source:` field is the _discovery root kind_, not the origin).

| Command                                 | Effect                                                                        |
| --------------------------------------- | ----------------------------------------------------------------------------- |
| `upskill update`                        | Update every skill that has a `.upskill` record.                              |
| `upskill update <skill>`                | Update just that skill.                                                       |
| `upskill update --dry-run`              | Report the per-path plan; write nothing.                                      |
| `upskill update <skill> --branch <ref>` | Update from a ref other than the recorded one.                                |
| `upskill update <skill> --from <o>/<r>` | Supply (and record) a source for a skill installed before provenance existed. |
| `upskill outdated`                      | List tracked skills whose recorded sha is behind their source.                |
| `upskill upgrade …`                     | Alias of `update`.                                                            |

Each path is classified with the same vocabulary the top-level [`upgrade`](#upgrade) command uses for bundled workspace files: `unchanged`, `updated`, `added`, `removed`, `kept-local`. When the recorded sha still matches the tip of the tracked ref, `update` reports **already current** and does not fetch the tree at all. Installs deliberately do _not_ call the GitHub commits API (codeload is not rate-limited); the first `update` resolves and stores the sha, so only that one pays for a whole-tree comparison.

#### Dotfile rule

**`upskill` never overwrites and never deletes a dotfile that already exists in a skill directory** — checked segment-wise, so `scripts/.config` counts. This applies to `update` _and_ to `--force` installs, which previously `rm -rf`'d the skill directory and so silently revoked the credentials skills keep in `scripts/.config` (`bb`, `gmail`, `fastly`), along with the `.upskill` record the update path needs to read.

A dotfile that is _absent_ locally is still installed: some skills legitimately ship `.gitignore` or `.config.example` upstream, and a literal "never write a dotfile" would make those uninstallable. Creating a file cannot destroy state; overwriting and deleting can. Non-dot local files that upstream no longer ships are still removed (classified `removed`), so a refresh converges on the upstream tree.

`.upskill` itself is exempt — `upskill` owns that file, it is never taken from upstream, and refusing to rewrite it would freeze the recorded sha at its first value.
