...
`chmod` uses upstream just-bash mode parsing, recursive traversal, and work limits. The bundled patch preserves backend errors instead of relabeling every failure as a missing file, and keeps `--help` after `--` as a filename. Local VFS modes persist and `chmod +x` enables direct shebang execution. Mounted metadata changes can report `ENOSYS`; a permission-denied shebang diagnostic still suggests its interpreter. Upstream `ls -l` currently prints fixed permission strings; use `stat -c %a FILE` to verify mode changes.
...
computer type -- --help         # types the literal "--help" into the current computer
...
| **patch**                                                   | `patch-command.ts`         | Apply a unified diff to the VFS. Patch text comes from `PATCHFILE`, `-i FILE`, or stdin. `-p NUM` strips leading path components; with no `-p` the name is tried as written, then one component at a time, and the first candidate that exists (or whose parent directory exists) wins. `-R` reverses, `--dry-run` reports without writing, `-F NUM` sets the context fuzz (default 2), `-s` silences the progress lines. Hunks are located by searching outward from their recorded line, so a drifted file still patches; hunks that cannot be placed go to `FILE.rej` and the exit status is 1 (2 for a missing target or garbage input). The diff grammar and applier live in `src/git/patch-core.ts`, next to the `git diff` producer.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| **mktemp**                                                  | `mktemp-command.ts`        | Create a uniquely named temporary file (or directory with `-d`) and print its name. GNU surface: `-d`, `-u`, `-q`, `-t`, `-p DIR` / `--tmpdir[=DIR]`, `--suffix=SUFF`, `TEMPLATE` expansion, `--version`; an unrecognized option is an error, never ignored. The default directory is `$TMPDIR` (see above), so a unit gets a path it may actually write instead of discovering later that it may not. Gives you a **unique** name, not a private one: local VFS entries store 0600/0700, but access is governed by SLICC's path policy and `/tmp` is readable by every unit; mounted backends may reject modes. Never put a secret in a shared `mktemp` file. **Overlay, not a home:** ported from [just-bash#377](https://github.com/vercel-labs/just-bash/pull/377) flag-for-flag so the eventual swap is invisible; delete it when that lands — `mktemp-builtin-tripwire.test.ts` fails the moment it does and carries the removal steps.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `mktemp`, `mktemp -d`, `mktemp -p "$TMPDIR" buildXXXXXX`, `mktemp --suffix=.json dataXXXXXX`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **rg**                                                      | `rg-command.ts`            | Overlay on just-bash `rg` (#3106, #3138). A byte-limit miss is a normal non-zero exit with capturable stderr, not a script abort; skipped binaries and `--max-filesize` files do not count; the printed number is the searchable budget actually enforced (`min(maxInputBytes, floor(maxLiveBytes / 2))`, 256 MiB at the default 512 MiB live/input caps). `-c` / `--count` / `--count-matches` omit a zero count on stdin the same way file args already do (exit 1 still means no matches), so a piped count is not a fabricated `0`. Bundled search still runs via `ctx.origCommand` once the set is under budget. Delete when upstream returns a command result for this limit, reports the trip point, does not charge skipped binaries, and omits zero counts on stdin.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `rg PATTERN [PATH …]`; `-a` / `--text` counts binaries; `-c` omits zeros                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| **id**                                                      | `id-command.ts`            | User and group identity, from the home directory the rest of the shell already uses (`shell/home-dir.ts` for a cone, `buildScoopShellEnv`'s pinned `HOME`/`USER` for a scoop). A cone is the human user at uid 1000 — one per runtime, since every cone shares one `/home/<slug>`; a scoop is a service identity whose uid is derived from its folder name (FNV-1a into 2000–60000), stable across reloads and distinct per scoop. Groups are a user-private group of the same name plus the role group `cone` (10) or `scoop` (20). `-u`, `-g`, `-G`, `-n`, `-r` (accepted and ignored — no setuid here), and an optional `USER` operand resolved against `/home/<name>` and `/scoops/<name>`. Registers a `whoami` that agrees with `id -un`; just-bash's bundled one answers a hardcoded `user`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **host**                                                    | `host-command.ts`          | Print the current leader tray status plus `join_url` and the `followers:` list; `host reset` recycles the leader session; `host leave` exits the tray (or switches to leader on `--leader <url>`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `host`, `host reset`, `host leave`, `host leave --leader <worker-url>`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| **tar**                                                     | `tar-command.ts`           | Create, extract, or list tar archives in the VFS, with optional gzip compression and automatic gzip detection when reading. `-C` changes the input base directory in create mode and the output directory in extract mode. Paths over 100 bytes are stored in the ustar prefix field, or in a PAX record when no split fits, so any tar reads them back.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `(-c\|-x\|-t) [-zv] -f <archive> [-C <dir>] [paths...]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| **node**                                                    | `node-command.ts`          | Execute JavaScript code                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `-e "console.log(1+1)"`, `--check file.js` with fs bridge                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **python3 / python**                                        | `python-command.ts`        | Execute Python code via Pyodide. The whole VFS (mounts included) is mounted lazily over the sync bridge, so stdlib `open` / `os` / `pathlib` work everywhere, and `subprocess` / `os.system` run shell commands (buffered, run-to-completion) with `os.environ` as their environment. `SLICC_PY_FS=opfs`, or a leader with no sync bridge, falls back to the OPFS preload, where sync access to a mounted path raises `OSError` (EIO) and the async `slicc.fs` module (`await slicc.fs.read_text(...)`, `listdir`, `read_bytes`, `write_text`, …) is the way in. See [docs/pitfalls.md — Python Realm](./pitfalls.md#python-realm-live-vfs-subprocess-and-the-opfs-fallback).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `-c "print([i**2 for i in range(5)])"`, `script.py [args...]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| **jshd**                                                    | `jshd-command.ts`          | Durable background `.jsh` units (one realm worker each). Records in `/workspace/.jshd/<name>.json`; logs in `/workspace/.jshd/log/<name>.log`. `ps` lists them (`kind: jsh`); `kill <pid>` stops without restarting. `--enable` restores the unit on reload. Thin-extension runtimes without `DedicatedWorker` still start units as best effort; `jshd ls` reports they are not durable there.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | `jshd start [-n <name>] [--enable] [--restart always\|on-failure\|no] [--cwd <dir>] [--env K=V] <script.jsh \| skill-command> [args...]`; `ls [--json]`; `status \| stop \| restart \| rm \| enable \| disable <name>`; `logs <name> [-f] [-n <lines>]`                                                                                                                                                                                                                                                                                                                                              |
...
| **screencapture**                                           | `screencapture-command.ts` | Capture user's screen (still or timed video) via browser screen sharing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `<output.png>`, `-c`, `-v`/`--view`, `--video` / `-V <s>` / `.webm`, `-g`/`--audio`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| **session**                                                 | `session-command.ts`       | Export a transcript bundle (active or frozen session) to the VFS as a signed, redacted ZIP. Redacts known secrets and credential-pattern strings before packaging; strips reasoning blocks; copies binary attachments unchanged. See [docs/transcript-export.md](transcript-export.md) for the full bundle layout and privacy guarantees. With the `memory-v2` feature flag on, also provides keyword `session search <query>` (title weighted 4× body; MiniSearch over `/sessions` **and every scoop snapshot dir** `/scoops/<folder>/sessions/<jid>/`, persisted in IndexedDB/VFS) and bounded `session read <id> [--from N --count M]` over JSONL sidecars — archives split prose markdown from machine-readable JSONL so grep no longer hits the embedded session JSON.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `session export [--id <frozen-session-id>] [--output <path>]` — default output `/workspace/slicc-transcript-<id>.zip`. With `memory-v2`: `session search <query> [--limit N]`; `session read <id> [--from N --count M]`                                                                                                                                                                                                                                                                                                                                                                              |
| **cost**                                                    | `cost-command.ts`          | Show session cost breakdown by origin. Defaults to the cone and live scoops; `--all` also includes dropped scoops and frozen sessions from `/sessions/index.json`. Silent one-shot children (`agent` and other `notifyOnComplete: false` scoops) fold their tokens/cost/model into the invoking parent on teardown, so bare `cost --json` keeps that spend on the parent and lists the child's model in `models` (#3437) — they do not appear as a separate dropped row under `--all`. Legacy frozen sessions without persisted cost remain visible with a dash. On a provider that bills against a rolling allowance (Adobe's `/v1/usage`), the report LEADS with that budget — percent USED of the window, a bar, and when it resets — and the session dollars follow below it. The Model column (and JSON `model`) is the model that unit is running now — its provider-qualified pin (a provider-less legacy pin yields to the latest turn when they differ), not the model with the most turns. JSON `models` lists each distinct model once; a bare Claude id and a region- or version-qualified spelling of the same model are one entry (a suffixed variant such as `-fast` is not), and the entry for the model in use now keeps the current spelling. `--json` emits the envelope `{ "budget": <window                                                                                                                                                                                                                                                                           | null>, "scoops": [ … ] }`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `--all`, `--json`, `-h`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| **usb**                                                     | `usb-command.ts`           | WebUSB access from the shell. Opaque device handles (`usb1`, `usb2`, …) back a page-side registry; control + bulk/interrupt transfers round-trip via panel-RPC. Handles are shared across the shell, realm `usb` global, and `slicc.usb` sprinkles, but interface claims are exclusive: a second `claim` is refused (or queued with `--wait`) naming the current holder, and `close`/`reset` refuse while another consumer holds a claim unless `--force` (which emits `claim-lost`/`disconnect` to the victim). `usb request` needs a real user gesture. `usb clear-halt` recovers a single stalled bulk/interrupt endpoint without the re-enumeration `usb reset` causes. Chromium-only; unavailable in the cloud / hosted-leader float.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `usb list`, `usb request [--vid 0x.. --pid 0x.. --class N --serial S]`, `usb open\|close\|reset <handle>`, `usb close\|reset --force <handle>`, `usb claim\|release <handle> <iface>`, `usb claim --wait`, `usb control-in\|control-out`, `usb transfer-in\|transfer-out`, `usb clear-halt <handle> <in\|out> <endpoint>`, `--raw`                                                                                                                                                                                                                                                                   |
...
| **tst**                                                     | `test-command.ts`          | Test runner over the [tst](https://github.com/dy/tst) library. Named `tst` because POSIX `test` / `[` are just-bash builtins and would silently swallow a command of that name (#3122). Discovers `*.test.{js,ts}` in `ctx.cwd`, TS-transpiles each, runs each file in its own realm. Relative `./…` imports are inlined from the VFS; bare / `node:` / `sliccy:` / ipk `require()`s fall through to the realm shim (same as `node`). Reporters: `tap` (default), `--reporter=spec` → tst `pretty`. Fork mode disabled. POSIX `test` stays the builtin.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `tst [glob]`, `--reporter=<tap\|spec>`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| **v86**                                                     | `v86-command.ts`           | Run an x86 virtual machine on the v86 wasm engine. Install-gated: `ipk add -g v86@<pinned>` provides both the JS glue and the wasm binary (nothing bundled, no CDN fallback). `v86 start` boots the guest as a named background unit in the kernel worker (ProcessManager-tracked, so `ps` / `kill` work); later invocations drive it: `type`/`key` (keyboard), `mouse` (PS/2 relative), `screenshot` (VGA → PNG in the VFS), `text` (text-mode dump), `serial` (send/read serial console), `state save/load`, `stop`. BIOS blobs are not in the npm tarball — download seabios/vgabios once into `/workspace/.v86/` (the command prints the exact curl lines). `-state` resumes a saved snapshot (`.zst` ok, skips BIOS), `-fs9p <url>` attaches a network-backed 9p root, `-net <ne2k\|virtio>[,relay=fetch]` picks the NIC — together these boot the copy.sh Arch Linux demo image straight into a shell; `relay=fetch` gives the guest outbound HTTP via v86's fetch relay, rerouted through the SLICC fetch proxy (guest DNS answered in-engine, plain-http to external hosts upgraded to https). `-vga <MiB>` sizes the Bochs-dispi SVGA/VBE framebuffer (default 8) for high-res VESA modes. `v86 start` registers `v86:<name>` as a computer; prefer `computer` for look-then-act (`v86 type`/`key`/`mouse`/`screenshot`/`text` remain as thin aliases). `v86 serve` is retired and prints `computer watch -c v86:<name>`.                                                                                                                                                         | `v86 start -cdrom alpine.iso`, `v86 start -state arch_state.bin.zst -fs9p https://i.copy.sh/arch/ -net virtio`, `v86 text`, `v86 screenshot`, `v86 type "root                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| "`, `v86 key ctrl-alt-del`, `v86 serial --tail`, `v86 stop` |
| **computer**                                                | `computer-command.ts`      | Look-then-act xdotool over a registered screen (`v86:<name>`, `tab:<id>`, `jsh:<name>`, `screen:<handle>`, `ssh:<runtimeId>`, `url:<host>`). Target: `-c` / `$COMPUTER` / last `computer use` / the only registered computer. `v86 start` registers `v86:<name>` (adopted pid — `computer rm` does not power the guest off). `computer add tab` refuses SLICC app tabs. `computer add screen` shares this display (needs a user gesture). `computer add ssh <follower>` is a tray follower's desktop. `computer add url <http(s)://base>` is an HTTP remote (`GET /computer`; node-server `--computer-demo` is the in-tree reference). Every poke writes a frozen JPEG and prints `target: <id>` then `screen: <path>`. Coordinates are screenshot-space of the last shot unless `--native`. Size presets: `low` 256 / `medium` 768 / `high` 1536. Anthropic aliases (`left_click`, `mouse_move`, …) map onto the same verbs. Prefer this over `v86 type\|key\|mouse\|screenshot\|text`. Protocol: [computer-protocol.md](./computer-protocol.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | `computer ls`, `computer add tab <id>`, `computer add url http://127.0.0.1:5710`, `computer screenshot`, `computer click 1 type hello`, `computer use v86:arch`                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| **ps**                                                      | `ps-command.ts`            | List active processes from `ProcessManager` — scoop turns, tool calls, shell execs, jsh/python scripts. Equivalent to inspecting `/proc/` directly. The default `SCOOP` column is a 10-character prefix (`cone`, `system`, `jshd`, or the start of the scoop jid). `ps -o` / `--columns` that names `scoop` prints the jid in full, so two scoops that share a prefix stay distinct and can be joined back to a `slicc.selectScoop` target. `STAT` and `PID` are unchanged.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `ps`, `ps -o scoop,stat,pid`, `--all`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| **uptime**                                                  | `uptime-command.ts`        | How long this SLICC has been up, measured from the last load of the browser window — a reload is a reboot. The page realm hands its `performance.timeOrigin` to the kernel worker in `kernel-worker-init` (`base/page-load-time.ts`), because the worker's own origin is later than the page's. `-p`/`--pretty` prints words, `-s`/`--since` prints the boot timestamp. The load average is real, not simulated: it is the average number of live kernel processes over the last 1/5/15 minutes, integrated exactly from the `startedAt`/`finishedAt` records `ps` reads. `ProcessManager` retains a bounded number of terminated records, so the older windows under-report after a long burst of short commands. The user count is always 1 — a SLICC runtime has one user identity (see `id`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| **say**                                                     | `say-command.ts`           | macOS `say`-equivalent — speak the text via Kokoro (on-device) or the Web Speech API. A TTY plays aloud; a non-TTY stdout (or `-o -`) writes 16-bit mono WAV bytes (Kokoro-only; fails loudly if the engine is not ready — never a zero-byte stream).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `say -l en-US "Hello"`, `-v <voice>`, `-o file.wav`, `-o -`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **hear**                                                    | `hear-command.ts`          | Push-to-talk voice transcription, or transcribe a file / stdin. **Page realm only** (mic, AudioContext) — the kernel worker bridges over the `hear-*` panel-RPC ops. Two engines: Web Speech API (immediate), hot-swapped to on-device Whisper (`onnx-community/whisper-tiny`) once ready. `-i -` and a non-TTY stdin (no `-i` / `-d`) read stdin once. Import the speech interface from `@slicc/webcomponents/composer/speech` (deep subpath) — never the barrel, which registers custom elements at import time and breaks DOM-less realms (see `docs/pitfalls.md` § "Speech: Never Import the Barrel in DOM-less Realms").                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `hear` (one-shot capture), `hear -i clip.wav`, `hear -i -`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
### `git diff --no-index` needs no repository

`git diff --no-index <path> <path>` compares two arbitrary paths and is the only
`git diff` mode that works from a cwd which is not inside a repository at all —
it is the standard way to get a unified diff of two loose files. It never calls
isomorphic-git: `git/commands/diff-no-index.ts` reads both operands straight off
the VFS and hands them to the same `unifiedDiff` / `--stat` renderer every other
diff mode uses.

- **Operands.** Two paths, or the usage line and exit 129. File-vs-file,
  directory-vs-directory (recursive, union of both trees, sorted by relative
  path), and `<dir> <file>` in either order — which git resolves to
  `<dir>/<basename of file>`. `/dev/null` is an empty file whether or not the
  path exists, so `git diff --no-index /dev/null new.txt` yields an add patch.
- **Exit code.** `--no-index` implies `--exit-code`: 1 when the paths differ, 0
  when they are identical (with no output). A path that cannot be read is
  `error: Could not access '<path>'` and exit 1, never an empty diff — the left
  operand is reported first.
- **Output flags.** `--name-only` (prints the b-side path, or `/dev/null` for a
  deletion), `--stat` (rows labeled with git's rename-compressed
  `{A => B}/f.txt` form, `Bin <n> -> <m> bytes` for binary pairs), and `-U<n>` /
  `--unified=<n>`. A pair with a NUL in its first 8000 bytes is reported as
  `Binary files a/X and b/Y differ` instead of being decoded and line-diffed.
- **Deviations from canonical git**, both deliberate: no `index` /
...
the next positional). It applies to every diff mode, not just `--no-index`.

### Hunk headers are byte-compatible with git

`unifiedDiff` (`git/diff.ts`) backs `diff`, `log -p`, `show`, `stash`, and
`--no-index`, so its `@@` line follows git exactly: a side that contributes no
lines is numbered by what came BEFORE the hunk (`@@ -0,0 +1,3 @@` for an
addition to an empty file, not `-1,0`), and a one-line side omits its count
...
without putting it on argv. The path is a VFS path (relative to cwd, or
absolute). `--file -` reads stdin, so
`printf 'subject\n\nbody\n' | git commit -F -` works the same as real git.
Unknown commit flags name the switch that was passed (`unknown option 'bogus'`,
`unknown switch 'Q'`) instead of claiming `-m` is missing (#2865).

### `git push` boolean flags do not swallow the remote
...
# Show the current leader tray status, join URL, and followers
host

# status is the runtime role (leader, follower, inactive, ...)
# join_url exposes the tray join capability when a session exists
# followers: lists connected exec/browser-capable followers under an active session
...
screencapture --video -V 10 clip.webm  # Timed screen recording
...
- **Exit codes** mirror curl: `0` ok, `2` bad usage, `7` the fetch failed, `22` with `--fail` on HTTP >= 400, `23` write error, `26` an unreadable input file, `28` `--max-time` expired. The one code that is NOT curl's is **`130`** — the run was cancelled (see below); nothing curl models happened, and reporting `28` there would read as a slow server worth retrying.
- **Giving up cancels the request.** When the `bash` call stops waiting — turn cancelled, `timeout` fired, `kill <pid>` — `curlwright` stops instead of running on: it drops out of the tab's queue rather than waiting its turn, stops before its next CDP round trip, and writes no `-o` / `-D` file and no `-w` line for a response that arrived after its caller left. Exit **130**, with the step it stopped at on stderr (`-v` traces still print; `-s` hides the reason like any other message). The in-page `fetch` itself rides ONE `Runtime.evaluate` and cannot be recalled once it is on the wire, so it is bounded by `-m` / `--max-time`, not by the cancellation — a cancelled request may still have reached the server.
...
**The sha is the fast path.** When the recorded commit still equals the latest commit that touched the skill's upstream path and every recorded file is still on disk, `update` reports `already current` from one ~200-byte commits call and never downloads the archive. A moved sha, a missing file, or no recorded sha falls through to the full content compare. Installs record that path commit even without a `github.token` (one commits lookup; the zip/codeload path still supplies the files). `navigate·upskill` uses the same sha to skip a no-op approval card.
...
| `mount info [--json] <path>`                        | Probe the volume semantics the VFS bridge actually applies at `<path>`: case sensitivity, Unicode normalization (byte-exact vs insensitive, and NFC/NFD storage if observable), executable-bit support, round-tripped name byte-exactness, max filename length, writability, and whether writes are host-backed. Creates a scratch entry, stats it back, and always removes it. `--json` emits the programmatic `MountInfo` object (`probeMountInfo()` in `fs/mount/probe-info.ts`). Do not discover this by renaming — that experiment is destructive on insensitive volumes (see #3107). |
...
| `--json`              | mount info            | Emit the programmatic `MountInfo` object instead of the human-readable report.                                                                                                                                              |
...
mount info /tmp                      # probed case / Unicode / exec-bit report
mount info --json /mnt/kb            # same as JSON (tooling)
...
- **10 `--ttl` snapshots per tray; live previews have no quota.** Snapshots hold R2 storage
  ...
  `serve: Preview mint failed: Snapshot limit reached (N of 10 --ttl snapshots in use, …)`;
  `serve --list` shows every snapshot (MODE `persistent`, or `uploading` while the upload
  runs) and `serve --stop <token>` frees a slot. The quota moves with the tray across roves.
- **Live previews expire with the leader.** A live preview is served from the leader's VFS, so
  once the leader has been disconnected for 5 minutes the worker deletes it (the grace covers
  page reloads and network blips); its URL then answers 404. A `--ttl` snapshot lasts until its
  TTL expires, whether or not the leader is connected. `serve --stop` removes either sooner.
...
- `--stop <token-or-url>` — Revoke the preview (for a snapshot, this frees its quota slot): closes bridge sockets, rejects new connections, deletes the auto-provisioned webhook. Accepts the printed token, the `serve --list` token, or the preview URL.
...
Handles are **shared** across the `usb` shell command, the realm `usb` global (`require('sliccy:usb')`), and `slicc.usb` in sprinkles — the same `usb1` object. Interface claims are **exclusive** per `(handle, interfaceNumber)`: a second `claim` from another consumer is refused with a message naming the current holder (or queued with `--wait` / `{ wait: true }`). Bridged `--wait` uses a 5-minute panel-RPC timeout; a timeout cancels the page-side waiter (including an in-flight grant that has not finished `claimInterface`) so it cannot take the interface after the command has already failed. `close` and `reset` refuse while another consumer holds a claim; pass `--force` / `{ force: true }` to displace them. Forced displacement logs a warning and emits `claim-lost` then `disconnect` on the USB bridge so the victim is told rather than left inferring from a failed transfer. `usb clear-halt` remains the targeted alternative to `reset`.
...
| `usb open\|close\|reset <handle>`              | Open, close, or reset a device. `close`/`reset` refuse while another consumer holds a claim unless `--force`. |
| `usb select-config <handle> <value>`           | Select a device configuration.                                                                                |
| `usb claim\|release <handle> <interface>`      | Claim or release an interface. A second claim is refused (names the holder) unless `--wait`.                  |
...
| `--force`                                        | `close` / `reset`            | Displace other holders; emits `claim-lost`/`disconnect`.   |
| `--wait`                                         | `claim`                      | Queue behind the current holder instead of refusing.       |
...
Look-then-act xdotool over a registered screen (`packages/webapp/src/shell/supplemental-commands/computer/`). Wire types and adapters: [computer-protocol.md](./computer-protocol.md). Agent skill: `/workspace/skills/computer/SKILL.md`.

A **computer** is any screen the agent can screenshot and poke. Backends: `v86:<name>` (registered by `v86 start`), `tab:<targetId>` (`computer add tab`), `jsh:<name>` (`sliccy:computer.register` from a `.jsh` / `jshd` unit), `screen:<handle>` (`computer add screen` — persistent display share), `ssh:<runtimeId>` (`computer add ssh` — follower desktop), `url:<host><path>` (`computer add url` — HTTP remote). Registered computers also appear as cards after browser tabs in the Browser overlay (live lightbox + bash-row frames).

Target resolution: `-c` / `--computer`, else `$COMPUTER`, else last `computer use`, else the only registered computer. `computer ls` lists ids. A `-c` query matches an id first, then the `-n` name (`NAME` in `ls` — fixed for the life of the registration), then the live title (`TITLE` — what the surface currently calls itself, which a tab rewrites from `document.title` on every capture).

Every poke writes a frozen JPEG to `$TMPDIR/computer/<name>/<seq>.jpg` and prints `target: <id>` (resolved target) then `screen: <path>`. The extension follows the bytes, so it is `.png` when a backend returns PNG the runtime could not transcode. `ls` / `add` / `rm` / `use` and `--json` omit the stamp. Coordinates are screenshot-space of the last shot unless `--native` (the pixel space the descriptor advertises). Tab captures are device pixels; the tab adapter divides by `devicePixelRatio` before CDP input so a screenshot-space click lands on the visual target. Size presets: `low` 256, `medium` 768 (default), `high` 1536.

`computer rm` unregisters. For a v86 guest that **does not** power the VM off (the pid was adopted). For `screen:` it **does** stop the getDisplayMedia tracks. `computer add tab` refuses SLICC app tabs. `computer add screen` needs a user gesture (panel terminal or cone approval card) and cannot restore on `jshd --enable`. `computer add ssh` probes the follower at add; a `capabilities.computer` Mac (Swift launcher) uses ScreenCaptureKit/CGEvent instead of `screencapture`/`cliclick`, capturing its **main** display in real pixels unless `--display <n>` picks another (1-based, same numbering as `screencapture -D <n>`; the id becomes `ssh:<runtimeId>:display:<n>`). `--allow-input` is a sudo hop so a phone can answer with Face ID. The iOS follower itself is never a driven computer (a real iPhone is out of scope; `--sim <udid>` drives a Simulator on a Mac follower via `simctl`/`idb`, bypassing native capture even on a `capabilities.computer` Mac). `computer add url` probes `GET /computer` over proxied fetch; trailing `/computer` is stripped. node-server `--computer-demo` mounts the in-tree reference on the bridge port.

Prefer `computer` over `v86 type|key|mouse|screenshot|text`; those remain as thin aliases.

### Subcommands

| Form                                                                   | Behavior                                                                                                                                                       |
| ---------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `computer ls`                                                          | List registered computers — `NAME` is the `-n` handle, `TITLE` the live title (`screen` rows include `[display slot]`; `ssh` rows `[input]` or `[view-only]`). |
...
| `computer record [-V\|--duration SEC] [--fps N] [file]`                | Timed clip. `screen` uses the live session recorder; other kinds pipe JPEG stills through ffmpeg wasm at the measured capture rate (max 60s).                  |
...
```bash
# v86 guest (boot still uses v86 start)
...
# browser tab (not a SLICC app tab)
playwright-cli tab-list
computer add tab <targetId> -n docs
computer screenshot -c tab:<targetId>
computer click 1 --at 100,80 type hello

# display share (panel terminal or cone approval card)
computer add screen -n desk
computer screenshot
computer record -V 5 clip.webm
computer rm

# follower desktop (exec-capable tray follower from `ssh --list`)
computer add ssh follower-abc -n desk
computer screenshot
computer add ssh follower-abc --allow-input   # sudo hop; ls shows [input]
computer click 1 --at 100,80

# HTTP remote (node-server --computer-demo on the bridge port)
...
the sync and async forms (`env` replaces the child environment, including
when the spawned file is `sh`/`bash -c`; a missing `cwd` is `ENOENT`).
`fork` always throws — no long-lived process model. `.bsh` scripts run in
the target page (no shell bridge), so `child_process` is unavailable there.
...
| --------------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `slicc.selectScoop(target)` | `Promise<boolean>`        | **Sprinkle-only** (trusted dips do not expose this method). Switch the app's view to an already-running scoop or cone (same as a switcher-chip click). `target` is `'scoop:<name>'`, `'cone:<folder>'`, or a bare `'cone'`. Resolves `true` when the target matched the live roster (including when that unit is already selected); `false` — never thrown — when the string is outside that grammar, nothing matches, or the user is mid-draft in the focused composer. |
| `slicc.selectedScoop()`     | `Promise<string \| null>` | **Sprinkle-only** (trusted dips do not expose this method). The shell's current selection in the same grammar `selectScoop` accepts (`'cone'`, `'cone:<folder>'`, `'scoop:<name>'`). Resolves `null` — never thrown — when nothing is selected or that unit is no longer on the live roster. A read: it does not switch the view.                                                                                                                                        |
...
| `slicc.usb.open(handle)` / `slicc.usb.close(handle, opts?)`         | `Promise<void>`                                                | Also `reset(h, opts?)`, `selectConfiguration(h, value)`, `claimInterface(h, n, opts?)`, `releaseInterface(h, n)`, `clearHalt(h, direction, ep)`. `opts.force` on close/reset and `opts.wait` on claim match the shell flags. Claims are exclusive across shell / realm / sprinkle consumers of the same handle. |
| `slicc.usb.on('disconnect'\|'claim-lost', cb)` / `off(...)`         | `void`                                                         | Fired when another consumer force-closes/resets a device this sprinkle had claimed. Payload: `{ type, handle, holder, displacedBy, reason, interfaceNumber? }`.                                                                                                                                                 |
...
2. **just-bash limitations**: Bash strings are Unicode; binary data must be encoded. Bundled `rg` throws `ExecutionLimitError` when its live/aggregate byte budget is exceeded, which aborts the whole script and bypasses `2>` — SLICC's `rg` overlay (`rg-command.ts`) turns that into a command exit and does not charge binary-skipped or `--max-filesize` files. The same overlay omits a `-c` / `--count-matches` zero on stdin (#3138), matching file-argument and multi-file behaviour.
