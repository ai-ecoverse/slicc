...
- **Extension-delegate leader** (pinned hosted tab): `webhook` URLs come from the connected tray worker (requires an active leader tray; otherwise `webhook create` reports "connect a leader tray"); `crontask` runs on the in-tab worker `LickManager` and fires only while the leader tab is open (tab-lifetime).
