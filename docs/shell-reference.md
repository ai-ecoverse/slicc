...
- **Teleport restores auth state**: `playwright teleport --start=<regex> --return=<regex>` transfers both cookies and page storage (`localStorage` + `sessionStorage`) back to the leader so SPA-style auth handoffs can resume without a manual storage workaround. For cross-origin SSO flows, teleport hydrates the captured app origin first, then lands on the best matching app URL.
