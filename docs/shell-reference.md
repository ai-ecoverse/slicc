...
Every poke writes a frozen JPEG to `$TMPDIR/computer/<name>/<seq>.jpg` and prints `target: <id>` (resolved target) then `screen: <path>`. `ls` / `add` / `rm` / `use` and `--json` omit the stamp. Coordinates are screenshot-space of the last shot unless `--native`. Size presets: `low` 256, `medium` 768 (default), `high` 1536.
