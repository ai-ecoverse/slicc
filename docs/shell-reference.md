...
| **python3 / python**                        | `python-command.ts`        | Execute Python code via Pyodide. Mounted directories (local FS Access, S3 / R2, da.live) are visible and writable from Python — OPFS-backed paths bridge directly via the `OPFS_SYNC_FS` plugin; remote (s3/da) mounts are materialized into MEMFS over the `vfs` RPC channel, capped by `--remote-mount-cap` (default `5m`, `0` to skip remote materialization). See [docs/pitfalls.md — Hybrid Mount Visibility](./pitfalls.md#python-realm-hybrid-mount-visibility--diff-aware-vfs-sync).                                                                                                                                                                                                                                                                                                                              | `-c "print([i**2 for i in range(5)])"`, `script.py [args...]`, `--remote-mount-cap=<size>` (`5m` default, `0` to skip remote mounts)                                                                                                                                                                                         |
