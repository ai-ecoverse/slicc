...
| **rg**                                                      | `rg-command.ts`            | Overlay on just-bash `rg` (#3106). A byte-limit miss is a normal non-zero exit with capturable stderr, not a script abort; skipped binaries and `--max-filesize` files do not count; the printed number is the searchable budget actually enforced (`min(maxInputBytes, floor(maxLiveBytes / 2))`, 256 MiB at the default 512 MiB live/input caps). Bundled search still runs via `ctx.origCommand` once the set is under budget. Delete when upstream returns a command result for this limit, reports the trip point, and does not charge skipped binaries.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `rg PATTERN [PATH …]`; `-a` / `--text` counts binaries                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
2. **just-bash limitations**: Bash strings are Unicode; binary data must be encoded. Bundled `rg` throws `ExecutionLimitError` when its live/aggregate byte budget is exceeded, which aborts the whole script and bypasses `2>` — SLICC's `rg` overlay (`rg-command.ts`) turns that into a command exit and does not charge binary-skipped or `--max-filesize` files.
