...
| **ffmpeg**                                  | `ffmpeg-command.ts`        | ffmpeg.wasm — audio/video transcoding and processing. On a cross-origin-isolated runtime the loader prefers the multi-threaded `@ffmpeg/core-mt` when installed (`ipk add -g @ffmpeg/core-mt@<pinned>`); `@ffmpeg/core` stays the universal fallback. The concat demuxer (`-f concat -i list.txt`) is supported: files named inside the list are staged from the VFS too, resolved against the list file's directory, with ffmpeg's `safe` rule enforced (absolute or parent-traversing members need `-safe 0`). The `concat:` protocol form is not.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `ffmpeg -i input.mp4 -vcodec libx264 output.mp4`                                                                                                                                                                                                                                                                             |
