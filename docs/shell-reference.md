...
| **computer**                                                | `computer-command.ts`      | Look-then-act xdotool over a registered screen (`v86:<name>`, `tab:<id>`, `jsh:<name>`, `screen:<handle>`, `ssh:<runtimeId>`). Target: `-c` / `$COMPUTER` / last `computer use` / the only registered computer. `v86 start` registers `v86:<name>` (adopted pid — `computer rm` does not power the guest off). `computer add tab` refuses SLICC app tabs. `computer add screen` shares this display (needs a user gesture). `computer add ssh <follower>` is a tray follower's desktop. Every poke writes a frozen JPEG and prints `target: <id>` then `screen: <path>`. Coordinates are screenshot-space of the last shot unless `--native`. Size presets: `low` 256 / `medium` 768 / `high` 1536. Anthropic aliases (`left_click`, `mouse_move`, …) map onto the same verbs. Prefer this over `v86 type\|key\|mouse\|screenshot\|text`. Protocol: [computer-protocol.md](./computer-protocol.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | `computer ls`, `computer add tab <id>`, `computer screenshot`, `computer click 1 type hello`, `computer use v86:arch`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
A **computer** is any screen the agent can screenshot and poke. Backends: `v86:<name>` (registered by `v86 start`), `tab:<targetId>` (`computer add tab`), `jsh:<name>` (`sliccy:computer.register` from a `.jsh` / `jshd` unit), `screen:<handle>` (`computer add screen` — persistent display share), `ssh:<runtimeId>` (`computer add ssh` — follower desktop). `url` is later. Registered computers also appear as cards after browser tabs in the Browser overlay (live lightbox + bash-row frames).
