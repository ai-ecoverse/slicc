...
- Path: `core/base64-mentions.ts` (heuristic, pure) → `core/base64-payload.ts` (decode + identify) → `ui/base64-preview-linker.ts` (DOM swap) → `ui/wc/wire-base64-previews.ts` (lifecycle + Quick Look). Chip: `<slicc-blob-chip>` in `@slicc/webcomponents`.
...
- **No VFS, ever.** Verification is a synchronous decode, so this wiring adds nothing to the boot critical path (contrast `wire-file-mentions.ts`, which opens the page-side VFS client).
