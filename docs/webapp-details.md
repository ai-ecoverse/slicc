...
- **Keyboard shortcuts** (`ui/wc/wc-shortcuts.ts`): installed by `mountWcShell`, so leader, extension popout, follower and the design-time preview share one set. A digit resolves against `switcher.scoops` — the strip AS RENDERED (`work-unit/client/presentation.ts` ordering) — and calls `switcher.select()`, which fires the same `slicc-scoop-select` a click does; the shortcut therefore inherits each float's selection semantics (queue stashing, read-only locks, tray round trips) instead of re-implementing them. Modifier is Ctrl on macOS, Ctrl+Shift elsewhere, because ⌘+digit and (off macOS) Ctrl+digit are the browser's tab switcher; digits come from `event.code`, so Shift does not turn `3` into `#`. `9` is the LAST unit, not the ninth. Unmodified keys (`?`, `/`, `c`) are gated by `isTypingTarget`, which reads `composedPath()` so a textarea inside a shadow root counts as typing. The help overlay is a `<slicc-dialog>` on `document.body`; the avatar menu's **Keyboard shortcuts** item (leader only — the follower never runs `wireWcNav`) calls the same `showHelp`.
