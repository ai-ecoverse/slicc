...
| `POST /api/tray/:trayId/preview`           | Mint a preview token; body `{ path, bridge?, maxTabs?, quiet?, webhookId?, ttlMs? }`; response `{ previewToken, url }`; `429 PREVIEW_LIMIT` `{ active, limit }` once 10 `--ttl` snapshots (uploads in progress included) exist; live previews have no quota (`429 LIVE_PREVIEW_LIMIT` only past 200, a tray-record guard) |
...
| `GET <token>.sliccy.now/*`                 | Preview HTTP pipe — leader sends 64 KiB chunks the DO buffers (≤ 25 MiB, else `413`); `Range` → `206`/`416` in ≤ 8 MiB windows, uncached; 30s timeout; bridge mode injects the preview-bridge script                                                                                                                      |
...
| `GET /api/models/providers/:id`            | Same-origin relay for pi's hosted model catalogue (`https://pi.dev/api/models/providers/<id>`, which sends no CORS headers); slug-validated id, 5-min edge cache, `ETag`/`Last-Modified` passthrough, `304` on a matching `If-None-Match`                                                                                 |
...
- Preview continuity (`preview-continuity.ts`) uses authenticated transfer, not a public
  ...
  most 24 hours; cleanup records do not consume the ten snapshot slots. Neither
