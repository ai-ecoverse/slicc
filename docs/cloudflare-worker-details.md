...
| `GET <token>.sliccy.now/*`                 | Preview HTTP pipe — leader sends 64 KiB chunks, the DO decodes each and buffers the file (≤ 25 MiB, else `413`); 30s timeout; bridge mode injects the preview-bridge script                                                                                                                                               |
