...
| `POST /api/tray/:trayId/preview`           | Mint a preview token; body `{ path, bridge?, maxTabs?, quiet?, webhookId? }`; response `{ previewToken, url }`; `429 PREVIEW_LIMIT` with `{ active, limit }` once 10 live/`--ttl`/pending previews exist |
