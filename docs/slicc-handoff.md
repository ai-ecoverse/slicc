...
3. The cone resolves the lick by verb:
   ...
   - **upskill** — agent-actionable via the `lick_confirm` / `lick_dismiss` tools: `lick_confirm` runs `upskill [--branch ..] [--path ..] <target> --all` (honoring the body's scope) and flips the card to ✓. `--all` is required — without a skill selector `upskill` only lists what it found and installs nothing — so a confirm installs **every** skill under the advertised `path`, which for a broad path (a plugin directory) can be many. `lick_dismiss` drops it (muted ✗). Before the card is raised, SLICC compares the advertised GitHub path's commit to the recorded `.upskill` sha and **skips silently** when they match, or when the matching install has no recorded sha (we cannot tell an install is needed). A skill that is not installed at all still raises a card.
