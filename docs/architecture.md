...
| `preemptive-runner.ts`         | `runPreemptiveJs(opts)` — spawns a `kind:'preemptive'` process backed by a per-task fresh DedicatedWorker. SIGKILL → `worker.terminate()` (uncatchable). SIGINT/TERM record state but don't terminate the worker (cooperative-only — the running JS is opaque).                                                                                                                                                            |
| `preemptive-worker.ts`         | DedicatedWorker entry for the preemptive runner. Runs JS code inside an `AsyncFunction` with shimmed `console` / `process.argv` / `process.env` / `process.stdout` / `process.stderr` / `process.exit(N)`; posts `preemptive-done` or `preemptive-error`. No FS access.                                                                                                                                                    |
