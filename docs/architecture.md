...
| `tray-follower-sync.ts`     | Follower-side `FollowerSyncManager` (TS — used by browser standalone follower and extension offscreen follower). Implements `AgentHandle`, receives agent events / snapshots / sprinkle messages from leader, advertises local browser targets, forwards CDP+FS for federated targets. `scoops.list` is currently a no-op on the TS side (no scoop switcher yet — iOS consumes it for swipe navigation)                                      |
...
| Follower→Leader | `tab.open.error`                                            | TS only         | iOS embeds CDP errors in `cdp.response.error` and always sends `.tabOpened` for tab.open                                             |
