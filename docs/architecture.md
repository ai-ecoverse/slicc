...
| `mount/`               | Backend abstraction. `backend.ts` (`MountBackend` interface), `backend-local.ts` (FS Access), `backend-s3.ts` (HTTP + SigV4, supports R2/MinIO via custom endpoints), `backend-da.ts` (HTTP + IMS bearer for da.live), `remote-cache.ts` (TTL + ETag, IDB-backed), `signing-s3.ts` (pure SigV4 v4 via Web Crypto), `fetch-with-budget.ts` (timeout + retry + abort), `profile.ts` (cred resolution from `s3.<profile>.*` secrets / IMS), `mount-id.ts` (UUID generator) |
| `index.ts`             | Re-exports                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
