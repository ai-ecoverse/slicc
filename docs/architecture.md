...
| `tray-sync-protocol.ts`     | Typed `LeaderToFollowerMessage` / `FollowerToLeaderMessage` unions + chunked snapshot/CDP/sprinkle helpers. Canonical wire format — the iOS follower's `SyncProtocol.swift` mirrors a **subset** (chat/snapshot/scoops/sprinkles/ping/pong + partial CDP/tabs); federated `fs.*`, CDP responses/events, follower-initiated `tab.open`, and `cdp.request` are TS-only. See `Multi-Browser Sync (Tray) Architecture` below for the full matrix |
...
- iOS implements: chat (snapshot/snapshot_chunk/agent_event/user_message_echo), control (status/error/ping/pong), scoops list, full sprinkle round-trip, partial CDP (one-way `cdp.request` from leader + `targets.registry`), `tab.open` (leader → follower only).
- TS-only: federated `fs.request`/`fs.response` in both directions, leader→follower `cdp.response`/`cdp.event`/`tab.opened`/`tab.open.error`, follower→leader `cdp.request`/`cdp.response`/`cdp.event`/`tab.open`/`tab.opened`/`tab.open.error`.
...
| Leader→Follower | `targets.registry`, `cdp.request`, `tab.open`               | TS + iOS (partial) | Federated CDP across runtimes — leader→follower direction                                                                            |
| Leader→Follower | `cdp.response`, `cdp.event`, `tab.opened`, `tab.open.error` | TS only            | Reply path for leader-initiated federated CDP / tab.open against a follower                                                          |
...
| Follower→Leader | `request_snapshot`, `scoops.select`                         | TS + iOS           | Request a fresh snapshot (optionally for a specific scoop the follower has selected)                                                 |
...
| Follower→Leader | `cdp.request`, `cdp.response`, `cdp.event`                  | TS only            | Federated CDP — follower-initiated reach back into another runtime via the leader                                                    |
| Follower→Leader | `tab.open`, `tab.opened`, `tab.open.error`                  | TS only            | Follower-initiated tab open against a remote runtime, and the reply path                                                             |
...
The follower's rendered sprinkle has **no VFS access** to the leader. Both follower implementations stub the file-IO bridge methods (`readFile`, `writeFile`, `readDir`, `stat`, `mkdir`, `rm`, `screenshot`) and reject calls — see `SprinkleFollowerController.createBridge` (TS) and `SprinkleWebView.bridgeJS` (iOS). Sprinkles that rely on these should feature-detect and degrade.
