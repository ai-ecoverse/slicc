...
| `scripting`                           | Programmatically injects the on-demand per-page sidebar (an ISOLATED-world relay + a MAIN-world launcher) into the tab the user clicks the toolbar icon on, and re-injects it on that tab's reloads while it stays open. There is no `content_scripts` entry — nothing is injected until the user explicitly activates the sidebar. |
| `activeTab`                           | Grants access to the tab the user activates via the toolbar-icon click so the on-demand sidebar can be injected into exactly that tab on the user's gesture, rather than requesting standing access to every page.                                                                                                                  |
| `sidePanel`                           | Opens the Chrome side panel to display the hosted leader UI as an alternative to the pinned tab, providing a persistent agent interface alongside the user's browsing without taking up tab space.                                                                                                                                  |
