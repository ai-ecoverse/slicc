...
- **Well-known probe** — a background `GET` of `/.well-known/ai-catalog.json` and `/llms.txt`, at most once per origin per session (deduped + timed out + `.catch`ed). Routed through `createProxiedFetch()` (CLI) / the SW's `host_permissions` fetch (extension) so it inherits CORS bypass. Probes are **credential-free and never follow redirects** (`credentials: 'omit'` + `redirect: 'manual'` in `probeWellKnown`): the probe fires while the user's tab may be mid-SSO on the same origin, and a credentialed probe that chases the auth-bounce chain overwrites the IdP's cookie-keyed return-URL slot — the tab then lands on `<origin>/llms.txt` after completing Okta/AAD auth. An artifact that doesn't answer `200` anonymously at its canonical location is treated as "no match".

Wiring: CLI/Electron/hosted-leader via `NavigationWatcher` (`packages/webapp/src/cdp/navigation-watcher.ts`) — target discovery, the sessions it attaches and the `Page`/`Network` domains it enables are all CONNECTION-scoped, so the watcher follows `transport.onStateChange` and does a clean internal restart (clear sessions → re-arm listeners → `Target.setDiscoverTargets` → re-enumerate) on the replacement connection after a `/cdp` upstream reset; the extension via `createDiscoveryObserver` in `service-worker.ts`; followers forward `discovery` licks to the leader. The watcher attaches to every page target, but keeps the `Network` domain OFF on SLICC's own app tab, which its `isOwnTab` option identifies by origin + pathname (`createOwnTabMatcher`; the page URL arrives as `KernelWorkerInitMsg.appPageUrl` in the worker and from `location.href` on the follower page). With `Network` on, Chrome reports that tab's own `/cdp` socket back as `Network.webSocketFrame*` events, which is what overflows swift-server's 1,000-message inbound pump (issue #2417). Path is part of the test on purpose: handoff pages are served from the app's OWN origin (`/handoff?...`) and must stay watched. `Page` stays on, so `Page.frameRequestedNavigation` / `Page.frameStartedNavigating` can arm `Network` at the START of a navigation off the app URL — early enough to still see that navigation's document response — and `Target.targetInfoChanged` turns it back off for a tab that navigates INTO the app URL. Only live top-level navigation carries valid discovery provenance, and restricted sub-agents without browsing commands do not receive these licks. Forwarders re-send an advertisement on every navigation, so the leader fingerprints `discovery` (and `navigate`) licks on the receiving side too — `LickManager.handleForwardedEvent` dedupes on artifact identity exactly like `emitEvent`.
