...
- **It pins itself** (`position: absolute; bottom: 0`) to the nearest
  ...
  and the dock rail sit at 3). A `::after` bleed (`left: 100%; width: 100vw`,
  `pointer-events: none`) extends the bar under an open tool pane the way the
  composer's own `::before` does.
