...
`require` shim ever reappears in the ESM bundle. The patch was removed in
PR #1782 (bump 2.35.1 → 2.35.3). `packages/cloud-core/tests/e2b-workerd-patch.test.ts`
stays as a defensive regression guard on our side — it fails if the eager shim
ever reappears in the installed dist. If e2b regresses this in a future
release, either land a fresh `patches/e2b+<ver>.patch` (`patch-package`) or
pin the range in `packages/cloud-core/package.json` until upstream fixes it —
do **not** un-bundle e2b from the worker, since the worker genuinely calls
the SDK at runtime.
