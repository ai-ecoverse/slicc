...
## OPFS Startup: Bound Preload Reads Across the Tree

ZenFS's `Async.crossCopy` preloads file contents into its synchronous cache.
Unbounded recursion starts every payload read and file-sized buffer allocation
...
Host candidates gather during `setLocalDescription`, which is before the offer or answer is on the wire. Chrome rejects `addIceCandidate` until the remote description is set (`ICE candidates can't be added without any remote session description`) and drops that candidate. The one it drops is the host candidate, so a CLI on the same machine as the leader loses the path that does not need TURN; TURN `CreatePermission` 403s then fail the dial with `io: read/write on closed pipe`. Hold local candidates until the offer or answer has been sent, hold remote candidates until `setRemoteDescription` resolves, and apply one bootstrap's signaling in arrival order. The Go follower does the same in `packages/slicc-cli/internal/tray`.
...
build SHIPS. The total-JS-payload budget counts every emitted file. The inverse
