...
throws ENOENT for a missing path. With the SW/SAB bridge it sets the times on
the live file at call time. Without one the cache models no times, so it's a
no-op, like `chmodSync`. It also leaves two cases as they are: a mount that
stores no times (hostfs, File System Access) and a file that so far exists
only in the cache.
