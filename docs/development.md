...
| `cd packages/cloudflare-worker && WORKER_BASE_URL=https://... npm test -- tests/deployed.test.ts` | Run the deployed tray-hub smoke test                                                                                      | Verify the live Worker contract (`POST /tray`, `POST /handoffs`, controller attach, leader WebSocket) |
...
The same worker also serves short-lived handoffs:

- `POST /handoffs`
- `GET /handoffs/:id`
- `GET /handoffs/:id.json`

These handoffs are stored in R2 with an app-enforced 24-hour expiry. Provision the production and staging `HANDOFFS` buckets separately, then configure a 1-day lifecycle cleanup rule in Cloudflare so expired objects are eventually deleted.
