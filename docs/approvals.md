...
| ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- |
| **navigate · upskill**              | Runs `upskill [--branch ..] [--path ..] <url> --all`, installing **every** skill under the advertised scope. The card is skipped when the advertised path is already at the recorded install commit, or the matching install has no sha. | Drops the install.                            |
...
**`slicc <url> follow --computer` (#3260)** moves the two TCC prompts off the agent's critical path. The CLI runs `Sliccstart --computer-preflight` at startup, so Screen Recording and Accessibility are asked for once, in front of the human who typed the command, instead of landing mid-turn on whichever `computer screenshot` the agent happened to run. The prompts are raised by the signed Sliccstart bundle — a bare CLI binary would be attributed by TCC to its responsible process (the terminal), so the dialog would name Terminal/iTerm and the grant would be the terminal's, not SLICC's. A partial grant is a warning, not a refusal: the box can be ticked in System Settings while the session runs and the follower re-asks on its next capture, still surfacing the System Settings text through `computer.native.error` / `computer.native.input.result`. This changes **who asks and when**, never what is allowed: `--allow-input` plus the sudo hop above remain the only gate on input. Dev caveat: TCC keys a grant to the code signature, so ad-hoc re-signed local builds re-prompt.
