...
| 6c    | Call-site migration, network domain (#2276 slice C): `scoops/tray-leader.ts`, `shell/proxied-fetch.ts` and `shell/mcp/redirect-uri.ts` stop probing the float themselves. Not rewired onto `broker.network.crossOriginFetch` — these are `SecureFetch`-shaped transport factories called from 18+ shell sites with no broker in scope, a different refactor than migrating a privileged operation. Instead each takes the answer by injection: `getChromeExtensionRealm()` (`base/api-endpoint.ts`'s lazily-cached, per-realm fact, mirroring the existing `extensionDelegateId` idiom) for the first two, an injected `topology` parameter for the third.                                                                                                                                                                                                                                                      | done (this PR)            |
...
**`approvals.request` is the native-gesture hop and nothing more.** It asks "put this in front of whoever decides on this float, and give me their answer". Everything around that hop is policy and stays in `sudo/`, above the broker: tray-first delegation to a follower's human, the 5-minute human-decision budget and its `reason: 'user-timeout'` deny (`withApprovalTimeout`), cone/scoop/agent routing, and pattern suggestion. Slice C's approvals PR WRAPS this capability inside `createSudoBroker`; it does not replace `createSudoBroker` with it. The adapters carry only a transport deadline (10s), because a relay that is not there at all — a side panel that never loaded leaves MV3's callback unfired forever — must not block a turn on a prompt no human will ever see. A broken relay is a `CapabilityFailure`, never a `deny`: an enforcement layer that reads a dead relay as a refusal tells the agent it was REFUSED and stops retrying, when nobody was ever asked.
...
**Slice C, network domain (6c): not every call site becomes a broker operation.** `scoops/tray-leader.ts` (`createTrayFetch`), `shell/proxied-fetch.ts` (`createProxiedFetch`) and `shell/mcp/redirect-uri.ts` (`resolveMcpRedirectUri`) all probed the float directly, but none of them is a privileged operation gated by an allowlist — they are `SecureFetch`-shaped transport factories called from 18+ shell sites with no `CapabilityBroker` in scope at any of them. Threading a broker instance through all of those call sites would be a different, much larger refactor than "stop probing." Instead each takes the SAME kind of answer the broker gives its callers, by injection rather than probing:

- `tray-leader.ts` and `proxied-fetch.ts` read `getChromeExtensionRealm()` — a lazily-cached, per-realm answer in `base/api-endpoint.ts`, resolved once on first read and reused, mirroring the module's existing `extensionDelegateId` / `localApiBaseUrl` idiom (each realm — page, kernel worker, real extension page — holds its own independent module instance, and `chrome.runtime.id` is stable for a realm's whole lifetime, so caching costs nothing). `base/api-endpoint.ts` is now the one place in the webapp that imports `isChromeExtensionRealm` from `@slicc/shared-ts`.
- `redirect-uri.ts`'s `resolveMcpRedirectUri` takes `topology: FloatTopology` as a required parameter; its two callers (`shell/mcp/provider.ts`, `shell/supplemental-commands/mcp-command.ts`) resolve `resolveFloatTopology()` at the point they actually need a redirect URI — the exact same timing as before, just one file over. This is a genuine relocation, not an elimination, and is fine here: the slice-D lint gate targets `scoops/` / `tools/` / `kernel/`, not `shell/`.
