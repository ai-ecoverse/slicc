...
| 6i    | Slice E, the LAST PR of #2276: moved the pure protocol modules `packages/chrome-extension/src` imported from `packages/webapp/src` into `@slicc/shared-ts` — `cdp/extension-bridge-protocol`, the `cdp/types` `TargetInfo` subset (`cdp-target-info.ts`), `shell/proxy-headers`, `net/discovery-link`, `net/well-known-probe`, `net/handoff-link`, the transitively-required `net/link-header` (not explicitly named in the plan but a necessary dependency of both discovery-link and handoff-link), and the `LEADER_EXT_ID_QUERY_NAME` constant — with webapp re-export shims at every original path so no webapp-internal caller's import changed. `ui/iframe-repaint.ts` (a DOM helper, not protocol code) was copied rather than moved: webapp's own `ui/sprinkle-renderer.ts` keeps using its copy, so a move would have created the exact wrong-direction dependency this slice closes. The 12 `kernel/messages.ts` types `service-worker.ts` needs stayed as `import type` — confirmed strictly type-only (no runtime use) — since that message-envelope union is core webapp-internal kernel infrastructure (11+ webapp files), not extension-specific, and `import type` carries no runtime/bundle coupling. New guard: `check-layer-back-edges.mjs`'s `findChromeExtensionWebappEscapes` / `scanChromeExtensionWebappEscapes` enforce zero `packages/chrome-extension/src` → `packages/webapp/src` imports except that one type-only path. Category 10's exit criterion is met.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Round-1 review (Grok + human) caught five issues, fixed on the same branch: the fingerprint NUL-separator escape in discovery-link.ts/handoff-link.ts had collapsed into a raw NUL byte (git rendered one file as binary) instead of the six-character `\u0000` escape — restored, with a test proving two identities that concatenate to the same string under a naive join still fingerprint differently; `iframe-repaint.ts` moved into `@slicc/shared-ts` outright rather than staying a copy (this package's tsconfig already has the `DOM` lib); the guard gained template-literal, `+`-concatenated, and TS triple-slash reference detection (`findChromeExtensionWebappEscapes` only saw quoted specifiers before); `LEADER_EXT_ID_QUERY_NAME` moved into its own one-export `leader-ext-id.ts` and the package gained `"sideEffects": false` (audited: no top-level side effects anywhere in `src/`) after two first-load page files importing just that constant pulled the whole barrel onto the eager graph — the page delta went from +3.9 kB to -15.9 kB vs merge-base; and node-server's coverage floor, which counts `@slicc/shared-ts` source through the workspace alias, gained `coverageExclude` entries in `coverage-thresholds.json` for the nine browser/extension-only modules this slice added (never by lowering the floors themselves). done |
...
- **The system Gelatiere is the one live-inheritance exception** ([#3265](https://github.com/ai-ecoverse/slicc/issues/3265)). It has no picker and always mirrors `modelFor(leadingRootOf(roster))`: the primary `cone` while present, otherwise the oldest surviving root. Creation and restore persist that model, a leading-cone model change or leadership change synchronizes it, and every Gelatiere prompt rechecks and re-resolves before the turn. A missing leading-cone model blocks the pass; it never falls through to global `selected-model`. This exception does not retarget ordinary scoops.
...
- `ScoopLifecycleManager` picks `VirtualFS` vs `RestrictedFS` from `policy.filesystem` (and `includeMounts` from the isolation `mode`), gates every privileged callback on the policy (`canCreateChildren`, `canManageChildren`, `canWriteSharedMemory`, `canResolveApprovals`, `approvalAuthority`), and records fatal errors as durable, model-inert error cards on the unit's direct parent (falling back to the default root when that edge is gone). The fatal path force-unmutes the child and releases `scoop_wait`, but never routes the notice through the parent's prompt queue, so nested ownership cannot amplify a provider outage. The owner notice is emitted with `endTurn: false` so a mid-turn / mid-`scoop_wait` parent does not have its processing state cleared.
...
- Name-based child resolution in the scoop-management tools (`feed_scoop`, `drop_scoop`, `scoop_mute`, `scoop_unmute`, `scoop_wait`, `list_scoops`, `scoop_scoop`'s duplicate check) runs against `subtreeOf(roster, caller.jid)` — the caller plus what it transitively owns. An unmatched name is an error naming what the caller can reach; it never widens to a global match, so cone A's `scoop_wait helper` cannot capture cone B's `helper` (#2360). Scoop _folders_ stay globally unique (suffixed on collision) because `/scoops/<folder>/` is one shared VFS path.
- **The leading cone is the one exception.** `leadingRootOf(roster)` (`work-unit/record.ts` — the primary root when registered, else the oldest, the same rule as the strip's `defaultRootOf` and the orchestrator's `defaultRoot`) additionally resolves two classes of scoop no subtree contains: `inherited` (its owning cone is gone — `rootOwnerOf` returns `undefined`) and `foreign` (owned by another live cone). `list_scoops` tags them `[INHERITED]` / `[FOREIGN: <cone>]`, and acting on one requires an explicit `cross_cone: true` argument — omitting it is an error naming the boundary, so the widening is never accidental (no sudo prompt is involved). Resolution tries an exact FOLDER first (folders are globally unique, so a local scoop whose display name equals a foreign scoop's folder cannot swallow the name and skip the gate), then an own-subtree display name, then any display name; `scoop_scoop`'s duplicate check stays subtree-scoped; cones are never targets; and a batch tool refuses the whole batch rather than acting on its `own` half. A CYCLIC chain is excluded rather than treated as inherited — `ownershipChainOf` separates `cycle` from `dangling` because `unregisterScoop`'s `childrenOf` cascade carries no visited set. `scoop_wait` therefore delivers its lick to the **requesting** unit (`scheduleScoopWait(jids, timeoutMs, requesterJid)`), not to each scoop's parent, which would file a cross-cone result in the owner's transcript. Bulk cross-subtree lifecycle work is still #2278's supervisor APIs (`createMany` / `join` / detach / promote).
