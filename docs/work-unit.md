...
- Licks a cone produces come back to it: every non-primary root's shell carries `SLICC_LICK_TARGET=<folder>` (`buildScoopShellEnv`), `fswatch` defaults `--scoop` to it (`defaultLickTarget`, `shell/lick-target-env.ts`; `crontask` follows in #2311), and background bash jobs stamp `targetScoop` the same way (`ownLickTargetFor`). The primary cone stays the untargeted default (`routeFormattedLickToCone` → `rootsOf(scoops)[0]`); addressing licks to a cone by name from elsewhere is #2311.
