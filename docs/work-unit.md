...
The unit keeps its folder (and therefore its chat session key). `workspaceFor` then treats it as an extra cone (`/cones/<folder>/workspace`); files under `/scoops/<folder>/` are not moved, the same non-migration extra cones got in #2271. Descriptors update immediately; a live agent's RestrictedFS stays until the next spawn.
