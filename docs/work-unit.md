...
- **The leading cone is the one exception.** `leadingRootOf(roster)` (`work-unit/record.ts` — the primary root when registered, else the oldest, the same rule as the strip's `defaultRootOf` and the orchestrator's `defaultRoot`) additionally resolves two classes of scoop no subtree contains: `inherited` (its owning cone is gone — `rootOwnerOf` returns `undefined`) and `foreign` (owned by another live cone). `list_scoops` tags them `[INHERITED]` / `[FOREIGN: <cone>]`, and acting on one requires an explicit `cross_cone: true` argument — omitting it is an error naming the boundary, so the widening is never accidental (no sudo prompt is involved). Own-subtree matches still win a display-name tie, `scoop_scoop`'s duplicate check stays subtree-scoped, cones are never targets, and a batch tool refuses the whole batch rather than acting on its `own` half. `scoop_wait` therefore delivers its lick to the **requesting** unit (`scheduleScoopWait(jids, timeoutMs, requesterJid)`), not to each scoop's parent, which would file a cross-cone result in the owner's transcript. Bulk cross-subtree lifecycle work is still #2278's supervisor APIs (`createMany` / `join` / detach / promote).
