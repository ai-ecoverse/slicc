...
| 5b    | Mount the WC shell on `WorkUnitClient` (#2382), PR A: `setModel` joins the protocol and one `createWorkUnitAgentHandle` replaces the leader's `OffscreenClient.createAgentHandle()` and the follower's sync-manager-as-`AgentHandle`, so the composer send, the stop and the model write name their unit on both sides. Transcript/selection (PR B), the model pill (PR C) and the single mount (PR D) follow. | done (this PR)            |
| 5–9   | Remaining `WorkUnitClient` mount slices, remaining CapabilityBroker adapters, snapshot COW / `shared-live`, deletion of legacy paths.                                                                                                                                                                                                                                                          | deferred; separate issues |
