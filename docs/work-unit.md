...
| `lick-target.ts`  | `resolveLickTarget` — a `targetScoop` alias → a record, most-specific first: exact `folder` (incl. `cone-<slug>`), then `<alias>-scoop`, then `name`                      |
...
- A cone is a lick target like any other unit: `resolveLickTarget` (`work-unit/lick-target.ts`) matches a `targetScoop` in three ordered passes — exact `folder` (`cone`, `cone-research`), then the conventional `<alias>-scoop` folder, then `name`. Folders are the unique key and names are user-typed, so a cone called `reviewer` next to a scoop in `reviewer-scoop` resolves deterministically instead of by registry order. The resolver is shared by `kernel/host.ts routeFormattedLickToCone` and `KernelFacade.routeSprinkleLick`. The two differ only in what an unmatched alias means: a lick is dropped (its scoop is gone; it must not surface in another chat), a sprinkle route falls through to the default root (the user raised it on a panel and must see it somewhere).
