...
| 6h    | Slice D, the lint gate (#2276): `check-no-float-probes.mjs` bans all eight float/topology probe names under `scoops/`, `tools/`, `kernel/` (except `kernel/host.ts`), mirroring `check-layer-back-edges.mjs`'s baseline-ratchet shape. The scan is import/re-export-CLAUSE based, not a whole-file identifier scan — every domain slices A–C migrated reuses the SAME name for a local composition-time-answer const/parameter/property, which a whole-file scan cannot tell apart from an actual probe import. A second pass folds re-exports discovered under a DIFFERENT name anywhere in `packages/webapp/src` into the banned-zone scan, so a rename-based bypass is caught at the banned-zone IMPORT site if one ever appears, without flagging `core/secret-topology.ts`'s pre-existing, unrelated `resolveFloatTopology as resolveSecretTopology` rename (consumed only outside the banned zone today). Baseline `float-probe-baseline.json` starts EMPTY — verified against a tree containing 6a–6g. Wired into `npm run lint` / `lint:ci` and the boy-scout debt gate.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | done           |
