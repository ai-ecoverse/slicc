...
- There is no cone list in the rail — the top tab strip (`<slicc-agent-tabs>`) is the only switcher: every cone first (oldest first), then the **selected** cone's scoops, then every other scoop (`orderForSwitcher(scoops, selectedJid)`; followers mirror it in `toFollowerSwitcherScoops(summaries, selectedJid)`). The rail only ever grows by one click-triggered line (the new-cone name form, the drop confirm), never on hover, so nothing shifts.
