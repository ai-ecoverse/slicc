...
The unit keeps its folder (and therefore its chat session key). `workspaceFor` then treats it as an extra cone (`/cones/<folder>/workspace`); files under `/scoops/<folder>/` are not moved, the same non-migration extra cones got in #2271.

After persist, a spawned unit is torn down and `createTab`'d so the live `ScoopContext` is a root: `this.unit` is re-derived, RestrictedFS is replaced with the shared `VirtualFS`, and the child-management tools / sudo wiring match `interactiveRootPolicy`. A unit that has never spawned is persist-only; the next spawn already sees the new edge.
