...
| `multiple-cones-follower.test.ts`   | leader + follower joined through a real `wrangler dev` tray hub: strip mirroring (`orderForSwitcher` vs `toFollowerSwitcherScoops`), a follower changing one cone's model while the leader sits in another (#2310). A read-only-scoop spec (#2312) sits beside them, `test.fixme`-gated on the `READ_ONLY_SCOOP_VIEW` constant and therefore not running, until that behaviour lands |
