...
| Root unit ↔ child unit           | Cone (`parentJid === null`)                                  | Scoop (`parentJid` = owner)                | Cone and scoop are roles over one `WorkUnit` (#1666). A new `scoop.isCone` branch in `scoops/` or `kernel/` re-creates the singleton-cone assumption; route on the unit's `policy.*`, `isRootUnit`, or `orchestrator.getWorkUnits()` (`lint:iscone-ratchet` fails on new reads outside `ui/`) |
