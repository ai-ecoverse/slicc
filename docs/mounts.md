...
`read` accepts a single `bytes=` range and answers `206` with `Content-Range: bytes <start>-<end>/<size>`; a well-formed range that lies outside the file is `416` with `Content-Range: bytes */<size>` and an `EINVAL` body, and a `Range` the server cannot parse is ignored (RFC 9110 §14.2) so the whole file is served. Bodies are streamed off disk rather than buffered, and both bridges advertise `Accept-Ranges: bytes`.
