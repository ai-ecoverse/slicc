...
The server serves each mapped folder over its local `/api/hostfs` bridge (loopback / bridge-token gated like every other `/api` route), and the webapp mounts them automatically once the shared filesystem exists — before scoop contexts are created — and retries them when persisted mounts are restored. No picker, no Chrome permission prompt, no user gesture. Properties:

- **Live view**: reads/writes go straight to the host filesystem, so external edits are usually visible immediately. `mount refresh` still walks the tree and reconciles the body cache — its summary reports how many files were checked (`unchanged`) so a populated idle mount is distinguishable from a no-op (#3435).
- **Real stats**: `/api/hostfs/stat` and each file row of `/api/hostfs/list` — through either transport, since the stable `POST /api/hostfs` dispatcher shares the same handlers — report the host's `ctime`, `dev`, `ino`, `uid`, `gid` and full `st_mode` alongside `kind`/`size`/`mtime`. `VirtualFS` carries them through as optional `Stats` fields, so a mounted path has a real inode and a real executable bit. Git needs all of them: isomorphic-git's `compareStats` calls a file stale unless every one matches its index entry, and a permanently stale comparison re-hashes the whole tree on every command (#2708). Timestamps are sent **unrounded** — isomorphic-git derives the seconds it compares with `Math.floor(ms / 1000)`, so rounding a `.9996 s` stat up would push it into the next second and leave that file stale on every walk. Remote backends (S3/DA/AEM) have nothing to report and keep the synthesized defaults.
...
- **Same-file rename is a no-op**: POSIX `rename(2)` requires that if old and new resolve to the same directory entry (or hard links to the same inode), the call succeed and do nothing. On a case- or Unicode-normalization-insensitive volume (APFS via hostfs) `Slicc.md` and `SLICC.md`, or an NFD name and its NFC form, are that pair. Both bridges compare `lstat` identity (`dev`+`ino`), not the path strings, before calling `rename`/`moveItem`. A copy-then-unlink rename that opens dest with `O_TRUNC` first zeros the only copy while `readdir` still shows the original name (#3107). Distinct names still rename. The in-VFS `/tmp` tree stays byte-exact and case-sensitive.
- **Config-owned**: the table is the single source of truth. Entries are not persisted in the browser; edit the table and relaunch to change them. `mount unmount` removes one for the session only. A live non-hostfs mount already at a table target is unmounted and replaced. A persisted picker/S3 row at the same target is not just skipped for the boot — it is **purged from the store**, and an armed `pendingMount:term:<target>` handle in `slicc-pending-mount` is cleared with it, so a later launch without the table entry can't silently fall back to a stale FS-Access handle and walk a tree you thought was config-owned (2026-08-24: an iCloud `~/Desktop/kb`). Picker mounts at other paths are left alone.
- **A failed table fetch is logged**: `GET /api/runtime-config` answering non-OK (a stale bridge token is 403), throwing, or exceeding 8 seconds logs the status and body and continues with no auto-mounts. The later restore pass retries. A response with no `autoMounts` field stays quiet — that float has no table. Pending-mount handles are cleared for the owned target and for any `pendingMount:term:` key whose path normalizes to it (`/mnt/foo/../kb` as well as `/mnt/kb`).
...
`read` and `write` deliberately keep their per-file URLs for streaming, range requests, validators, and raw PUT bodies; those repeating URLs also make their CORS preflights reusable. Body coherence is client-side via an app-owned `RemoteMountCache` (same IndexedDB store as S3/DA): every `GET /api/hostfs/read` uses `cache: 'no-store'` so the opaque browser HTTP cache is out of the path, and successful reads up to 4 MiB are memoized with TTL + ETag revalidation. Larger files are served without an IDB copy so packfiles do not consume the origin quota shared with the VFS. Configured target + host paths derive a stable cache namespace across boots. Own `write`/`rename`/`remove` update or drop cache keys immediately, and an invalidation generation prevents an in-flight older read from restoring a dropped key. External host edits are pushed as `hostfs_invalidate { mount, paths }` over `/licks-ws` by the launcher's recursive watcher (`hostfs-watch.ts` / `HostFSWatch.swift`); the kernel host drops all matching prefixes in one IDB scan. Recursive `remove`, directory rename, and a path-storm from the watcher invalidate complete subtrees. `mount refresh` re-lists the tree (skipping already-seen directory `dev:ino` identities so symlink cycles cannot explode), classifies each file against the body cache using both Node (`mtimeMs`) and Swift (`floor(mtimeMs*1000)` µs) ETag encodings, reports cache keys absent from the walk as `removed`, and treats cached bodies whose listing lacks a comparable validator as `changed`. `--bodies` re-fetches only the changed paths.
...
`list` stats every dirent server-side, so a listing already knows each file's `size`, `lastModified` and — since [#2708](https://github.com/ai-ecoverse/slicc/issues/2708) — its `ctime`, `dev`, `ino`, `uid`, `gid` and `mode`. Those fields ride all the way up: `HostFsMountBackend.readDir` → `MountDirEntry` → `VirtualFS.readDir` → `DirEntry` (all optional, all best-effort), and the panel-side `vfs-read-dir` RPC mirrors them.
...
- **The backend opts in.** `MountBackend.listingStatsMatchStat` means "when my listing reports size/mtime, those numbers equal what my `stat()` would return" — consumers use them _instead of_ a stat and cannot tell the difference. Presence itself is per-call (`ReadDirOptions.includeStats`): FSA listings omit the fields unless asked (#2733 / #2765), HTTP listings carry them for free. Only `hostfs` (both bridges stat each dirent inside `list`) and `local` (both paths read `FileSystemFileHandle.getFile()`) set the flag. **S3 and AEM deliberately do not**: both answer `stat` from the body cache when they have one, where `mtime` is `cachedAt` — when the body was cached, not the object's mtime — and AEM reports the _decoded_ size there while a Source Bus listing carries the stored (compressed) one. Aligning those two sources is a separate change; until then their listings stay `{name, type}` and consumers pay the stat rather than getting a different number. `VirtualFS.readDir` drops the fields for any backend that has not opted in.
...
| `EXDEV: cannot create a symlink across a mount boundary to '/mnt/…'`                          | `ln -s` to a mount target cannot create a real link — mount backends have no symlink inode, and a VFS link would follow into the empty LightningFS placeholder (#3311) | Use the mount path directly, or copy. Same-layer VFS symlinks (`ln -s` under `/tmp`) still work  |
...
- **Listings do not stat by default.** `readDir(path)` returns `{name, kind}` only. `size`/`lastModified` cost one `getFile()` per file entry, and the dominant object-store caller — isomorphic-git listing `objects/pack` — throws them away; `objects/pack` has 91 entries and `log --all` listed it ~25,000 times. Callers that genuinely need the metadata ask with `readDir(path, { includeStats: true })`. `VirtualFS.readDir` forwards that flag; the git adapter requests it for worktree/refs walks and keeps pack + loose fan-out dirs names-only (#2765); the shell adapter always requests it because `ls -l` / `du` size every entry immediately after listing. The files panel keeps listings names-only and stats only the ≤200 capped entries it renders, so a directory with thousands of files cannot force uncapped `getFile()` work. The HTTP backends get sizes for free in their listing response and always report them, so `MountDirEntry.size`/`lastModified` are optional per backend _and_ per call — a consumer must fall back to `stat()` when they are absent, and a backend must **omit** them rather than fill in zeros (a zeroed size/mtime is exactly what makes isomorphic-git's `compareStats` call every file stale and rewrite `.git/index`, #2708).
