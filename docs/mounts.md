...
`read` and `write` deliberately keep their per-file URLs: a `POST` response is not cacheable, and a read the browser can revalidate with a `304` is worth far more than its preflight — and those URLs repeat (the same packfile, the same `.git/index`), so the preflight cache does work for them. Write coherence is client-side via an app-owned `RemoteMountCache` (same IndexedDB store as S3/DA): every `GET /api/hostfs/read` uses `cache: 'no-store'` so the opaque browser HTTP cache is out of the path, and successful reads are memoized with TTL + ETag revalidation. Own `write`/`rename`/`remove` update or drop cache keys immediately. External host edits are pushed as `hostfs_invalidate { mount, paths }` over `/licks-ws` by the launcher's recursive watcher (`hostfs-watch.ts` / `HostFSWatch.swift`); the kernel host drops the matching RemoteMountCache keys. Recursive `remove` and a path-storm from the watcher clear the whole mount prefix. `mount refresh` clears the cache for that mount.
