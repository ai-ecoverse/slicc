...
- **A failed table fetch is logged**: `GET /api/runtime-config` answering non-OK (a stale bridge token is 403) or throwing logs the status and body and continues with no auto-mounts. A response with no `autoMounts` field stays quiet — that float has no table.
