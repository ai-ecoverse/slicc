...
Light and dark follow the device (`ThemePalette.resolve` in `ChatView`). A leader `theme.apply` still supplies its token palette when `theme.base` matches that appearance, and only the `--ctx` accent when it does not — a dark theme does not force the phone into dark mode. Sprinkle CSS on a mismatch paints those device surfaces (so the wrapper's dark `--s-*` and `background:#0F0F1A` do not stick) and keeps only `--ctx` (`sprinkleCSSOverrides(for:)`). The Ghostty terminal stays always-dark; see [Terminal](#terminal). Screenshot screens `theme-light` and `theme-forest` pin the simulator appearance so those captures still show the matching palette.
...
### The composer and nav-bar pills float

The composer and the nav-bar pills are Liquid Glass (`Views/GlassChrome.swift`, `floatingGlass`) over the rows, with no band behind them: the nav bar's background is hidden and the transcript uses a `.soft` scroll edge. The opaque band the composer used to have stopped at the safe area, so the canvas showed through beneath it as a light strip. `MessageListView`'s body must be a `ZStack`, **not a `Group`**. A `Group` hands the inset to each branch, so the composer was rebuilt (losing its state) when the first message turned the empty state into the list. Each button in the controls pill takes the full 36pt slot as its hit area (`sessionControlHitArea`): under the glass, iOS 26 left a button only its ~18pt glyph, and XCUITest's tap on the snowflake stopped opening the freezer (`FrozenSessionsUITests`) though a real touch still worked.

### One scroll view per unit

The transcript carries `.id(selectedScoopJid)`. When one scroll view was reused across units, it kept the previous unit's offset. After a longer thread, that offset lay past the end of a shorter one, so the shorter thread opened blank until a small scroll forced a layout pass. The composer sits outside the id, so a draft survives a switch. `ReadOnlyScoopUITests.testALongThreadIsStillOnScreenAfterSwitchingBackAndForth` is the gate: with `-uiTestTranscriptRepeat <n>`, `-uiTestUnitRoleFixture` gives the cone the long transcript and the scoop one sixth of it under its own ids. Without the id, the scoop opens blank on the first switch.
...
- **Staleness.** The leader streams only the unit it displays and the units a follower selected, so any other buffer goes stale when its unit works. `rosterChanged` un-syncs a non-selected unit when its `ScoopSummary.turns` counter changes. A rise is a finished turn, even one whose `working` frame the leader's 50ms coalescing swallowed. A fall is a leader reload, which also re-baselines. Leaders without `turns` fall back to the `working → not working` edge. `reset()` on `dataChannelOpened` refetches everything (buffers are kept — they still paint instantly).
...
The switcher pill (`Views/ScoopSwitcher.swift`) opens `Views/ThreadListView.swift`: a slide-over from the pill's edge (leading; trailing with `leftHandedDock`) over a scrim at compact width, a persistent, foldable 280pt sidebar at regular width. `ThreadListLayout.presentation` picks the shape from `ShellLayout.mode` + available width + whether a workbench is open. It never reads device idiom, `UIScreen`, or orientation, so a foldable's inner display and a narrow Split View land correctly. The sidebar falls back to the slide-over when it would leave the conversation under 400pt (320pt each beside a workbench). The list only exists with more than one unit. Picks go through `selectScoop(jid:)`. Swipe walks the same order. State lives in `ThreadListModel`, owned by `ChatView`, not `AppState`. Hooks: `-uiTestThreadListFixture YES|scoop` (two cones, nested scoops, every marker, two scripted turns for unread), `-uiTestThreadListOpen YES`, `-uiTestShellWidth <pt>` (narrow-pane stand-in, compact class), `-uiTestThreadSummaryDelay <ms>` (scripted stand-in for the on-device summarizer; the preview shows at once and the scripted line lands after the delay). Tests: `ThreadListTests`, `ThreadListUITests`.

### Thread decoration parity

Web = `slicc-agent-tabs` (`webcomponents/src/switcher/`) fed by `work-unit/client/presentation.ts`. Wire = `ScoopSummary` in `shared-ts/src/tray-sync-protocol.ts`. Swift = `ScoopSummary` in `swift-trayfollower/.../Models/SyncProtocol.swift`.

| Marker                                    | Web                                                                    | Wire field                   | Swift decode                     | iOS                                                                                                              |
| ----------------------------------------- | ---------------------------------------------------------------------- | ---------------------------- | -------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| Lifecycle (ring / eyes)                   | `slicc-agent-tabs.ts:118-125`, `presentation.ts:184`                   | `state` (:933)               | yes (:222)                       | row avatar eyes (phrase in VoiceOver, `ThreadListRow.activityPhrase`)                                            |
| Phase thinking / tool (pin shape)         | `slicc-agent-tabs.ts:116-120`                                          | `activity` (:949)            | yes (:226)                       | avatar expression (phrase in VoiceOver)                                                                          |
| Awaiting user                             | `wc-tray-scoops.ts:49`, `slicc-agent-tabs.ts:207`                      | `activity: awaiting`         | yes                              | avatar gaze (phrase in VoiceOver)                                                                                |
| Context fill (arc / pupils)               | `slicc-agent-tabs.ts:261`                                              | `fill` (:951)                | yes (:228)                       | pupils (`NN%` in VoiceOver)                                                                                      |
| Near limit ≥75 (amber)                    | `slicc-scoop-overflow.ts:186`                                          | `fill`                       | yes                              | pupils only; no amber text                                                                                       |
| Broken (red ring)                         | `slicc-agent-tabs.ts:121`                                              | `state`                      | yes                              | dead eyes ("broken" in VoiceOver)                                                                                |
| Unread dot (cones only, not selected)     | `slicc-agent-tabs.ts:133-135`, `unread.ts:39`, `presentation.ts:224`   | `turns` (:971)               | **no** (corpus pins `dropped`)   | count badge from `ThreadUnreadLedger`, running the web's `state`-transition fallback                             |
| Hue (cone gold, scoop palette)            | `wc-scoop-color.ts:16`                                                 | `name` + role                | yes                              | `avatarColor` (`SliccAgentAvatarGeometry.swift:290`)                                                             |
| Label (cone: assistantLabel, scoop: name) | `presentation.ts:211`                                                  | `assistantLabel`, `name`     | yes                              | `ThreadListRow.label` (pill keeps assistantLabel)                                                                |
| Kind cone / scoop                         | `presentation.ts:209`                                                  | `parentId` (:908), `isCone`  | yes (:215)                       | tile shape + hue + indent (VoiceOver says cone/scoop), `UnitRole`                                                |
| Ownership + order                         | `presentation.ts:137,163`                                              | `parentId`, `addedAt` (:920) | `parentId` yes; `addedAt` **no** | `ThreadListOrder`: nested per cone (web flattens roots first), roots in leader order until `addedAt` is mirrored |
| Read-only                                 | composer only (`wc-follower.ts:716`), no tab mark                      | `parentId`                   | yes                              | no glyph (VoiceOver says read-only, `UnitRole.isReadOnly`)                                                       |
| Leader-active unit                        | none (web follower selects it, `wc-tray.ts:618`)                       | `scoops.list.activeScoopJid` | yes                              | radio-waves glyph                                                                                                |
| Per-unit model                            | model pill for the selected cone only (`presentation.ts:68`)           | `model` (:983)               | yes (:233)                       | VoiceOver only                                                                                                   |
| Trigger                                   | not rendered                                                           | `trigger` (:921)             | yes (:217)                       | not rendered (parity)                                                                                            |
| Attention glow ("spoke most recently")    | `slicc-agent-tabs.ts:113`, set by `wc-live-callbacks.ts:213`           | leader-local, not on wire    | —                                | none, leader-only                                                                                                |
| `ephemeral` class                         | `slicc-agent-tabs.ts:669`                                              | never set by any producer    | —                                | none                                                                                                             |
| Biscotto / guest marks                    | none: a biscotto seat is never sent `scoops.list` (`broadcast.ts:291`) | —                            | —                                | none                                                                                                             |

**Row text is the label plus one line about the thread.** Kind, fill, phase, read-only and model are not spelled out on screen: the avatar already shows them, and the row's VoiceOver label keeps all of it. The second line comes from `Models/ThreadSummary.swift`. `ThreadSummaryStore` shows a plain preview (the first line of the newest message) at once. Where Apple's on-device model is available (`FoundationModels`, `SystemLanguageModel.default`), that preview is replaced with an at-most-eight-word summary of the last six turns. Nothing leaves the device. Summaries are made one at a time, only while the list is on screen, and each is keyed to the transcript's tail, so a thread that has not changed is never summarized again. A streaming tail is skipped. The second line is a fixed-height slot reserved before any text exists, and the text column takes whatever width the markers leave. A summary arriving, or the model's line replacing the preview, does not change a row's frame. The shell holds the store in `ThreadSummaryHost`, built once and never publishing, so the conversation beside the list does not redraw either. Tests: `ThreadSummaryTests`, `ThreadListUITests.testAModelSummaryReplacingThePreviewDoesNotMoveTheListOrTheTranscript`.

**Follow-ups (need a corpus flip, not a TS wire change):** mirror `turns` and `addedAt` into Swift. The leader already sends both, but `tray-sync-protocol-corpus.ts` marks them `dropped` for iOS and `SyncProtocolCorpusTests` enforces that. Flipping them to `mirrored` means (a) editing `packages/webapp/src/scoops/tray-sync-protocol-corpus.ts` and regenerating the fixture, (b) adding optional `turns: Double?` / `addedAt: String?` to `ScoopSummary`, and (c) passing them to `ThreadUnreadLedger.Unit.init` and `ThreadListOrder.entries(addedAt:)`, which already take them. Until then an off-screen turn that starts and ends inside one 50ms coalescing window is not counted as unread, and cones registered out of order keep leader order.
