...
Light and dark follow the device (`ThemePalette.resolve` in `ChatView`). A leader `theme.apply` still supplies its token palette when `theme.base` matches that appearance, and only the `--ctx` accent when it does not — a dark theme does not force the phone into dark mode. Sprinkle CSS is filtered the same way (`sprinkleCSSOverrides(for:)`). The Ghostty terminal stays always-dark; see [Terminal](#terminal). Screenshot screens `theme-light` and `theme-forest` pin the simulator appearance so those captures still show the matching palette.
