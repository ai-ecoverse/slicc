...
- **Preferred (OAuth / file-based auth):** Log in via Settings → Providers → GitHub (or `oauth-token github`). That writes a masked token to `/workspace/.git/github-token`. Then `git clone https://github.com/owner/repo` — before each network op, git asks the OAuth broker for a fresh token and re-syncs that bridge file, so isomorphic-git never keeps a hand-written snapshot. If the access token is still held but the replica is missing, the bridge remasks (same dual-mode write as `oauth-token`) and writes the new replica instead of deleting the file or leaving a stale snapshot. Env fallbacks (`GH_TOKEN` / `GITHUB_TOKEN`) still apply when no OAuth account is logged in.
...
The OAuth login popup flow is the user-approval gate. Once a token is cached, subsequent `oauth-token <provider>` calls return the masked replica immediately (no additional approval required, but the masked value is benign — the agent never sees the real token). If the access token is still held but the replica is missing (the secrets store was cleared while `slicc_accounts` survived), retrieval remasks and prints the new replica instead of declaring the token unusable. `--force-login` also prints the replica, never `accessToken`.
