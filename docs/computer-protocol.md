...
| `url`    | `computers/adapters/url.ts`                                           | HTTP remote. Probe `GET /computer` → `ComputerDescriptor`. `GET /computer/screenshot`, optional `GET /computer/text` (404 → null), `POST /computer/input`. Optional native `WS /computer/frames` only when `capabilities.frames === 'push'`; otherwise the registry polls. All HTTP goes through injected fetch (`createProxiedFetch` in production — lazy from `run.ts`, never a static adapter import). `computer add url http://127.0.0.1:5710` (trailing `/computer` stripped). Reference server: node-server `--computer-demo` (`src/computer-demo.ts`). |
