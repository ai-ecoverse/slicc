...
**REQUIRED for features with user-observable UI behavior** (e.g., the `agent-command` help text, terminal-panel integration, bridge-hook presence, live smoke tests, scoop-panel visibility, any change that affects what the user SEES in the UI — even indirectly, such as orchestrator callbacks that drive `ScoopsPanel.refreshScoops()` or similar UI refresh paths). OPTIONAL for pure utility wrappers whose `fulfills` assertions are ALL `Tool: vitest` AND which have no code path that changes UI state — in that case, full vitest + typecheck + both build gates are sufficient signal; document the decision in `whatWasLeftUndone` so it's auditable. **IMPORTANT**: if the feature description mentions UI, panel, scoop visibility, side panel, status display, or similar user-observable surfaces, manual verification is REQUIRED regardless of the `Tool:` classification — unit tests alone can miss UI-refresh-timing and callback-ordering bugs (e.g., a `ready` status callback firing before the scoop is actually removed from the tabs map leaves a ghost row in the panel). If you skip manual verification when the feature has user-observable scope, set `followedProcedure: false` in the handoff so scrutiny can audit.
