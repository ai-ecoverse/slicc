...
- `spawn({ cwd, allowedCommands, prompt, modelId?, parentJid? })`:
  ...
  8. Determines output via `pickChronologicallyLastOutput(messages, captured)`: walks `ctx.getAgentMessages()` in reverse, walking each assistant message's `content` blocks in reverse, and returns the first qualifying block — either a non-empty assistant-text block or a `send_message` toolCall whose corresponding `captured[]` entry is returned. Falls back to `captured[last]` if the walk finds nothing but `captured[]` is non-empty (preserves legacy mock-driven callers), and to `''` otherwise. This implements VAL-OUTPUT-016: "whichever came LAST chronologically wins." The companion system-prompt update at `packages/webapp/src/scoops/scoop-context.ts` (the "When using send_message" section of `buildSystemPrompt`) tells agents "your last message wins" so their emit patterns align with this semantic — a `send_message("Starting.")` progress update followed by a plain-text final answer now correctly surfaces the plain-text answer as `agent` stdout. When the winning block is text, the walker additionally collects every consecutive text block that precedes it within the SAME assistant message, bounded by the nearest preceding `toolCall` block (which acts as a chronological checkpoint). Text blocks that appear BEFORE the last `toolCall` within the same message are intermediate narrative and are intentionally dropped. Collected text blocks are concatenated literally (`.join('')`) — matching the canonical assistant-text assembly in `ScoopContext.handleAgentEvent` (`message_end` case) and `extractLastAssistantText()` — so split-token output like `[text('Hello'), text(' world')]` is preserved verbatim as `'Hello world'` with no fabricated whitespace.
...
Per-event persistence is **necessary but not sufficient** for the running-scoop selection path. After `ChatPanel.switchToContext()` hydrates a persisted in-progress assistant message, the selected panel's live relay (`OffscreenClient.currentMessageId`) must also reconnect to that hydrated message ID. Otherwise post-click `text_delta` can fork a second assistant message, while `tool_end` / `response_done` can be dropped because the live relay has no current message to attach them to. The reconnection step must use the **same SessionStore snapshot** that `switchToContext()` renders (or pass the hydrated in-progress message ID through the selection flow): a separate pre-hydration `SessionStore.load()` can race with fire-and-forget `OffscreenBridge.persistScoop()` writes and leave `currentMessageId` behind the panel's hydrated state. Any future VAL-SPAWN-016 fix must preserve all three invariants: persisted pre-click state, snapshot-consistent selection hydration, **and** message-ID continuity across the selection boundary.

Concrete continuity rule (misc-hydration round 2): the hydrated in-progress assistant row must remain the row that receives post-click deltas. A synthesized replacement row after selection is still a failure, even if the panel shows only one assistant row, because it can discard the bridge's already-buffered pre-click text and later persist that truncated panel buffer back over the fuller offscreen session state.
