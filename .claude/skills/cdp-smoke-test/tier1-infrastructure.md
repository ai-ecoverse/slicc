...
| Theme switch    | avatar `Account` → `Theme settings…` → `slicc-cdp click "Midnight Scoop"` → assert → `slicc-cdp click "Default"` → `Done`                   | `getComputedStyle(document.body).backgroundColor` goes `rgba(0, 0, 0, 0)` → `rgb(13, 17, 23)` and back; `localStorage['slicc-active-theme']` = `midnight-scoop` while active                    |
