...
- **Float-probe ratchet** (`npm run lint:no-float-probes`, #2276 slice D): `tools/check-no-float-probes.mjs` bans the eight float/topology probe names under `scoops/`, `tools/`, `kernel/` (except `kernel/host.ts`), folding re-exports-under-a-new-name into the same scan; baseline `float-probe-baseline.json` (`--update`), starts empty. [details](../../docs/dev-tools-details.md#float-probe-ratchet).
