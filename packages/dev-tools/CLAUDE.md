...
- **Regression cluster hunter** (release-triggered, `regression-cluster-hunter/` + `regression-cluster-hunter.yml`): when a release lands, sweeps for surviving siblings of one shipped bug fix and files them. Complements the nightly debt rotation (static taxonomies, one issue/run); this one starts from a real fix. [details](../../docs/dev-tools-details.md#regression-cluster-hunter).
...
- **Agent-skill install pin reconcile**: `packages/dev-tools/skill-pin-reconcile/`. Dual-pinned npm packages with an `ipk add` line in a vfs-root skill (today: v86); `renovate-skill-pin-reconcile.yml` raises a leftover stale skill line on `skill-pin` PRs. CI backstop is the live canary, not a lint gate.
...
- **Layer back-edge ratchet** (`npm run lint:layer-back-edges`): `tools/check-layer-back-edges.mjs`; baseline `layer-back-edge-baseline.json` (`--update`). Also a zero-tolerance check that no relative import escapes `packages/webapp/src` into a sibling package (#2798). [details](../../docs/dev-tools-details.md#layer-back-edge-ratchet).
- **Float-probe ratchet** (`npm run lint:no-float-probes`, #2276 slice D): `tools/check-no-float-probes.mjs` bans the ten `FLOAT_PROBE_NAMES` plus the raw `__slicc_connect_mode` global-bag key under `scoops/`, `tools/`, `kernel/` (except `kernel/host.ts` / `kernel/kernel-worker.ts` / `kernel/port-bridge-client.ts`) — a module-path ban (any import form) for probe-only modules plus a named-clause scan elsewhere, folding re-exports/wrapper-functions discovered under a new name into the same scan; baseline `float-probe-baseline.json` (`--update`, refuses to grow without `--allow-growth`), starts empty. [details](../../docs/dev-tools-details.md#float-probe-ratchet).
...
- **Debt boy-scout gate**: `node packages/dev-tools/tools/check-touched-exemptions.mjs [base-ref]` (+ `size-exemption-lib.mjs`) — enforces `biome.json` debt overrides + the three `*-baseline.json` ratchets (layer-back-edges, float-probes, record-string-unknown). See [verifying](../../.agents/skills/verifying-before-push/SKILL.md).
