...
- **Function-size + cognitive-complexity debt gate**: `packages/dev-tools/tools/check-touched-exemptions.mjs` (+ pure lib `size-exemption-lib.mjs`) — boy-scout gate enforcing both complexity debt blocks in `biome.json`. Exits non-zero if any PR-changed file is on either debt list, and blocks growing either list. Auto-skips on non-PR events. Run: `node packages/dev-tools/tools/check-touched-exemptions.mjs [base-ref]`. See the [verifying-before-push skill](../../.agents/skills/verifying-before-push/SKILL.md) for details.
