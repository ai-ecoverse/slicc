...
- **Agent merch grid**: `node packages/dev-tools/tools/agent-merch.mjs --seed <str> --grid 7 --cell 428` — hash-stable sparse grid of `<slicc-agent-avatar>` faces (names from `agent-names.ts`, ice-cream-canon colours, 3 featured in colour) with a tagline, rendered via Playwright to transparent light/dark PNGs in `dist/merch/`. Build `@slicc/webcomponents` first. Knobs: `--color`, `--sparse`, `--theme`, `--tagline`, `--out`.
