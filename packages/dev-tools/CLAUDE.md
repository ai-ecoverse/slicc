...
- **Debt boy-scout gate**: `packages/dev-tools/tools/check-touched-exemptions.mjs` (+ pure lib `size-exemption-lib.mjs`) — enforces every single-rule debt override in `biome.json` (complexity and promise safety), inline generated layer-boundary suppressions, and the legacy `ui-back-edge-baseline.json` list for uncovered directories. It fails when a PR touches a listed file or grows a list. Auto-skips on non-PR events. Run: `node packages/dev-tools/tools/check-touched-exemptions.mjs [base-ref]`. See the [verifying-before-push skill](../../.agents/skills/verifying-before-push/SKILL.md) for details.
