...
- **Agent merch grid**: `tools/agent-merch.mjs` (+ `agent-merch-lib.mjs`) — hash-stable sparse grid of `<slicc-agent-avatar>` faces + tagline, rendered via Playwright to transparent 300-dpi light/dark PNGs in `dist/merch/`. Build `@slicc/webcomponents` first. Knobs: `--seed`, `--grid`, `--rows`, `--cell`, `--scale`, `--color`, `--sparse`, `--theme`, `--tagline`, `--out`. `--rows` < `--grid` widens the composition (a square grid is locked at 1:1.595; DTG wants 1:1.333).
