...
- **Debt boy-scout gate**: `packages/dev-tools/tools/check-touched-exemptions.mjs` (+ pure lib `size-exemption-lib.mjs`) — enforces all three debt lists: both complexity blocks in `biome.json` and the `ui-back-edge-baseline.json` back-edge list. Exits non-zero if any PR-changed file is on any debt list, and blocks growing any list. Auto-skips on non-PR events. Run: `node packages/dev-tools/tools/check-touched-exemptions.mjs [base-ref]`. See the [verifying-before-push skill](../../.agents/skills/verifying-before-push/SKILL.md) for details.
