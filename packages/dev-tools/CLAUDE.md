...
- **Agent merch grid**: `node packages/dev-tools/tools/agent-merch.mjs --seed <str> --grid 7 --cell 428` — hash-stable sparse grid of `<slicc-agent-avatar>` faces (names from `agent-names.ts`, ice-cream-canon colours, 3 featured in colour) with a tagline, rendered via Playwright to print-ready transparent light/dark PNGs in `dist/merch/`. Output alpha is binary (fully transparent or fully opaque), files carry 300 DPI metadata, and `--scale 2` is the high-resolution default (5,992 px wide for the documented command). Build `@slicc/webcomponents` first. Knobs: `--color`, `--scale`, `--sparse`, `--theme`, `--tagline`, `--out`.
...
- **First-load size ratchet** (part of `npm run size -w @slicc/webapp`): `packages/dev-tools/tools/check-first-load-size.mjs` (+ `first-load-size-lib.mjs`) — measures the eager import closures of the page entry (via `.vite/manifest.json`) and the kernel-worker entry (parsed from emitted chunks) against `packages/webapp/first-load-budget.json`. Guards cold-boot payload, not per-file size.
