...
- **SPM ↔ xcodegen pin reconcile** (`lint:swift-pins`): `swift-pin-reconcile/` — dual GitHub pins (Package.swift + project.yml) must overlap; `renovate-swift-pin-reconcile.yml` bumps the stale side.
- **Agent-skill install pin reconcile**: `skill-pin-reconcile/` — dual npm pins with an `ipk add` line in a vfs-root skill; `renovate-skill-pin-reconcile.yml` bumps the stale line; live canary is the backstop.
...
- **iOS test-isolation gate** (`npm run lint:ios-test-isolation`): `tools/check-ios-test-isolation.mjs` (+ `-lib.mjs`) — the serial ios-app bundles must keep `randomExecutionOrder` and write no `UserDefaults.standard`; pairs with `tools/ios-sim-prepare.sh` (shared boot + container reset for both test legs). [details](../../docs/dev-tools-details.md#ios-test-isolation-gate).
...
- **Baseline ratchets** (each `tools/check-*.mjs` + a `*-baseline.json`, `--update`):
  - **Layer back-edges** (`lint:layer-back-edges`): `check-layer-back-edges.mjs`. Per-package stacks (webapp, node-server, chrome-extension, cloudflare-worker) plus zero-tolerance: no relative import may escape `packages/webapp/src` into a sibling. Webapp also flags `scoops/` value-importing `kernel/` (#3231). [details](../../docs/dev-tools-details.md#layer-back-edge-ratchet).
  - **Float probes** (`lint:no-float-probes`): `check-no-float-probes.mjs` (`--allow-growth`) — bans the ten `FLOAT_PROBE_NAMES` + raw `__slicc_connect_mode` under `scoops/`/`tools/`/`kernel/`. [details](../../docs/dev-tools-details.md#float-probe-ratchet).
...
- **Swift forbidden-import gate** (`npm run lint:swift-forbidden-imports`): `tools/check-swift-forbidden-imports.mjs` (+ `-lib.mjs`) — widget hosts / `SliccWidgetKit` must not import WebRTC (or tray-follower modules that pull it in); `public` is the SPM cross-module surface. [details](../../docs/dev-tools-details.md#swift-forbidden-import-gate).
- **Dead feature-flag gate** (`npm run lint:dead-flags`): `tools/check-dead-flags.mjs` — registry vs `isFeatureEnabled` / `getFeatureValue` / Cherry host / worker overlay keys; waiver `// unused-flag-ok`. [lifecycle](../../docs/feature-flags.md), [details](../../docs/dev-tools-details.md#dead-feature-flag-gate).
- **iOS PR screenshots**: `tools/ios-screenshots.mjs` (+ `-lib.mjs`) reads `screenshot-screens.json`, emits Storybook `manifest.json`.
- **Agent merch grid**: `tools/agent-merch.mjs` (+ `agent-merch-lib.mjs`) — hash-stable sparse grid of `<slicc-agent-avatar>` faces + tagline, rendered via Playwright to transparent 300-dpi light/dark PNGs in `dist/merch/`. Build `@slicc/webcomponents` first; run `--help` for knobs (`--seed`/`--grid`/`--rows`/…). `--rows` < `--grid` widens (square grid locked at 1:1.595; DTG wants 1:1.333).
...
- **CI phase timings** (`tools/ci-job-timing.mjs` + `-lib.mjs`): reads the active Actions job's step timestamps, writes a Markdown run summary + JSON artifact; used by both Cloudflare staging workflows.
...
- **Release gating**: `tools/release-plan.mjs` (Linux preflight) + `release-native.mjs` — gate macOS/iOS packaging, the `slicc` Go CLI (`packages/slicc-cli/sign-and-package.sh`), Chrome Web Store / worker publish, and `@ai-ecoverse/biome-jsh`. `tools/merge-queue-busy.mjs` (+ `merge-queue-lib.mjs`) defers Release while the default-branch merge queue has entries so the queue never waits on Release. `tools/release-publish.mjs` wraps `npx semantic-release` and exits 0 when the version-commit push is rejected because `main` moved during prepare (`fetch first`); the newer push or the schedule catch-up publishes.
...
`CDPClient` → `/cdp`) against live headless Chrome via a stand-in `/cdp` proxy
reproducing node-server + swift-server policies. Scenarios run via `npx tsx`;
`run-all.ts` writes JSON to `dist/cdp-stress/`. Pass criteria asserted by the
opt-in suite `packages/webapp/tests/cdp/cdp-stress.gate.test.ts`
(`SLICC_TEST_CDP_STRESS=1`). [README](cdp-stress/README.md).
...
Five isolated harnesses (`tools/dev-{standalone,swift,extension,electron-node,electron-swift}-fresh.sh`) on distinct ports; reaping opt-in (`SLICC_FRESH_REAP=1`), port-scoped. Never touch prod bridge `:5710` or CDP `:9222`. Run via `npm run dev:standalone:fresh` (also `dev:swift:fresh`, `dev:extension:fresh`, `dev:electron:{node,swift}:fresh`); override with `PORT`/`WRANGLER_PORT`/`ELECTRON_APP`. Port table, Darwin helpers, and reaping rules: [dev](../../docs/development.md), [details](../../docs/dev-tools-details.md#fresh-dev-harnesses).
