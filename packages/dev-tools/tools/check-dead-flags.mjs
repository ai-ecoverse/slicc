#!/usr/bin/env node
// Dead / undeclared feature-flag gate.
//
// The typed registry in packages/webapp/src/core/feature-flags.ts is the source
// of truth. This gate fails when a declared id has no consumer, and when a
// consumer (isFeatureEnabled / getFeatureValue / Cherry host key / worker
// overlay or fallback key) names an id the registry does not declare. The
// worker and Cherry talk in strings, so the FeatureFlagId union cannot catch
// that drift by itself.
//
// A parked flag is annotated on its registry entry with
// `// unused-flag-ok: <reason>` (or `// unused-dep-ok:`, same shape as the
// Swift unused-dependency gate). `since` is required; a flag whose bundled
// default has been constant across every float for STALE_DAYS is reported as
// a warning (the "this is now just code" signal) but does not fail the lint.
...
/** Index of the matching closer for the bracket at `openIdx`, or -1. */
...
/**
 * Blank `//` and `/* *\/` comments, preserving newlines. Strings are copied
 * through so a URL like `chrome-extension://…` is not treated as a comment.
 */
...
/** `FeatureFlagId` union members, in source order. */
...
/** `FeatureFlagFloat` union members, in source order. */
...
/** Registry entries from `const FEATURE_FLAGS = Object.freeze([…])`. */
...
/** Keys of `FALLBACK_BASE_FLAGS` in the worker flags module. */
...
/** Strip JSONC comments without treating `chrome-extension://` as a line comment. */
...
/** Flag keys from wrangler `vars.FEATURE_FLAGS` (top-level and `env.staging`). */
...
/**
 * Compare declared ids against consumers.
 *
 * Worker overlay / fallback keys are a string-boundary declaration: they must
 * name a registry id (undeclared-flag otherwise) but they do not keep a flag
 * alive. A flag is live only when `isFeatureEnabled` / `getFeatureValue` or a
 * Cherry host key reads it.
 */
...
/** Scan a repo root. Exported for tests (`--root`). */
