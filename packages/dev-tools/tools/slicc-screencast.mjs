 ...
 * frames directly, or pass `--video` to assemble an mp4 (best-effort; needs
 ...
 *   --video              Assemble frames into <out>/screencast.mp4 (best-effort)
...
async function main() {
  ...
  let seq = 0;
  const frames = [];
  conn.on('Page.screencastFrame', async (p) => {
    seq += 1;
