...
/**
 * Lay out an SPM package whose `.build/` mimics one build system: `binRel` is
 * the bin path `swift build --show-bin-path` reports, `bundles` the `.xctest`
 * bundles inside it. The stub `swift` answers both path queries and the stub
 * `xcrun` rejects empty arguments, then prints a passing TOTAL row.
 */
