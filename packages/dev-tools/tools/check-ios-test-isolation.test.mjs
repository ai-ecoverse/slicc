// Tests for the ios-app test-isolation gate (check-ios-test-isolation.mjs).
// The gate exists because ios-app's bundles are pinned serial, so random order
// plus per-test state is all the isolation they have — and both are a one-line
...
describe('parseSchemeTestTargets', () => {
  ...
  it('is not fooled by a build target that shares the scheme name', () => {
    // `SliccFollower` is both an application target and the scheme, and the
    // build action has its own `targets:` map.
