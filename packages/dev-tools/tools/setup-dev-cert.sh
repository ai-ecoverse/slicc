...
echo "  2. Grant the server access to its secrets blob ONCE. Either run the"
echo "     server in a terminal and click \"Always Allow\", or run the"
echo "     non-interactive grant (ad-hoc binaries live in the unsigned: partition):"
echo ""
echo "       security set-generic-password-partition-list \\"
echo "         -S apple-tool:,apple:,unsigned: \\"
echo "         -s $KC_SERVICE -a $KC_ACCOUNT \\"
echo "         -k \"<your-login-keychain-password>\""
echo ""
echo "  Because the DR is now stable, that grant persists across rebuilds."
