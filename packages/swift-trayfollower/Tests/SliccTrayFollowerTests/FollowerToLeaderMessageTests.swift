...
final class FollowerToLeaderMessageTests: XCTestCase {
    ...
    /// The `--computer` pairing token survives the wire, and stays absent when
    /// unset — a `pairId: ""` or a dropped field would either fold nothing or
    /// (worse) group every follower that omitted it (#3260).
