...
`SliccTraySession` is a Foundation-only SPM library for cross-device discovery of active tray join URLs over iCloud key-value sync. It is shared by the macOS launcher (`packages/swift-launcher`, the producer and first consumer) and the iOS follower (`packages/ios-app`, which uses it for iCloud session discovery). Supports `.macOS(.v14)` and `.iOS(.v17)`; **nothing here may import AppKit or UIKit** — CI builds the package for the iOS Simulator to enforce that.
