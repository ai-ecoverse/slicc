#!/usr/bin/env node
/**
 * Stand-in for the Go `slicc` follower CLI, driven by the same argv the
 * scripts use (`<join-url> prompt|exec|follow …`). Keeps a fake VFS under
 * `$FAKE_SLICC_DIR/vfs` and understands exactly the command shapes
 * `lib.mjs` builds, plus a few probes tests use to force failures:
 *
 *   exec `fail <n>`            exit n with "boom" on stderr
 *   exec `dial-fail …`         behave like a WebRTC dial timeout while
 *                              $FAKE_SLICC_DIR/dial-failures > 0 (decremented)
 *   exec `sleep <s>`           block (timeout tests)
 *   exec `cat-stdin`           echo piped stdin
 *   prompt containing SLOW     block ~30 s (timeout tests)
 *   prompt containing DIALFAIL same counter as dial-fail
 *   follow                     print `slicc follow: connected` then idle;
 *                              FAKE_SLICC_FOLLOW=exit → exit 3 at once,
 *                              FAKE_SLICC_FOLLOW=silent → idle without the line
 */
...
// A single-quoted shell word as `shellQuote` emits it ('…' with '\'' for quotes).
const Q = String.raw`'(?:[^']|'\\'')*'`;
