...
```bash
npx vitest run --project github-workflow      # unit tests (lib + gh-io)
npm run lint                                  # biome (.mjs) + prettier (yml/md)
```

The live gate is `.github/workflows/github-workflow-smoke.yml`: it boots a real leader from the published `sliccy` package against production sliccy.ai and exercises every provider-free action, then runs `slicc-leader.yml` at the PR's ref. It runs on PRs touching this package or the `slicc-*.yml` workflows. There is no way to smoke-test a hosted leader without a tray hub, so the gate needs network egress.
...
- **Pure vs I/O split.** Anything decidable without a runner lives in `lib.mjs` with a unit test. Scripts do I/O and call into it. New logic follows the same split.
- **The join URL is a capability.** `start-leader` masks it by default (`::add-mask::`); every CLI action re-masks the value it receives. Masked values are dropped from _job_ outputs by GitHub, so cross-job use needs `mask-join-url: false` — documented, never the default.
