...
- `POST /api/tray/:trayId/preview` — mint a preview token (Bearer = controllerToken); response `{ previewToken, url }`. The URL is a `<token>.sliccy.now` (prod) / `<token>.sliccy.dev` (staging) host that routes back to this worker via the wildcard route.
