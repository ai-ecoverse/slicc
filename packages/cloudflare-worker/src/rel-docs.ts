...
const RELS: Record<string, RelInfo> = {
  ...
  'successor-version': {
    ...
    summary:
      'Emitted by the tray hub when a tray has been superseded — the leader reconnected, minted a fresh tray, and abandoned this one. The link target is the replacement tray\'s join URL: followers should re-attach there instead of retrying this tray. Unlike <code>handoff</code> and <code>upskill</code>, this is not a SLICC extension rel — <code>successor-version</code> is registered by RFC 5829 §3.4 and appears as the bare token; this page documents how SLICC uses it. The response is an HTTP <code>308</code> whose <code>Location</code> names the same target, so a client that lets its platform follow redirects re-attaches without reading anything; the link is the channel for clients that suppress redirect-following, since it survives a body-shape change and, unlike <code>Location</code>, carries no <code>json=true</code> and is therefore what a follower should persist. The JSON body (<code>code: "TRAY_SUPERSEDED"</code>, <code>joinUrl</code>) is kept as well. A replacement URL the hub cannot parse still answers <code>409</code> with no link at all — a redirect needs a target.',
