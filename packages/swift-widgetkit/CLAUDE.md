...
- `Model/UnitRecencyLedger.swift` — per-unit recency, which the wire does not carry. Stamps a unit when its lifecycle, activity or whole-percent fill moves; a rename or a model swap is not activity. Both capture sides own one.
...
- **Three families, three layouts — never one layout at three scales.** small = four peers in a 2x2 (_who needs me?_); medium = one focus plus a field (_what is the one thing?_); large = the cone-grouped tree (_how is this organised?_). Only large shows ownership, and only large has the height to. `UnitRanking` decides who earns a cell: **cones first, always; then scoops; each group by recency** — never urgency, which floated a broken scoop above the cone that owns it. Breaking IS a change, so it still rises inside its own group. Fewer units get BIGGER faces rather than a small avatar marooned in the corner.
