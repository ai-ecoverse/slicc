...
allowedCommands:
  ...
  - printf
  - rg # recursive, .gitignore-aware search; what the curator actually uses over find|xargs grep. Exit 1 = no match (`-c` prints nothing, not `0`); exit 2 = searchable-byte limit (stderr names the budget). Later commands still run.
  - sed
  - sort
  - tail
  - tr
