...
A webhook POST also tells you the truth about the delivery: `202` means a unit was woken (or your `--filter` dropped the event on purpose), `404 WEBHOOK_NOT_REGISTERED` means the id is unknown, and `422 WEBHOOK_TARGET_UNRESOLVED` means the target is gone and the event was discarded.
