...
- **`serve` limits — plan for them:** every file must be ≤ 25 MiB (live previews answer 413 above that; split larger media). A tray holds at most **10 previews**, and live previews plus `--ttl` snapshots share that pool. Previews do **not** end with your session: a live preview lasts until `serve --stop`, a snapshot until its TTL. Each `serve` prints `Preview token: …`; `serve --stop <token-or-url>` when you are done, and on `Preview limit reached` run `serve --list` and stop previews you no longer need.
