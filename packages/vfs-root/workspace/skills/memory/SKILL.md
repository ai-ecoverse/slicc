...
- `memory dream` reads no session archive — the dreamer consolidates the memory file itself under the same `/etc/MEMORY.md` instructions as curation (its own `dreamTimeoutSeconds` bound), through the same staged draft and merge. Detached by default; the outcome lands in `/sessions/.curation/dream-<date>-<cone>.md/status.json`.
