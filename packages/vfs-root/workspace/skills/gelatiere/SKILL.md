...
| ------------------- | ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `gelatiere-install` | `{ id, skill, install, title }` | Run the `install` command verbatim (an `upskill <repo> … --skill <name>` invocation) and report success in one line. The card is already settled by the runtime. If `install` is missing, `upskill search <skill>` and ask first. |
| `gelatiere-try`     | `{ id, prompt, title }`         | Treat `prompt` exactly as if the user had typed it and do the work. The card is already settled by the runtime.                                                                                                                   |
