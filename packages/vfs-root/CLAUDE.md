...
virtual filesystem on init/reset. Paths below are relative to `packages/vfs-root/`.
...
Design, full pieces table, and sprinkle details: `docs/gelatiere.md`.
...
- `shared/GELATIERE.md` is the gelatiere's twin of `MEMORY.md`: the store's build-time fallback and
  ...
  dialect (`base/instruction-frontmatter.ts`); keys are `intervalHours`, `nightly` (5-field cron),
...
- The gelatiere is a persistent SCOOP under a synthetic owner (`scoops/gelatiere-unit.ts`), not a
  ...
  `GELATIERE_ALLOWED_COMMANDS` (a child escalates anything else); cross-pass memory in
...
- `shared/MEMORY.md` is the single source for the runner's build-time fallback and the seeded
  ...
  hard character budget applies to the whole file, no protected region.
...
- Every `##`/`###` section ends with a `YYYY-MM-DD` last-verified date (UTC, matching archive
  timestamps). Each pass re-verifies oldest sections first; undated ones are maximally stale.
...
- Write grant is `/workspace/CLAUDE.md` alone, not `/workspace/`: a directory-wide grant would also
  ...
  Single-file `writablePaths` entries work because `generateScoopSudoers` emits the bare path and
  `/**`.
- The pass is detached, so it spawns with `notifyOnComplete`; its closing message reaches the cone
  on the `scoop-notify` channel (where a skill suggestion lands), else the report is lost.
...
- `etc/APPROVALS.md` is the single source for the runner's build-time fallback and the seeded
  ...
  same place and reason as `/etc/sudoers`: the file is self-protected, so anything else creating it
  (the `upgrade apply` merge) would prompt the owner to approve the default already in force.
...
- **Skills**: add under `workspace/skills/<skill-name>/` with a `SKILL.md` plus any companion assets
  or `.jsh` scripts.
- **Sprinkles**: add under `shared/sprinkles/<name>/`; keep the main file named `<name>.shtml` to
  match discovery.
...
- `shared/wiki/` seeds the shared knowledge base at `/shared/wiki/`: `WIKI.md` is the schema
  contract (upgrade-merged like `MEMORY.md`); `index.md`/`log.md` are user data seeded once.
- The read-only CLI is `workspace/skills/wiki/wiki.jsh`, adapted from `ai-ecoverse/skills` →
  `skills/llm-wiki/wiki.jsh` with the wiki root pinned to `/shared/wiki`; keep it in sync with
...
- The `//` comment must name every `CommandId` (a test asserts it) and carries the v1 keymap as a
  paste-back block, where `s` maps to `sprinkles` (not a cycle command).
...
`navigate` lick → cone approval card. When behavior changes, keep the skill and doc aligned; do not
...
distinct from the developer-facing `CLAUDE.md` files in the repo. Don't merge the two roles.
