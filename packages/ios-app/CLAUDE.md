...
- `SliccFollower/Views/` — chat (compact workbench sets `toolbarSuppressed`; http(s) links open in-app unless `openLinksInBuiltInBrowser` off), sprinkles (`.shtml`), dock (48pt rail), `TerminalView`, `TabsCarouselView` (full-screen hides rail via `browserViewingTabId` / `viewingComputerId`; computer cards watch at 2 fps / 480 px only while visible), `ToolProgressChrome`, thread list (`ScoopSwitcher` → `ThreadListView`: slide-over when compact, sidebar when regular; picked by width + size class, never idiom/orientation; web parity table: [`docs`](../../docs/ios-app-details.md#thread-decoration-parity)). Invariants:
  ...
  - **Composer is a bottom inset** — `InputBar` rides `.safeAreaInset` on the transcript, never a `VStack` sibling (the keyboard blanked a long transcript); a snapshot never erases an unconfirmed send (`Sync/LocalSendLedger`). [`docs`](../../docs/ios-app-details.md#the-composer-is-a-bottom-inset-not-a-sibling)
