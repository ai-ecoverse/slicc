...
- `SliccFollower/Sync/` — `ThreadSync` (background per-unit transcript prefetch via `request_snapshot.peek`, leader ≥ 9: [`docs`](../../docs/ios-app-details.md#background-thread-sync)); `LocalSendLedger`; `Keepalive`; `TerminalClient` (single-flight `exec.*`); `ConnectionSettle` (`ConnectionHealth` + `ConnectionSettler` = blip hold behind `settledConnection`). `SliccFollower/CDP/` — `CDPBridge` + `CDPTarget` host WKWebViews as CDP targets.
- `SliccFollower/Views/` — chat (compact workbench sets `toolbarSuppressed`; http(s) links open in-app unless `openLinksInBuiltInBrowser` off), sprinkles (`.shtml`), dock (48pt rail), `TerminalView`, `TabsCarouselView` (full-screen hides rail via `browserViewingTabId` / `viewingComputerId`; computer cards watch at 2 fps / 480 px only while visible), `ToolProgressChrome`, thread list (`ScoopSwitcher` → `ThreadListView`: slide-over when compact, sidebar when regular; picked by width + size class, never idiom/orientation; web parity table: [`docs`](../../docs/ios-app-details.md#thread-decoration-parity)). Invariants:
  ...
  - **Composer is a bottom inset** — `InputBar` rides `.safeAreaInset` on the transcript, never a `VStack` sibling (the keyboard blanked a long transcript); it floats as glass with no band (`floatingGlass`), and the transcript is re-identified per unit; a snapshot never erases an unconfirmed send (`Sync/LocalSendLedger`). [`docs`](../../docs/ios-app-details.md#the-composer-is-a-bottom-inset-not-a-sibling)
  - **Provider error parity** — `Models/ErrorFamilies.swift` mirrors the web exhausted-budget table for Adobe and Grok, including optional reset copy; follower cards stay actionless because provider changes belong to the leader. [`docs`](../../docs/ios-app-details.md#follower-state-invariants)
...
- `computers.list` / `computer.frame` → cards + live JPEG (`AppState+Computers`). iOS never advertises `capabilities.computer` and ignores `computer.native.*`. Soft keys send `computer.input`. Watch is visibility-refcounted.
