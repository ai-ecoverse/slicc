...
| `SliccFollower/Models/ICloudSessionList.swift`, `SliccFollower.entitlements`                        | iCloud tray-session discovery over `SliccTraySession` + KVS entitlement; liveness uses shared `SessionReachability`                                                                                                                                                                                         |
...
`AppState.sessionStore` reads **`packages/swift-traysession`**; the launcher publishes, the phone joins, and `SettingsView` selects sessions. Shared `SessionReachability` follows bounded `TRAY_SUPERSEDED` chains; only terminal HTTP 200 with `leader.connected == true` is live. It never persists replacement join URLs. KVS id `S8LB56P782.ai.sliccy.trays` MUST match macOS. Unprovisioned builds use an empty cache; `SLICC_IOS_NO_ICLOUD=1` omits the entitlement. Never expose `joinUrl`.
