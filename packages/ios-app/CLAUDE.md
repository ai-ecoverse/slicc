...
| `SliccTrayKit/FileProvider/`, `SliccFileProvider/`                                                  | Read-only Files.app provider for the leader VFS (kit logic + thin appex). Appex Info.plist must set `NSExtensionFileProviderSupportsEnumeration` or Files can register the domain while leaving `userEnabled=false` and hiding it from Browse → Locations. |
