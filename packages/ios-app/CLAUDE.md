...
- `SliccFollower/Views/` — chat (compact workbench sets `toolbarSuppressed`; http(s) links open in-app unless `openLinksInBuiltInBrowser` off), sprinkles (`.shtml`), dock (48pt rail), `TerminalView`, `TabsCarouselView` (full-screen hides rail via `browserViewingTabId` / `viewingComputerId`; computer cards watch at 2 fps / 480 px only while visible), `ToolProgressChrome`. Invariants:
