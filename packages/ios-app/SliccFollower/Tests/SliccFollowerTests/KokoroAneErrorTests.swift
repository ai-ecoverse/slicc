...
final class KokoroAneErrorTests: XCTestCase {
    func testErrorDescriptionsAreStable() {
        ...
        let cases: [(KokoroAneError, String)] = [
            ...
            (.vocabParseFailed(vocabURL, "bad json"), "KokoroAne vocab.json at /tmp/vocab.json is malformed: bad json"),
            ...
            (.acousticFramesExceedCap(have: 2500, cap: 2000), "KokoroAne PostAlbert produced T_a=2500 frames > MAX_FRAMES=2000. Chunk the input."),
            (.unexpectedOutputShape(stage: "Noise", expected: "[1, T, 256]", got: "[1, 0]"), "KokoroAne stage 'Noise' returned unexpected shape (expected [1, T, 256], got [1, 0])."),
