...
extension AppState {
    /// Tray stream caps — match `COMPUTER_TRAY_MAX_FPS` / `_MAX_WIDTH` in
    /// `tray-sync-protocol.ts`. The leader also caps; asking higher is wasted.
    ...
    /// Refcounted: a visible card and the full-screen view can both watch.
