...
class AppState: ObservableObject {
    ...
    /// The leader's reason for each of this device's sends it rejected
    /// (`user_message_ack`), keyed by message id; painted under the bubble.
    ...
    private func ingestSnapshot(messages chatMessages: [ChatMessage], scoopJid: String) {
        ...
        if scoopJid == selectedScoopJid {
            // An identical snapshot (the revalidation of a warm buffer) must not
            // republish: every row would be rebuilt for nothing.
