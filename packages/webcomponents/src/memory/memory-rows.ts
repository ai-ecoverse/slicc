/**
 * Markdown → `<slicc-memrow>` cards. Lives next to the row element so the
 * library does not import webapp. Hosts inject a markdown→HTML renderer
 * (`RenderMarkdown`) — webapp passes `renderMessageContent`.
 */
...
/** Host-supplied markdown→HTML callback. Keep this package webapp-free. */
...
function markdownText(markdown: string, renderMarkdown: RenderMarkdown): string {
  return (trustedFragment(renderMarkdown(markdown)).textContent ?? '').replace(/\s+/g, ' ').trim();
}

/** Prefer a nearby clause boundary, then enforce a lossless hard cap. */
function splitBullet(markdown: string, renderMarkdown: RenderMarkdown): MemorySplit {
  const plain = markdownText(markdown, renderMarkdown);
...
function tagForMemory(
  markdown: string,
  section: string,
  renderMarkdown: RenderMarkdown
): MemoryTag | null {
  const content = markdownText(markdown, renderMarkdown);
...
function memoryRow(markdown: string, section: string, renderMarkdown: RenderMarkdown): MemoryRow {
  const split = splitBullet(markdown, renderMarkdown);
  return {
    title: split.title,
    summary: split.summary,
    section,
    tag: tagForMemory(markdown, section, renderMarkdown),
 ...
 */
export function parseMemoryRows(markdown: string, renderMarkdown: RenderMarkdown): MemoryRow[] {
  ...
  const flush = (): void => {
    ...
    if (text) rows.push(memoryRow(text, current.section, renderMarkdown));
    current = null;
  };
  for (const line of markdown.split('\n')) {
    ...
    if (heading) {
      flush();
      const headingText = markdownText(heading[2], renderMarkdown);
...
export function createMemoryRows(markdown: string, renderMarkdown: RenderMarkdown): HTMLElement[] {
  return parseMemoryRows(markdown, renderMarkdown).map((row) => {
