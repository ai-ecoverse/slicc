/**
 * The DOM-free half of `<slicc-time-preview>`: turning resolved occurrences
 * into the labels, week strip and day bar the card draws.
 *
 * Everything calendar-shaped is computed on `YYYY-MM-DD` KEYS rather than on
 * instants. A key is read off an instant once, in the target time zone, and
 * all day stepping after that is plain UTC calendar arithmetic on the key —
 * which is immune to DST transitions in a way that "add 24 hours" is not.
 */

/** One resolved occurrence, as the time parser reports it (ISO strings with offset). */
export interface TimeOccurrence {
  start: string;
  end?: string;
  allDay: boolean;
  /** The expression bounded only one side ("after 6pm" → `end`). */
  open?: 'start' | 'end';
}

/** Everything the card needs to draw one time expression. */
export interface TimePreviewData {
  /** The phrase as it appeared in the message. */
  text: string;
  /** The "now" the phrase was resolved against (ISO). */
  reference: string;
  /** IANA time zone the occurrences are shown in. */
  timeZone: string;
  occurrences: TimeOccurrence[];
  /** RFC 5545 recurrence rules, when the phrase repeats. */
  rrules: string[];
...
export interface WeekStripDay {
  ...
  weekday: string;
  day: number;
  /** An occurrence starts on this day. */
  marked: boolean;
  /** This day is the reference day ("today"). */
  today: boolean;
}

/** The part of a day an occurrence covers, as fractions of 24 h. */
export interface DayBar {
  from: number;
  /** Equal to `from` for an instant. */
  to: number;
  /** Where "now" falls, when the occurrence is on the reference day. */
  now?: number;
}

const DAY_MS = 86_400_000;

/** Calendar parts of an instant in `timeZone`. */
function zonedParts(
  ms: number,
  timeZone: string
...
/**
 * The day an occurrence starts on. An all-day occurrence may arrive as a bare
 * `YYYY-MM-DD`, which `Date.parse` reads as UTC midnight — re-zoning that
 * would move it to the previous day anywhere west of Greenwich.
 */
...
/** "Thu, Sep 24" for a `YYYY-MM-DD` key. */
...
/**
 * "in 3 days", "2 hours ago", "now" — the coarsest unit that is still honest.
 * Rounds rather than truncates, so 23.6 hours reads "in 24 hours", not 23.
 */
export function relativeLabel(targetMs: number, referenceMs: number, locale?: string): string {
  ...
  for (const [unit, size] of units) {
    // Days stay days up to two weeks: "in 10 days" beats "in 1 week".
...
/**
 * A short English reading of one RFC 5545 rule: "Every weekday",
 * "Every 2 weeks on Fri", "Monthly on the last Fri". Unknown shapes fall back
 * to "Repeats" rather than to the raw rule — the card is for people.
 */
...
/**
 * Seven days, Monday first, covering the first occurrence (or the reference
 * day when there is none), with every occurrence that starts in them marked.
 */
...
/**
 * Where an occurrence sits inside its start day. An end past midnight is
 * clamped to the end of the day — the bar shows one day, the label shows the
 * real range.
 */
...
/** "9:00 AM – 10:00 PM", "All day", "after 6:00 PM" for one occurrence. */
