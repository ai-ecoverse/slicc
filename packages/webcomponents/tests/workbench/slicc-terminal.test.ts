 ...
 * for wterm to open into) and wait for the async WASM load to finish.
 */
...
/** Poll until the dynamically-imported wterm is initialized. */
...
/** Read the rendered wterm rows from the shadow DOM (what the user sees). */
...
/**
 * Poll until a condition holds (wterm paints asynchronously — a
...
describe('slicc-terminal', () => {
  ...
  describe('write API', () => {
    it('renders direct Kitty RGB graphics through the Ghostty core', async () => {
      const el = await mount();
      // One red pixel, transmitted and placed by the Kitty graphics protocol.
  ...
  describe('lifecycle', () => {
    ...
    it('does not throw when removed before wterm finishes loading', async () => {
      ...
      el.style.height = '240px';
      document.body.appendChild(el);
      // Remove immediately, while the async import or WASM load is in flight.
      expect(() => el.remove()).not.toThrow();
