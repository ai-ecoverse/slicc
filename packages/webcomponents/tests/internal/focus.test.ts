...
/** A scope with two slots; moving between them is what a layout rebuild does. */
...
describe('withFocusPreserved', () => {
  ...
  it('without it, the same move drops the focus to the body', () => {
    // Pins the browser behaviour the helper exists for — if a future Chromium
    // kept the focus across a plain move, this would say so.
    const { a, b } = scope();
    const ta = document.createElement('textarea');
    a.append(ta);
    ta.focus();
  ...
  it('leaves focus outside the scope alone', () => {
    ...
    // The rebuild itself moved the focus; the helper only undoes a LOSS of
    // focus it owned, never a focus change it did not start from.
