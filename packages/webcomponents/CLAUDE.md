...
- **Agent-avatar expression kit:** `<slicc-agent-avatar activity>` adds shape morph, brows, lids and gaze behaviours on top of the four original channels. Every channel is a point, a scalar or an event (SwiftUI-mirrorable); the pure grammar is `switcher/avatar-expression.ts`. No `activity` attribute = the legacy pointer-tracking face. Static outranks every channel. Table + constants: `docs/webcomponents-details.md`.
