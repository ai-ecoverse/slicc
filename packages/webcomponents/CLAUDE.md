...
- **Slotted subtrees + chat prose need containment.** `::slotted()` matches only a slot's TOP-LEVEL children, so a shadow component accepting rendered markdown (`slicc-lick-card`) ships containment as an idempotent document `<style>` keyed to the host tag; chat message bodies carry `overflow-wrap: anywhere` (fenced code opts OUT via `pre code`). The same body chrome caps **markdown media**: `.msg__media` (img/video) maxes at the column width and `.msg__media-gallery` grids runs of two or more, so a 4K frame or a batch of clips cannot widen the transcript. The webapp renderer stamps those classes; the class names are the contract (`packages/webapp/src/ui/message-renderer.ts`), since `webcomponents` must not import from `webapp`. Rationale: `docs/webcomponents-details.md`.
