...
- **Drag-drop interaction**: `tilesMovable` / `tiles-movable` remains an opt-in embedder/test contract but is dormant in the shipped webapp: flag-off leaves it off, while flag-on replaces the dock-tree with `<slicc-layout>`. When enabled, unlocked leaves show a corner move button and share the drop state machine with `beginExternalDrag`; locking still wins.
