...
- **Cup mode (`--cup`)**: standalone-only; no cone is bootstrapped (`skipConeBootstrap`). Chrome boots with `?cup=1` giving exactly one CDP authority. Exposes a loopback HTTP steering API: `POST /api/shell/exec`, `GET /api/shell/session/:id`, `GET /api/vfs/read`, `POST /api/vfs/write`, `GET /api/vfs/stat`, `POST /api/vfs/list`, `POST /api/lick/emit`, `GET /api/targets`. All routes forward to the connected browser via the lick bridge (`src/routes/lick-bridge.ts` → `packages/webapp/src/scoops/lick-ws-bridge.ts` → `packages/webapp/src/scoops/shell-bridge-handler.ts` — the latter two are **webapp** files, not node-server). **Lick-back** (the symmetric outbound mirror of `/api/lick/emit`) adds `POST /api/lickback/claim`, `POST /api/lickback/heartbeat`, `GET /api/lickback` (SSE drain), `POST /api/lickback/reply`, and `POST /api/lickback/stop`: an external brain claims a channel (`chat`; cup-owned atomic claim + ~45s lease, overridable via `LICKBACK_LEASE_MS`) and answers the browser's chat-panel messages plus the cone's orphaned licks (`upgrade`/`sprinkle`/…) that the cone-less webapp would otherwise drop. The SSE drain emits a `: ping` keepalive every `LICKBACK_PING_MS` (default 25s) so the consumer's `fetch` never hits undici's 300s default `bodyTimeout` and mis-reads an idle-but-alive cup as gone. `POST /api/lickback/stop` is the operator stand-down: it releases the channel owner and ends the owner's open SSE with an `event: lickback-control` frame so the handler's blocked long-poll consumer returns (exit 4) and stops **without** killing the cup — it is loopback-trusted + session-present but deliberately **NOT** owner-gated (the steering session has a different `X-Slicc-Session` than the handler that owns the channel; owner-gating would make it unusable — do not "harden" it into owner-gating), and is strictly less powerful than the already-exposed claim route and `/api/shell/exec`. Ownership/queue/SSE live in `src/routes/lickback-registry.ts` (`registry.stop()` releases ownership first, then signals the drain — no persistent latch, preserving the buffered queue for a later owner); routes in `src/routes/lickback-api.ts` (both wired by `mountCupRoutes`); the browser's outbound push drains through the bridge's `setLickbackSink`. See `.claude/skills/slicc-lickback-handler/SKILL.md` for the handler-subagent role (attach-only: it never launches a cup; lifecycle belongs to the steering session). `GET /api/targets` returns the federated fleet — local CDP tabs plus tray followers, each annotated with a `runtime` (null for local, the follower runtime id for federated) — **unfiltered**: unlike `playwright tab-list` (which post-filters to actionable pages via `isActionablePage`), it includes the `?cup=1` app tab and chrome-internal targets, so a consumer must skip the app tab (navigating it kills the cup). The worker-side handler aggregates via `scoops/federated-targets.ts` (local `listAllTargets()` + a `list-remote-targets` panel-RPC supplement). Trusted-localhost (spec §9): a per-process bridge token IS minted (cup is thin-bridge), gating `/cdp` and the cross-origin `/api` CORS gate (the hosted leader's same-token requests are allowed, others `403`); the steering routes are additionally loopback-only via a Host-header guard, and loopback / no-Origin callers run ungated — the steering path. The trust boundary is the `127.0.0.1` bind + the Host guard (DNS-rebinding defense), not token absence. Session identity is caller-supplied via `X-Slicc-Session` header; sessions GC after 5 min idle. On boot it writes a discovery file `~/.slicc/cup.json` (`{ port, pid, startedAt }`, mode 0600, cleared on exit; see `src/cup-discovery.ts`) and `GET /api/status` carries `{ status, service, timestamp, cup, servePort, pid }` (see `buildStatusPayload` in `src/links-middleware.ts`; `cup`/`servePort` are the discovery-relevant fields for attach detection), so a second orchestrator session can detect a running bridge and attach with its own session instead of launching a parallel instance. Mutually exclusive with `--hosted`. Extension float has no node-server — cup is standalone-only (spec §11). See `.claude/skills/slicc-cup/SKILL.md` for the full steering API.
