/**
 * Reference HTTP computer behind `--computer-demo`. Implements the
 * phase-3 URL contract so `computer add url http://127.0.0.1:<port>` and
 * the fake-LLM e2e can drive a real remote without a third-party host.
 *
 *   GET  /computer
 *   GET  /computer/screenshot?maxWidth=&format=
 *   GET  /computer/text
 *   POST /computer/input
 *   WS   /computer/frames?fps=4   (optional; HTTP poll still works)
 */
...
/** 1×1 SOF0 JPEG — enough for protocol tests; not a displayable photo. */
...
/** True when this upgrade was the demo frames socket. */
