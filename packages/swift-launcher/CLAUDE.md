...
`SliccstartWidgets.appex` (`com.slicc.sliccstart.widgets`) shows cones and scoops in Notification Centre / on the desktop; views live in **`packages/swift-widgetkit`**, this package owns only the `@main` bundle and build wiring. Capture is `Models/WidgetTrayObserver.swift`: a **read-only tray follower** off `leaderJoinUrl`, **gated on the widget being installed** (`WidgetInstallationQuery`) so no WebRTC slot stays open for an unused tile. Native desktop capture is a **separate** follower (`Models/ComputerTrayFollower.swift`, runtime `sliccstart-computer`) that **always dials** when a leader join URL is set and never advertises `exec`. **`capabilities.computer` is derived from the live Screen Recording grant, never hardcoded** (`Models/ComputerCapabilityAdvertisement.swift`) — claiming it while ungranted costs the leader the `screencapture` tray-exec fallback; TCC has no change notification, so a grant watch re-sends `hello` when a grant flips and the Accessibility half rides in the MOTD. [details](../../docs/swift-launcher-details.md#widget-extension)
...
**SIGPIPE** — `main.swift` calls `BrokenPipeSignal.ignore()` before anything else, so a write to a closed socket or pipe (libwebrtc, stdio) returns `EPIPE` instead of silently killing the GUI or a headless mode (#3418). Write stdio with `try? FileHandle.write(contentsOf:)`, not the raising `write(_:)`.
...
**Headless CLI (`Sliccstart --computer-follow` / `--computer-preflight`)** — parsed in `main.swift` right after `--list-sessions`, **before** SwiftUI boots, for `slicc <url> follow --computer` (#3260). `--computer-follow <url> [--pair <token>]` runs **only** `ComputerTrayFollower`: no menu bar, no widget observer, `NSApplication` `.accessory` (**not `.prohibited`** — that suppresses the TCC prompts this whole detour exists for). Stdout is a line protocol: `SLICC_COMPUTER_FOLLOW_READY` **immediately** (knows the flag — an older launcher ignores it and boots its GUI, never exiting), then `SLICC_COMPUTER_FOLLOW_ATTACHED` after `hello` is sent, or `SLICC_COMPUTER_FOLLOW_FAILED <reason>` + exit 1. **Ready is not attached** — never print attached before `onConnected`. It exits when its **parent pid** dies (kqueue), so a SIGKILLed CLI cannot leave it capturing. `--pair` rides on `hello.pairId` so the leader folds this follower into the CLI's roster entry. `--computer-preflight [--json]` raises both prompts and reports grants (exit 0 granted / 3 not). A malformed invocation **exits 2** rather than falling through to the GUI. [details](../../docs/swift-launcher-details.md#headless-cli-sliccstart---computer-follow--computer-preflight)
