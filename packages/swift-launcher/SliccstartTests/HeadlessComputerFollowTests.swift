...
/// The rules the `slicc … follow --computer` CLI relies on when it reads the
/// headless launcher's stdout (#3260, #3339 review): ready at once, attached
/// only after `hello`, and every ending stopping the follower first.
@MainActor
final class HeadlessComputerFollowTests: XCTestCase {
    ...
    /// Hop through the main actor so the follower's `Task { @MainActor … }`
    /// delegate bodies have run.
    ...
    func testAttachedIsReportedOnceWhenTheChannelOpens() async {
        ...
        await drain()
        openChannel(follower)  // a reconnect after a leader drop
        await drain()
    ...
    /// The first review finding: a launcher that could not reach the leader was
    /// still reported healthy. Now it names the reason and exits non-zero.
    ...
    /// The second review finding: a SIGKILLed CLI left this process running and
    /// able to capture. The parent watch ends it — cleanly, follower first.
