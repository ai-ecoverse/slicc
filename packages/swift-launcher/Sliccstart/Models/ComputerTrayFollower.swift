...
/// Sliccstart's native computer follower: ScreenCaptureKit JPEG frames and
/// CGEvent input over additive `computer.native.*` tray messages.
///
...
final class ComputerTrayFollower: NSObject {
    ...
    /// How an input `wait` sleeps; tests hold it open to observe what the
    /// main actor does meanwhile.
