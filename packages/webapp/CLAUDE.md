...
- `speak.ts` — shared synthesis surface: `speak()` picks kokoro (ready + English, or an explicit kokoro voice id) with Web Speech fallback; `speechTextFromMarkdown` reduces replies to speakable prose; `hasVoiceForLang(lang)` reports whether any engine (on-device kokoro or an installed Web Speech voice) can speak a language, and `webSpeak` picks a Web Speech voice matching `req.lang` when no explicit voice is given. Consumed by the `say` command's local path AND the `speak-text` / `list-voices` panel-RPC handlers, so the worker float picks the kokoro upgrade up with no protocol change.
