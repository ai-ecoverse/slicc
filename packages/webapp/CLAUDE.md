...
- **Next-gen WC shell (`?ui=wc`)**: Phase 0 of the `@slicc/webcomponents` migration. `ui/wc/` mounts the library's full-app shell (nav / freezer rail / chat thread / composer / workbench / dock) over the design-time chat fixture instead of booting the legacy layout — standalone float only; the legacy UI stays the default. Webapp code imports per-component subpaths (`@slicc/webcomponents/src/...`), never the package barrel: the library's `slicc-press-button` tag collides with the legacy webapp element of the same name until that legacy file is replaced in a later phase. The webapp coverage gate excludes `packages/webcomponents/**` (the library enforces its own, higher floors in its own package).
