...
- **Model `temperature` capability**: Bedrock-backed Claude Opus 4.7/4.8 reject `temperature` (Bedrock 400 → Adobe proxy 502 → relayed on `/api/fetch-proxy`). `src/providers/temperature-support.ts` (`modelSupportsTemperature` / `withSupportedTemperature`) owns the single reject-list; both `providers/adobe.ts` and `providers/built-in/bedrock-camp.ts` consult it. Add a new rejecting model there, **not** at the call site. pi-ai already drops `temperature` when thinking is enabled, so this only affects thinking-disabled helper calls (`ui/quick-llm.ts`). See `docs/pitfalls.md`.
