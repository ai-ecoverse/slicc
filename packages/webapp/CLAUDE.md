...
- `whisper-engine.ts` — lazy `onnx-community/whisper-tiny` loader mirroring `ffmpeg-wasm.ts`: dynamic `import('@huggingface/transformers')`; both the library AND its weights load from the VFS (Wave 7) — `ipk add @huggingface/transformers onnxruntime-web` plus `hf download onnx-community/whisper-tiny` populates them. `transformers-env.ts` pins `wasmPaths` to `toPreviewUrl('/workspace/node_modules/onnxruntime-web/dist/')` and `localModelPath` to `toPreviewUrl('/workspace/models/')` with `allowRemoteModels = false`, so the preview SW streams everything from OPFS; no CDN. WebGPU (fp32) with automatic WASM (q8) retry; `warmup` fires on the first granted push-to-talk hold — never at boot.
...
- `kokoro-engine.ts` — lazy Kokoro-82M TTS (`onnx-community/Kokoro-82M-v1.0-ONNX` via `kokoro-js`); same Wave 7 contract as whisper — `ipk add kokoro-js` for the library, `hf download onnx-community/Kokoro-82M-v1.0-ONNX` for the weights, then the warmup CHAINS automatically off the whisper load (`whisper-engine.ts`). kokoro-js pins transformers ^3.x — the Vite configs `dedupe` it onto the workspace 4.x (npm overrides don't reach workspace deps) so one transformers + one ort version ships. The 4.x registry refactor keys `MODEL_TYPE_MAPPING` by class name only, so a config-only resolve of Kokoro's `style_text_to_speech_2` model_type misses and warn-falls back to EncoderOnly; `applyStyleTts2ConfigShim` patches the shared `AutoConfig.from_pretrained` to inject `architectures: ['StyleTextToSpeech2Model']` before load — a clean direct hit, whisper untouched (model_type guard).
...
- **Follower mode (no-kernel multi-browser sync):** when a validated `/join/` URL or `?tray=` query is present (plus `?cherry=1` for embedded followers), `main.ts` delegates to `wc-follower.ts:mountWcUiFollower` — a page-side follower boot (chat + sprinkles + leader-driven CDP) that runs with **no kernel worker**. `SprinkleFollowerController` implements the follower's sprinkle mirroring via `page-follower-tray.ts`, which gets an `onOpen` hook so `bridge.open(path)` can guard relative paths (no VFS responder → `/preview/*` would 404). Page-side navigate-lick forwarding is wired via `follower-navigate-watcher.ts` for non-cherry floats. Switch-out is storage-only and triggers a `location.reload()`. `/licks-ws` HTTP injection is not available in this mode.
