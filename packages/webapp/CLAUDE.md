...
- **opus-4-8 model-capability shims** (pinned pi-ai 0.75.3 predates opus-4-8 → Bedrock 400 → Adobe proxy 502 relayed on `/api/fetch-proxy`): fix at the provider layer, never the call site. (1) **temperature** — Opus 4.7/4.8 reject it; `src/providers/temperature-support.ts` (`modelSupportsTemperature` / `withSupportedTemperature`) owns the reject-list, consulted by `providers/adobe.ts` + `providers/built-in/bedrock-camp.ts` (only thinking-disabled helpers like `ui/quick-llm.ts` hit it — pi-ai drops temperature when thinking is on). (2) **adaptive thinking** — opus-4-8 needs `thinking:{type:"adaptive"}` + `output_config.effort`, but pi-ai emits the legacy enabled+budget shape; `src/providers/adaptive-thinking.ts` (`withAdaptiveThinkingShim`) wires an `onPayload` rewrite in `providers/adobe.ts` (hits the main stream). A new affected model is a one-line edit to each module's list. See `docs/pitfalls.md`.
