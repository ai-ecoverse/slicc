...
- `preview-sw.ts` serves `/preview/*` content from VFS and is built as a standalone IIFE. **Legacy path** (pre-Phase-3): used only by `open <vfs-path>` for one-off file opens. The `serve` shell command no longer routes here — it mints a worker-hosted `<token>.preview.sliccy.ai` URL via `scoops/preview-request-handler.ts` (leader-side) and the worker pipe described in the cloudflare-worker package.
