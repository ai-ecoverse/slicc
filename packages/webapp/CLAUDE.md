...
- **just-bash UTF-8 patch**: `patches/just-bash+3.0.1.patch` (postinstall, patch-package) mirrors just-bash PR #265 — `Interpreter.executeScript` decodes each statement's stdout before concatenating, fixing Latin-1 mojibake in `$(...)` and text/byte statement interleave (issue #957). Drop it when a just-bash release includes the fix. Guarded by `tests/integration/shell-utf8.integration.test.ts` (real browser — the Node harness can't run multi-statement scripts) plus the Node subset in `tests/shell/just-bash-utf8.test.ts`.
