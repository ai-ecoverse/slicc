...
- `host.ts` — `createKernelHost(config)` factory. Single boot sequence shared by the offscreen document (extension), the standalone DedicatedWorker, and tests: orchestrator + lick-manager + agent-bridge + tray subs + cone bootstrap + BshWatchdog + `/proc` mount. Returns `{ orchestrator, browser, bridge, lickManager, sharedFs, processManager, dispose }`. In non-extension floats the host also opens the `/licks-ws` bridge (`scoops/lick-ws-bridge.ts`) so the node-server's `/api/webhooks`, `/api/crontasks`, `/api/tray-status`, and inbound webhook/handoff routes can talk to the worker-side `LickManager` — that wire was deleted with the inline-orchestrator removal and is restored here.
