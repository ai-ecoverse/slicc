...
/**
 * Tests for the hover-preview wiring: what it decorates, which card a hover
 * opens, and when a question may be answered.
 */
...
/** Flush idle callbacks (setTimeout fallback) and pending promises. */
...
beforeEach(() => {
  ...
  // jsdom has no rAF-driven layout; run callbacks immediately.
...
describe('wireMentionPreviews', () => {
  ...
  it('opens a link card on hover and fills it from the fetched page', async () => {
    ...
    // Leaving the anchor for somewhere else schedules the close.
  ...
  it('answers the latest question and dispatches the answer on the thread', async () => {
    ...
    // Re-opening shows the answer, and it cannot be answered twice.
    card.hide();
    span.click();
    await settle();
