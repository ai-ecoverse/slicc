...
describe('attachmentHandling', () => {
  it.each([
    ...
    ['application/octet-stream', 'data.ts', 'text-redacted'],
    ['application/octet-stream', 'script.js', 'text-redacted'],
    ['application/octet-stream', 'data.bin', 'binary-unchanged'],
...
describe('processTranscriptAttachments — VFS path-only UI attachments', () => {
  ...
  it('reads binary attachment bytes via vfsReader when path is provided', async () => {
    ...
    const vfsBytes = new Uint8Array([0x50, 0x4b, 0x03, 0x04]);
