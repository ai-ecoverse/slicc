/**
 * A detached `bash` job that dies with the kernel worker is reported on the
 * next boot instead of vanishing (#3441): the journal outlives the worker,
 * and the sweep turns a leftover record into the completion lick the agent
 * was waiting for.
 */
...
/** Plant the record a previous kernel worker left behind when it died. */
