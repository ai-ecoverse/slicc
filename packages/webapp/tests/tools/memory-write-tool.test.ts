/**
 * `memory_write` — the single write path for memory files (#3157).
 *
 * The budget rule under test: under budget any write lands; over budget a
 * write lands only if it shrinks the file; every success reports the size
 * and the remaining room or overage so no `wc -c` turn follows.
 */
...
// Zero archived sessions → the base allowance (4000 + 2000·log2(2)).
