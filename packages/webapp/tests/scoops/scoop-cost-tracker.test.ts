...
describe('ScoopCostTracker', () => {
  ...
  it('aggregates costs by model across all live scoops', () => {
    ...
    expect(opus!.input).toBe(4500);
    expect(opus!.output).toBe(2250);
    expect(opus!.cost).toBeCloseTo(0.0675, 4);
    expect(opus!.turns).toBe(3);

    const sonnet = result.find((r) => r.model === 'claude-sonnet-4-5');
    expect(sonnet).toBeDefined();
    expect(sonnet!.input).toBe(1500);
    expect(sonnet!.output).toBe(750);
    expect(sonnet!.cost).toBeCloseTo(0.009, 4);
  ...
  it('includes dropped scoops in the model aggregation only on request', () => {
    ...
    expect(opus!.output).toBe(1500);
    expect(opus!.cost).toBeCloseTo(0.045, 4);
