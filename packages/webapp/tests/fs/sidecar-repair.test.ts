...
describe('repairSidecarDocument', () => {
  ...
  it('drops the self-referential /.metadata.json entry instead of truing it up', async () => {
    ...
    expect(summary.changed).toBe(true);
    expect(summary.sizesFixed).toBe(0);
...
describe('repairSidecarDocument — ino uniqueness (#2146)', () => {
  it('reassigns zero and duplicate inos to fresh unique ids above the max', async () => {
    ...
    expect(summary.inosReassigned).toBe(3);
    expect(summary.changed).toBe(true);
    const entries = doc.entries as Record<string, { ino?: number; data?: number }>;
    expect(entries['/'].ino).toBe(0);
    expect(entries['/c.txt'].ino).toBe(7);
...
describe('repair progress ticks', () => {
  it('ticks once per entry scanned, including root and untouched entries', async () => {
    ...
    const onEntry = vi.fn();
    await repairSidecarDocument(doc, probeFrom({ '/a': { kind: 'file', size: 1 } }), onEntry);
