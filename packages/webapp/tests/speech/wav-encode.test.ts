...
describe('encodePcmChunksToWav', () => {
  it('writes a valid RIFF/WAVE header for a single mono chunk', () => {
    ...
    expect(leU16(wav, 20)).toBe(1);
    expect(leU16(wav, 22)).toBe(1);
    expect(leU32(wav, 24)).toBe(24000);
    expect(leU32(wav, 28)).toBe(24000 * 2);
    expect(leU16(wav, 32)).toBe(2);
    expect(leU16(wav, 34)).toBe(16);
