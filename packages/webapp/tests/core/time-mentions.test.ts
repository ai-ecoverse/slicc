...
/**
 * A parser scripted by phrase: any input containing a known phrase reports a
 * span over it; parsing the phrase ALONE yields its occurrences.
 */
...
describe('findTimeMentions (scripted parser)', () => {
  ...
  it('drops spans that resolve to nothing on their own', async () => {
    // Found in context, but parsing the span alone yields no occurrences.
...
// The real model on the CPU backend — pins the heuristics above to how
// `gpu-time` actually behaves on agent prose.
