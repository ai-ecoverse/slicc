...
/**
 * A PNG whose compressed body holds `FF C0` plus plausible dimensions, as
 * every real screenshot does. The header-only `pngHeader` fixture is too
 * clean to reproduce #3372 — the spurious pair is what convinced the
 * transcode guard the PNG was already a JPEG.
 */
...
describe('page-side screenshotTab / inputTab', () => {
  ...
  it('marks overCap when the tab cannot encode narrower than the cap (#3373)', async () => {
    // A tab that ignores the cap hands back its native pixels. `--size` is a
    // promise about encoded width, so say it was missed rather than letting
    // the model believe a 1082 px frame is 500 px wide.
    const tab = {
      send: vi.fn(async () => ({})),
