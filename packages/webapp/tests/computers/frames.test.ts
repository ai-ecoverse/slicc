...
/**
 * A PNG whose compressed body contains `FF C0` followed by plausible
 * dimensions — what every real screenshot looks like, and what a free scan
 * for a SOF marker mistakes for a JPEG (#3372).
 */
...
describe('frozen frames', () => {
  ...
  it('names the file from the bytes, not from frame.mime', async () => {
    ...
    const png = pngWithSpuriousSofMarker();
    // `mime` claimed JPEG while the bytes were PNG for every poke frame on a
    // tab computer, so the extension must come from the payload (#3372).
    const path = await writeFrozenFrame({
      fs,
      cwd: '/',
