...
// Upstream quote-removed a case pattern before matching, so `-\?` was the
// wildcard `-?`. GNU libtool's `-\?|-h) func_usage` then caught every
// two-letter option, and `libtool --mode=compile …` printed its usage and
// exited (vercel-labs/just-bash#480).
describe.each([
  ['node', Bash],
  ['browser', BrowserBash],
