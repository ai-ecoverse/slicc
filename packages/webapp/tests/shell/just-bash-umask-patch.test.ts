...
// Upstream listed `umask` as a builtin but never implemented it, so
// `(umask 077 && mkdir "$tmp")` in config.guess, libtool and every
// config.status failed (vercel-labs/just-bash#475). The hunk adds the builtin,
// scopes the mask like the cwd, and applies it to files the shell creates.
// Redirection modes are covered in umask.test.ts on the VFS: InMemoryFs resets
// a file's mode on every write.
describe.each([
  ['node', Bash],
  ['browser', BrowserBash],
