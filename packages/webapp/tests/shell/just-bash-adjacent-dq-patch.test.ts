...
// Adjacent double-quoted segments parse on their own (vercel-labs/just-bash#493).
// Upstream 3.4.2 merged `"$x""_y"` into `"$x_y"`, so autoconf's
// AX_PREFIX_CONFIG_H (`"… $ac_prefix_conf_UPP""_\\1/"`) lost its prefix.
describe.each([
  ['node', Bash],
  ['browser', BrowserBash],
