...
// Upstream handed a pipeline stage's stdin to while/until, ( … ) and { …; }
// but not to if / for / (( ;; )) / case, so their bodies read nothing:
// `echo hi | if :; then cat; fi` printed nothing. Every autoconf config.status
// writes its outputs through `sed … | if …; then $AWK …; fi >out`, so GNU
// make's configure wrote an empty Makefile (vercel-labs/just-bash#499).
describe.each([
  ['node', Bash],
  ['browser', BrowserBash],
