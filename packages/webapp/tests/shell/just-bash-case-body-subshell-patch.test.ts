...
// Upstream's case-item parser errored on any `(` followed by a WORD token in
// an item body, so autoconf's AC_CHECK_HEADER block
// `( $as_echo "## Report this to … ##" ) | sed …` made libtiff's configure
// unparseable (vercel-labs/just-bash#486).
describe.each([
  ['node', Bash],
  ['browser', BrowserBash],
