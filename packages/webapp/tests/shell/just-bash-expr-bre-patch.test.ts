...
// POSIX expr matches a basic regex: \( \) group and return the capture.
// Upstream handed the pattern to RegExp as-is, so every autoconf configure
// died parsing `CPPFLAGS=…`: `expr "x$ac_option" : 'x\([^=]*\)='` gave 0.
describe.each([
  ['node', Bash],
  ['browser', BrowserBash],
