...
// POSIX pattern bracket expressions in `case` and `[[ == ]]`. Upstream copied
// `[...]` into the RegExp verbatim, so `[!...]` meant "! or ..." and
// autoconf's `case $ac_envvar in *[!_$as_cr_alnum]*)` rejected every VAR=value.
describe.each([
  ['node', Bash],
  ['browser', BrowserBash],
