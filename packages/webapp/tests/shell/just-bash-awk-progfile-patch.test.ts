...
// Upstream awk rejected `-f progfile` and read a `var=value` operand as a file
// name (vercel-labs/just-bash#468). libpng's configure and build run
// `awk -f scripts/options.awk out=… version=search file…`.
describe.each([
  ['node', Bash],
  ['browser', BrowserBash],
