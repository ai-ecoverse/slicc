...
// After a persistent `exec </dev/null`, upstream seeded every command's stdin
// from fd 0, even a pipeline stage fed by a pipe: `printf hi | cat` printed
// nothing. autoconf runs `exec 7<&0 </dev/null` near the top of every
// configure, so each `$(… | …)` after it came back empty.
describe.each([
  ['node', Bash],
  ['browser', BrowserBash],
