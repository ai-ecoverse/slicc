...
// An unquoted expansion that begins with an IFS delimiter ends the field built
// so far (backport of the splittable half of vercel-labs/just-bash#367). Upstream
// 3.4.2 joined it to the preceding literal: libtool's
// `--finish$current_libdirs` (current_libdirs=" /lib") became one unknown
// option and every libtool `make install` failed.
describe.each([
  ['node', Bash],
  ['browser', BrowserBash],
