...
// Upstream parsed -t but never sorted by mtime, and rejected -L. automake's
// "build environment is sane" check runs `ls -Lt configure conftest.file`
// (falling back to `ls -t`) and expects the new file first.
describe.each([
  ['node', Bash],
  ['browser', BrowserBash],
