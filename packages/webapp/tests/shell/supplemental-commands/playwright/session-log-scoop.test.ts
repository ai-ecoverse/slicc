/**
 * playwright-cli's best-effort session log must never raise a sudo approval in
 * a sandboxed scoop (#3440). The scoop's fs here is a real SudoFS over the
 * per-scoop policy shape (builtin `/tmp` grant + its own folder), with
 * `require-approval` as the default disposition — exactly what
 * `initShellAndSkills` wires for a child unit.
 */
...
/** Map-backed fs with just the surface playwright-cli's logging touches. */
...
/** A SudoFS shaped like a sandboxed scoop's (`/tmp` + its folder writable). */
function scoopFs() {
  const raw = memFs();
