...
describe('which command', () => {
  ...
  it('lists every .jsh copy when a built-in wins the same name', async () => {
    const mockVfs = createMockVfs(['/workspace/skills/a/cat.jsh', '/workspace/skills/b/cat.jsh']);
    const cmd = createWhichCommand({
      fs: mockVfs,
      getStaticBuiltins: () => ['ls', 'cat'],
    });
    const result = await cmd.execute(
      ['cat'],
      createMockCtx({
    ...
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('/usr/bin/cat');
    expect(result.stdout).toMatch(/shadowed by built-in cat/);
    expect(result.stdout).toContain('(shadowed /workspace/skills/a/cat.jsh)');
    expect(result.stdout).toContain('(shadowed /workspace/skills/b/cat.jsh)');
  });
  ...
  it('resolves a saved workflow to its path labeled (workflow)', async () => {
    ...
    const ctx: any = {
      cwd: '/workspace',
      env: new Map(),
      getRegisteredCommands: () => ['ls', 'cat', 'audit'],
    };
  ...
  it('shows the .jsh path and marks the workflow shadowed when both exist', async () => {
    ...
    const ctx: any = {
      cwd: '/workspace',
      env: new Map(),
      getRegisteredCommands: () => ['ls', 'cat', 'foo'],
    };
    const res = await createWhichCommand({
      fs,
      scriptCatalog: catalog,
  ...
  it('a static built-in wins over a same-named saved workflow (shadowed by built-in)', async () => {
    ...
    const res = await createWhichCommand({
      fs,
      scriptCatalog: catalog,
      getStaticBuiltins: () => ['ls', 'cat', 'test'],
    }).execute(['test'], ctx);
    expect(res.stdout).toContain('/usr/bin/test');
