...
// `exec cmd …` replaces the shell (vercel-labs/just-bash#495). Upstream 3.4.2
// ran the command and kept going, so libtool's `eval exec "$exec_cmd"` fell
// through to `exit $EXIT_FAILURE` and every libtool install exited 1.
describe.each([
  ['node', Bash],
  ['browser', BrowserBash],
