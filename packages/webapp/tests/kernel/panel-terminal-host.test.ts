...
describe('panel terminal execution deadline', () => {
  // just-bash aborts a command that runs past maxExecutionTimeMs (an hour by
  // default). Skew the clock two hours forward while a command sleeps.
  async function runAcrossTwoHours(
    ...
    await shell.executeCommand('true'); // settle boot-time work first
  ...
  it('the factory gives panel sessions those limits', async () => {
    ...
    try {
      await w.client.open();
      await w.client.exec('true'); // settle boot-time work first
      const realNow = Date.now.bind(Date);
      let skew = 0;
      const spy = vi.spyOn(Date, 'now').mockImplementation(() => realNow() + skew);
      const timer = setTimeout(() => {
