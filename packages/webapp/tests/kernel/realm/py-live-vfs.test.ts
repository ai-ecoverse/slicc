/**
 * `mountPyLiveVfs` end to end on real Pyodide: the fs transport is swapped
 * for a host-`node:fs` bridge and the exec route for a fake XHR, so one
 * Python script can write a file, run a "child" that must already see it,
 * and read back what the child wrote.
 */
...
/** Fake exec route: the "child" copies in.txt to out.txt and reports what it saw. */
class ExecXHR {
  status = 0;
  responseType = '';
  timeout = 0;
