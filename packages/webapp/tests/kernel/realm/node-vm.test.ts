/**
 * The realm's best-effort `vm`: contexts are plain objects evaluated through a
 * scope proxy. Pins the semantics emscripten's JS compiler relies on (settings
 * files of `var`s, library files of top-level functions, macro expressions
 * against a shared context), plus the end-to-end `require('node:vm')`.
 */
...
describe('vm contexts', () => {
  ...
  it('keeps top-level functions on the context for later runs', () => {
    ...
    // A redeclaration wins over the older context value within its own run.
  ...
  it('keeps top-level const/let/class for later runs (emscripten macro blocks)', () => {
    const ctx = vm.createContext({});
    // libwebgl.js: a {{{ }}} block declares a const that later macros read.
    ...
    // The completion value is the script's own, even with the capture.
