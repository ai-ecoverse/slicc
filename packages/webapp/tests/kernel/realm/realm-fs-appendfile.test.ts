/**
 * Realm-bridge concurrent appendFile (#3174).
 *
 * Mirrors `virtual-fs-mutations.test.ts` through createFsBridge →
 * vfs.appendFile RPC → VirtualFS.appendFile (withWriteLock). The
 * previous client-side exists→read→write lost the second payload.
 */
