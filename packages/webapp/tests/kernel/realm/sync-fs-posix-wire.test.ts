/**
 * Wire contract for the POSIX sync-fs ops, both transports:
 *   - SW route: what `createSyncFsXhrBridge` sends is exactly what
 *     `parseSyncFsRequest` decodes (and a hostile body is sanitized);
 *   - SAB: `createSyncFsSabBridge` builds the same request objects.
 */
...
/** Feed one recorded XHR through the SW parser. */
