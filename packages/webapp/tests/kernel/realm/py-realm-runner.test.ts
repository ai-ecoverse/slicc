/**
 * PYTHON_RUNNER against real Pyodide: a script imports the package that
 * sits beside it (CPython's sys.path[0]), which is how `emcc.py` finds
 * its `tools` package.
 */
...
function run(code: string, filename: string, argv: string[]) {
  ...
  // A plain JS array, as the realm sets it (it arrives as a JsProxy).
...
describe('PYTHON_RUNNER sys.path', () => {
  it("puts a script's directory first so its sibling package imports", () => {
    ...
    // CPython script mode: the script dir replaces the cwd entry.
