...
`);
  ...
  it('drops chmod / utime on a mount that stores no metadata instead of failing', () => {
    ...
    // The bridge still sees /work paths (the mount's root); they map into `meta`.
...
`);
      ...
      // The mount's own mode, not the dropped 0o600.
      expect(py('meta_mode')).toBe(nodeFs.statSync(join(meta, 'made.txt')).mode & 0o777);
      // Anything but ENOSYS still fails the call.
      expect(
        py(`
import errno
