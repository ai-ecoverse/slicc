 ...
 *  endpoint at boot and stuffs the result here so the (synchronous) provider
 *  `getModelIds` callback can answer without a network round-trip. */
...
/** Best-effort fetch of `GET /v1/models`. Refreshes the local cache on
 *  success; clears it on any error so SwiftLM transparently disappears
 *  from the model dropdown when the server stops. */
async function refreshModelsCache(): Promise<void> {
  try {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 2000);
    const res = await fetch(`${SWIFTLM_BASE_URL}/models`, { signal: controller.signal });
    ...
    const models: CachedModel[] = (json.data ?? [])
