...
/**
 * Optional hooks on the CLI launcher. `onPopupBlocked` is called with the
 * authorize URL when `window.open` returns null (popup blocker, a missing
 * user gesture, etc.). The callback lets the UI surface a clickable link
 * the user can activate themselves — a fresh click counts as a new user
 * gesture, so the browser will honor it. The returned promise keeps
 * listening for the OAuth callback until the user either completes the
 * flow via that link or the 2-minute timeout elapses.
 */
export interface OAuthLauncherOptions {
  onPopupBlocked?: (authorizeUrl: string) => void;
}

/**
 * Create an OAuthLauncher appropriate for the current runtime.
 *
 * The extension path uses `chrome.identity.launchWebAuthFlow` which has
 * no popup-blocker constraint; `options` is ignored there.
 */
export function createOAuthLauncher(options: OAuthLauncherOptions = {}): OAuthLauncher {
  if (isExtension) return launchOAuthExtension;
  return (authorizeUrl: string) => launchOAuthCli(authorizeUrl, options);
}
...
async function launchOAuthCli(
  authorizeUrl: string,
  options: OAuthLauncherOptions
): Promise<string | null> {
  return new Promise<string | null>((resolve) => {
    // Try the popup normally. If the browser declined to open it (blocked
    // by the popup blocker or user activation expired across awaits), fire
    // `onPopupBlocked` so the UI can render a fallback link. We keep the
    // promise pending; when the user clicks that link the postMessage /
    // server-polling channels below will still resolve us.
    const popup = window.open(authorizeUrl, '_blank', 'width=500,height=700,popup=yes');
    if (!popup) {
      try {
        options.onPopupBlocked?.(authorizeUrl);
      } catch (err) {
        console.warn(
          '[oauth-service] onPopupBlocked handler threw:',
