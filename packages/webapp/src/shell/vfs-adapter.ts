...
export class VfsAdapter implements IFileSystem {
  ...
  /**
   * Forward {@link VirtualFS.updateMetadataBatch} so `tar x` can apply many
   * modes/times with one sidecar write. Not part of just-bash `IFileSystem`.
   */
  ...
  /** See {@link VirtualFS.forgetSidecarConsistency}. Forwarded when the backing VFS has it. */
