 ...
 * (`throw` vs a command `exit`), so later statements never run and `2>` is
 * empty. It also reserves `stat.size * 2` against `maxLiveBytes` (512 MiB),
 * so a ~256 MiB tree trips a message that still quotes 512 MiB, and binary
 * files skipped as non-matches are charged before the skip.
 *
 * This overlay lists the files the bundled command would search, drops
 * binary / oversize inputs from the budget, and either:
...
/** Bytes of a file inspected for a NUL (just-bash rg uses the same window). */
export const RG_BINARY_PEEK_BYTES = 8192;

/**
 * Searchable-byte budget actually enforced by bundled rg's 2× live reservation
 * (`reserveBytes(stat.size * 2)` against `maxLiveBytes`) and its aggregate
 * input counter. Report this number, not the raw 512 MiB live/input labels.
 */
...
function parseRgArgv(args: string[]): ParsedRg {
  ...
  // -u (no ignore), -uu (+hidden), -uuu (+binary) — same ladder as bundled rg.
