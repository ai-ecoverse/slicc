 ...
 * the chooser reliably (mirrors the `mount-popup` rationale). So the
 * extension `usb request` path opens `usb-picker-popup.html` in a real
 * popup window; the popup runs `requestDevice` on its own button-click
 * gesture, which grants the device to the extension origin. The popup
 * posts back the device identifiers; the caller then re-acquires the
 * now-granted device via `navigator.usb.getDevices()` in its own realm.
 */
...
/**
 * Open `usb-picker-popup.html`, passing the requested filters, and
...
export function openUsbPickerPopup(filters: UsbDeviceFilter[]): Promise<UsbPickerResult> {
  ...
  const requestId = `usb-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
  const filtersParam = encodeURIComponent(JSON.stringify(filters ?? []));
  const url = chromeApi.runtime.getURL(
    `usb-picker-popup.html?requestId=${encodeURIComponent(requestId)}&filters=${filtersParam}`
  );
  ...
  return new Promise<UsbPickerResult>((resolve) => {
    ...
    const listener = (msg: unknown) => {
      ...
      if (m?.source !== 'usb-picker-popup' || m.requestId !== requestId) return;
      cleanup();
      if (m.cancelled) {
        resolve({ cancelled: true });
        ...
        resolve({ granted: true, info: m.info as UsbPickerIdentifiers });
      } else {
        resolve({ error: 'usb picker returned an unexpected response' });
      }
    };
    ...
    chromeApi.windows
      ...
      .catch((err: unknown) => {
        cleanup();
        resolve({ error: err instanceof Error ? err.message : 'Failed to open USB picker' });
      });
  });
}
