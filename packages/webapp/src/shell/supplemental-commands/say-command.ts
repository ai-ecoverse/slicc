...
/** `-` is a filename (stdout), not a flag, so `-o -` is valid. */
...
/** Synthesize via kokoro. Failures return `fail()` with empty stdout so a
 *  consumer never reads a zero-byte stream as silence (#3178). */
...
/** `-o -` / non-TTY stdout: the same 16-bit mono WAV as `-o <file>`, on stdout. */
