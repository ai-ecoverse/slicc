/**
 * `ipk mamba` — install emscripten-forge / conda packages into the VFS conda
 * prefix (`/shared/lib/conda`). Separate from npm-based `ipk install`.
 *
 * Thin installer: fetches channel repodata, picks one package version,
 * downloads the `.tar.bz2`, and extracts into the prefix. Not a full SAT
 * solver (no transitive hard-dep install, no virtual-package satisfaction).
 */
...
/**
 * Dispatch `ipk mamba …`. Caller must route here before top-level `--help`
 * so `ipk mamba install --help` stays nested.
 */
