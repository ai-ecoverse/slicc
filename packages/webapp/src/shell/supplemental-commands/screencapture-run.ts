/**
 * `screencapture` body — lazy-loaded from `screencapture-command.ts` so the
 * help text, video capture path, and DOM MediaRecorder helpers stay out of
 * the worker's eager first-load graph (same pattern as `gelatiere/run.ts`).
 */
...
type ExecContext = Parameters<Command['execute']>[1];
...
> {
  try {
    ...
    if (local) {
      // Lazy: MediaRecorder / getDisplayMedia live in a DOM-only chunk that
      // must not join the kernel worker's eager first-load graph.
      const { captureDisplayMedia } = await import('./screencapture-media.js');
...
export async function runScreencapture(args: string[], ctx: ExecContext): Promise<CommandResult> {
