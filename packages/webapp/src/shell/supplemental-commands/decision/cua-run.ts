...
import { fail, ok, readArgText, readPipedOrFlag } from './io.js';
import { formatPlan, parsePrintedPlan, planToPlaywrightLines } from './plan-commands.js';
import type { CuaRuntime } from './runtime.js';
import { elementsFromSnapshot, type FormElement } from './snapshot-elements.js';
...
async function resolveRuntime(options: CuaS1CommandOptions): Promise<CuaRuntime> {
  ...
  const { createDefaultCuaRuntime } = await import('./runtime.js');
