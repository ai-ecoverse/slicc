/**
 * just-bash never exposes isatty(3) on the command context, so `say` cannot
 * see that its stdout is a pipe. Register this transform plugin on the Bash
 * instance: `exec()` / `transform()` walk the AST and, when `say` is piped,
 * redirected, or inside a capture (`$(…)` / `<()`), inject `-o -` so it emits
 * WAV bytes the same way a non-TTY Unix `say` would (#3178).
 *
 * The browser bundle of just-bash does not export `parse` / `serialize`; the
 * plugin mutates the AST the interpreter already parsed.
 */
...
/** Inject `say -o -` when stdout is a pipe, redirect, or capture. */
