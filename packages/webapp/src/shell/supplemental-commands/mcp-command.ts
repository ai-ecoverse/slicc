...
/** A coerced flag value: JSON scalars, arrays, or objects (`type: object`). */
type McpArgValue = string | number | boolean | null | McpArgValue[] | McpArgObject;

/** Nested object produced by JSON coercion or dotted flags. */
...
/** Map `--flag` / `--flag.nested` onto a declared schema property. */
...
/** Consume the value token for `--flag`, or treat a bare boolean as true. */
...
/** Write a coerced value into `out`, merging objects and dotted paths. */
...
/** Nest `value` at `out[rootKey][path…]`, creating intermediate objects. */
function setDotted(
  out: McpToolArguments,
  rootKey: string,
  path: string[],
