...
export interface ShebangExecHintFs {
  ...
  /** Preferred: only the shebang prefix, so `./huge-file` cannot stall the worker. */
...
/** Enough for `#!` plus a long env line; never the rest of the file. */
...
/** How many argv slots this env flag occupies (flag + optional argument). */
