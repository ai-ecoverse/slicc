 ...
 * body below the structured data block.
 *
 * Lives at the transcript layer (not `ui/`) because the frozen-archive
 * writer needs it from the kernel worker as well as from the page: a
 * compaction snapshot (`scoops/live-session-snapshot.ts`) renders the same
 * body a "New chat" freeze does. `ui/chat-clipboard.ts` re-exports it for
 * the panel-side callers.
 *
