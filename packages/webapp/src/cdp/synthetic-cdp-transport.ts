...
export abstract class SyntheticCdpTransport implements CDPTransport {
  /**
   * Last-known target URL. Seeded from `opts.targetUrl` and advanced after a
   * successful `Page.navigate`. Reported via `getCurrentUrl()`, which subclasses
   * override when the realm's URL can change WITHOUT a CDP navigate (Cherry's
   * host iframe soft-navigates, so it reads live `location.href`).
   */
  ...
  /**
   * The URL to report for the target right now. Base returns the last-known URL
   * (seeded at construction, updated on navigate). Subclasses whose realm URL can
   * change out-of-band override this — Cherry returns live `location.href` so a
   * host-page SPA navigation isn't reported as the stale boot-time URL.
   */
  ...
  /**
   * Hook invoked when the leader issues `Target.closeTarget`. Base is a no-op
   * (the synthetic session simply reports success). Subclasses that own a real
   * closable connection (the preview bridge) override this to tear it down.
   */
