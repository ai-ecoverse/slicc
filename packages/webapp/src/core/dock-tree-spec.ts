 ...
 * preset library.
 *
 * These live in `core/` rather than `ui/wc/` because the layer stack
 * (`fs → shell/git → cdp → tools → core → scoops → ui`) forbids lower layers from
 * importing `ui/`, and three of them need this data: the `layout` shell command
 ...
 * right home — it just happened to be where the rendering code lived.
 *
