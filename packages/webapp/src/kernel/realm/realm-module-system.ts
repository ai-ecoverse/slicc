...
export function createModuleSystem(opts: {
  ...
  // Per-realm: `path.resolve(rel)` must use THIS realm's `process.cwd()`, not
  // a host fallback of `/` (#3110). `processShim.cwd` is the same function
  // user code sees as `process.cwd()`.
