...
export async function runJsRealm(init: RealmInitMsg, port: RealmPortLike): Promise<void> {
  ...
  // skill-root `refs`/`assets` (parent of `scripts/` when the script lives
  // there), script-dir `.config`, and the skill-scoped token store; see
  // `skill-global.ts` for the surface and rationale.
