/**
 * `subprocess` for the Pyodide realm.
 *
 * CPython's `subprocess` needs `fork`/`posix_spawn`, which Emscripten does not
 * have. This module replaces `subprocess.Popen` (and `os.system`) with a
 ...
 * and its output is buffered. That fits compiler drivers such as `emcc`, which
 * run one tool at a time and read its output after it exits; it does not fit
 * interactive pipes.
 *
 * Coherence: `beforeExec` / `afterExec` bracket every child so it sees
 * Python's writes and Python sees its writes (see `live-vfs-fs.ts`).
 */

import type { PyodideInterface } from 'pyodide';
import type { SyncExecXhrBridge } from './sync-exec-xhr-bridge.js';

/** The request Python sends (JSON) for one child. */
...
/** The reply Python receives (JSON). `error` is a POSIX code on a bridge failure. */
...
export interface PySubprocessHooks {
  ...
  /** Push Python-side buffered writes to the VFS before the child starts. */
  beforeExec(): void;
  /** Drop Python-side cached VFS state after the child exits. */
  afterExec(): void;
}
...
/** The JS half, registered as the `_slicc_proc` module. */
...
/**
 * The Python half. Installed after the realm's filesystem is mounted and
 * `os.environ` is populated, before user code runs.
 */
...
/** Register `_slicc_proc` and patch `subprocess` / `os.system`. */
