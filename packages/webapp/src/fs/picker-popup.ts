 ...
 *
 * Lives in `fs/` (bottom of the layer stack) so mount helpers can call it
 * without importing up into `shell/`. Shell device pickers re-export from
 * `shell/supplemental-commands/picker-popup.ts`.
 *
...
/**
 * Serializable device identifiers posted by the popup. Per-kind wrappers
 * narrow further (vid/pid, serial number, …).
 */
...
/** Options passed to `chrome.windows.create` for the picker popup. */
