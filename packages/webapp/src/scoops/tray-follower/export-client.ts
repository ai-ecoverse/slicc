...
/** Follower-side transcript export spool + chunking. */
export class FollowerExportClient {
  /** Live export entries. Public so tests can assert the bounded-memory shape. */
  ...
  /**
   * Decode the base64 payload and append to the spool. `nextExpectedIndex`
   * and `totalBytes` are updated SYNCHRONOUSLY before the async spool write
   * so that a `complete` message arriving concurrently sees the correct
   * counts. The ack is sent AFTER the spool write resolves so that it
   * reflects true durability.
   */
