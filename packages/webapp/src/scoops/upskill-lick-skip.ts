/**
 * Decide whether a navigate·upskill lick should raise an approval card.
 *
 * The card is the human interruption. `upskill`'s on-disk "already exists"
 * check only runs after confirm, so cache-busted page loads of an already
 * installed skill used to spam a no-op card (#3100). Skip silently when
 * the advertised GitHub path is already installed at the same commit.
 *
 * Skills with no install provenance (or no recorded sha) also skip: we
 * cannot tell they need an install, and raising would re-train the
 * operator to confirm no-ops. A skill that is not installed at all still
 * raises.
 */
...
/** Skip the card when the advertised GitHub skill is already at that commit. */
...
/**
 * Return whether the navigate·upskill card should be suppressed.
 *
 * - `raise` — not installed, or installed at a different commit.
 * - `skip-same-sha` — every matching install is already at the advertised sha.
 * - `skip-no-provenance` — matching install exists but has no usable sha, or
 *   the upstream sha could not be resolved. Do not spam.
 */
