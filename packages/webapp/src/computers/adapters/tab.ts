...
interface CapturedTabFrame {
  ...
  /** Encoded pixels are still wider than the requested cap (#3373). */
  overCap?: boolean;
}
...
async function captureTabFrame(
  ...
  return {
    ...
    native,
    // The cap is what `--size` promises; when the tab cannot be made to encode
    // that narrow, say so rather than shipping wider pixels silently (#3373).
