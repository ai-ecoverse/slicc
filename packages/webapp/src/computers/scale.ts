/**
 * Screenshot-space → native pixel mapping using the last shot the model saw.
 *
 * Input coordinates are interpreted in the space of `lastShot` unless the
 * caller passes `--native`. Every screenshot line prints
 * `WxH → wxh (scale s)` so the mapping is visible in the transcript.
 */
...
/** Round-trip a native point through a screenshot scale and back. */
