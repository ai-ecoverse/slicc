#!/usr/bin/env node
/**
 * merge — combine the out dirs of a sharded run (one per matrix job) into one, for the report
 * and a single publish.
 *
 *   node packages/bench/scripts/merge.mjs --out bench-out shards/bench-1 shards/bench-2 …
 *
 * Shards split a plan by task, so their records and traces don't overlap; when a record does
 * appear twice (a shard's artifact from a re-run attempt), the judged one wins, then the first.
 * Each shard's journal (calls.jsonl, events.jsonl, diagnostics/, leader logs) moves under
 * `shards/<name>/`. The results, report.md, report.json and report.html are then rebuilt from the
 * merged records, with the earliest shard start as the run start.
 */
...
/** What a merged run keeps per shard, besides records and traces. */
...
/**
 * Copy `<shard>/records/**` into `<out>/records/`; returns the records it could not read (a
 * truncated artifact), which are left out rather than failing the whole report.
 */
...
/** Merge shard out dirs into `out`; returns `{ shards, records, runStart }` for the log. */
