...
final class CDPProxyRuntimeCoverageTests: XCTestCase {
                    ...
                    return Response(
                        ...
                        body: .init(
                            byteBuffer: ByteBuffer(
                                string: #"{"webSocketDebuggerUrl":"ws://fixture/devtools/browser/default"}"#
                            ))
                    )
                case .badStatus:
    ...
    func testTrackingMalformedDetachAndValueHelpers() async throws {
        ...
        await harness.emitText(
            #"{"method":"Target.attachedToTarget","params":{"sessionId":"S1","targetInfo":{"targetId":"T1","url":"https://example.test/"}}}"#
        )
        await harness.emitText(
            #"{"method":"Page.frameNavigated","sessionId":"S1","params":{"frame":{"id":"root","url":"https://example.test/next"}}}"#
        )
