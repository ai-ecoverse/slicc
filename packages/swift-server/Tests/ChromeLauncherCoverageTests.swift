...
private final class ChromeFetchSequence: @unchecked Sendable {
    ...
    func fetch(_ url: URL) throws -> (Data, URLResponse) {
        lock.withLock {
            ...
            return (
                Data(#"{"Browser":"Chrome/Test","webSocketDebuggerUrl":"ws://127.0.0.1:9333/devtools/browser/test"}"#.utf8),
                response
            )
        }
    }
...
final class ChromeLauncherCoverageTests: XCTestCase {
            ...
            Response(
                ...
                body: .init(
                    byteBuffer: ByteBuffer(
                        string: #"{"Browser":"Chrome/default-fetch","webSocketDebuggerUrl":"ws://fixture"}"#
                    ))
            )
        }
        let app = Application(
    ...
    func testOutputMonitorFlushesPartialFinalLineAndReportsEarlyExit() async throws {
        ...
        let payload = Data(#"{"webSocketDebuggerUrl":"ws://fixture"}"#.utf8)
