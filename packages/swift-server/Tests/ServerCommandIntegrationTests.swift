...
final class ServerCommandIntegrationTests: XCTestCase {
    ...
    // #3418: Sliccstart wires the server's stdout/stderr to `Pipe()`s. When the
    // launcher dies the read ends close, and the next log line used to raise
    // SIGPIPE and kill the server silently. It must keep serving instead.
    func testServerSurvivesClosedStdoutAndStderrPipes() async throws {
        ...
        // Shutdown handling is installed right before the "CDP proxy at"
        // banner, which only goes to stdout, so drain until it shows up.
        try await waitForStartupOutput(process: process) { stdoutCapture.text }
        try await Task.sleep(nanoseconds: 100_000_000)

        // The parent "dies": nobody reads the server's output any more.
        stdoutPipe.fileHandleForReading.readabilityHandler = nil
        stderrPipe.fileHandleForReading.readabilityHandler = nil
        try stdoutPipe.fileHandleForReading.close()
        try stderrPipe.fileHandleForReading.close()

        // Every request logs a line to stdout; each write now hits EPIPE.
