...
Repeatable `--mount <os-path>:<slicc-path>` (`ServerCommand.mount` → `ServerConfig.mounts`, parsed by `parseMountMapping`; parity with node-server's `parseMountTableMapping`). `HostFSRoutes.swift` serves mapped folders over `/api/hostfs`, mirroring `hostfs.ts` byte-for-byte (routes — including the stable `POST /api/hostfs` dispatcher for list/stat/mkdir/rename/remove — errno JSON, traversal/symlink containment, mount-root delete refusal, 100 MiB whole-body/range cap, and bounded byte-range reads for large git packs). `BridgeSecurity.preflightMaxAge` mirrors node-server's: `/api/hostfs*` preflights get Chrome's 7200 s cap, the rest 600 s (#2715). Advertised as `autoMounts` on `GET /api/runtime-config`; the webapp auto-mounts at boot, no picker/permission. Sliccstart feeds the flags from Settings → Mounts (`MountTablePreference`). Docs: [`docs/mounts.md`](../../docs/mounts.md#auto-mounted-host-folders-the-mount-table).
