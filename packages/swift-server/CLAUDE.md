...
Both launchers (`ChromeLauncher.buildLaunchArgs` here, node-server's `chrome-launch.ts` — kept byte-identical) append `--disable-features=LocalNetworkAccessChecks,LocalNetworkAccessChecksWebSockets,IntensiveWakeUpThrottling,HighEfficiencyModeAvailable` plus `--disable-background-timer-throttling`, `--disable-backgrounding-occluded-windows`, `--disable-renderer-backgrounding`. The LNA pair: Chromium 142+ gates the local hop behind an "Apps on device" prompt; Deny silently breaks CDP + `/api/*`. The rest keep the leader tab alive in the background: Memory Saver freezing a backgrounded leader suspends its event loop — tray unreachable, turns stuck on a working turn — so the launched Chrome deliberately opts out of background power savings (a real battery cost on portables). See [`docs/pitfalls.md`](../../docs/pitfalls.md).
