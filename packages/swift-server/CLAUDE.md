...
`Sources/Server/ContentType.swift` is the Swift twin of shared-ts `isTextContentType` and gates the fetch-proxy response scrub in `makeStreamingProxyResponse`. Do **not** re-derive that list inline: the old local classifier missed `application/xml` / `application/javascript` / `image/svg+xml` (real secrets forwarded to the agent that node-server masked) and treated any `charset=` parameter as text, forcing binary bodies through a lossy `String` round-trip (#2822). Both halves are pinned by `Tests/CrossImplementationTests.swift` and `packages/shared-ts/tests/content-type-parity.test.ts`.
