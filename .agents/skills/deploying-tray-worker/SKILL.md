...
`worker-staging-e2b-slicc-staging` concurrency group with cancellation disabled. Both
mutate the same staging Worker, so allowing them to overlap could replace one run's
artifact between finalization and its smoke test. Keep the cross-workflow group names
identical; unrelated CI jobs remain parallel.
