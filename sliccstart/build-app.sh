...
# Copy icon (convert PNG to icns if sips is available)
ICON_SRC="$SLICC_ROOT/logos/slicc-favicon-128.png"
if [ -f "$ICON_SRC" ]; then
  ...
  iconutil -c icns "$ICONSET" -o "$RESOURCES/AppIcon.icns" 2>/dev/null && rm -rf "$ICONSET" || true
fi
