...
Every command is automatically logged to `/.playwright/session.md`. State-changing commands (click, fill, goto, etc.) also save an accessibility snapshot to `/.playwright/snapshots/`.
...
- `open` and `tab-new` open tabs in the **background** by default. Use `--foreground` or `--fg` to make the new tab the current tab.
