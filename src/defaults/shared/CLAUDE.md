...
- **da** — Document Authoring (Adobe DA) content I/O: `da config`, `da list`, `da get`, `da put`, `da preview`, `da publish`, `da upload`. Config stored in `/shared/.da-config.json`. Run `da help` for details.
...
- **open <path|url>** — Open a URL or single VFS file in a new browser tab. Use `open --view` when you need to see an image inline. `.shtml` files are opened as sprinkles instead of browser tabs. HTML files opened in preview tabs automatically get `window.slicc` injected — the same bridge API as sprinkles (`slicc.lick()`, `slicc.on('update')`, `slicc.setState()`, `slicc.getState()`, `slicc.readFile()`). Lick events arrive with `sprinkleName: 'playground:/path:id'`. Send updates back via `sprinkle send playground:/path '...'`.
