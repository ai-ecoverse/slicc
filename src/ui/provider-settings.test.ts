...
describe('model metadata overrides', () => {
  ...
  it('getModelIds metadata overrides pi-ai defaults for known models', () => {
    ...
    providerConfigs.set('custom-proxy', {
      id: 'custom-proxy',
      name: 'Custom Proxy',
      description: 'Test',
      requiresApiKey: false,
    ...
    const models = getProviderModels('custom-proxy');
    expect(models).toHaveLength(1);
    expect(models[0].id).toBe('claude-opus-4-6');
    expect((models[0] as any).contextWindow).toBe(1000000);
    expect((models[0] as any).maxTokens).toBe(8000);
  });
  ...
  it('getModelIds metadata creates correct fallback for unknown models', () => {
    ...
    expect(models[0].api).toBe('custom-proxy-openai');
    expect((models[0] as any).contextWindow).toBe(131072);
    expect((models[0] as any).maxTokens).toBe(8000);
  });
  ...
  it('modelOverrides applies to OAuth provider models', () => {
    ...
    providerConfigs.set('custom-oauth-override', {
      id: 'custom-oauth-override',
      name: 'Custom OAuth',
      description: 'Test',
      requiresApiKey: false,
      requiresBaseUrl: false,
    ...
    const models = getProviderModels('custom-oauth-override');
    const model = models.find((m) => m.id === 'claude-sonnet-4-0');
    expect(model).toBeDefined();
    expect((model as any).contextWindow).toBe(500000);
    expect((model as any).maxTokens).toBe(4000);
  });
  ...
  it('getModelIds metadata takes priority over modelOverrides', () => {
    ...
    providerConfigs.set('custom-proxy-with-override', {
      id: 'custom-proxy-with-override',
      name: 'Custom Proxy',
      description: 'Test',
      requiresApiKey: false,
      requiresBaseUrl: false,
      modelOverrides: {
        'claude-sonnet-4-0': {
          context_window: 100000, // Should be overridden by getModelIds
        },
      },
      getModelIds: () => [
        {
          id: 'claude-sonnet-4-0',
          context_window: 200000, // This should win
        },
      ],
    });
    mockGetRegisteredProviderConfig.mockImplementation((id: string) => providerConfigs.get(id));

    const models = getProviderModels('custom-proxy-with-override');
    expect(models[0].id).toBe('claude-sonnet-4-0');
    expect((models[0] as any).contextWindow).toBe(200000);
  });

  it('api field encoded in model.api for stream routing', () => {
    const providerConfigs = new Map(
      mockGetRegisteredProviderIds().map((id: string) => [id, mockGetRegisteredProviderConfig(id)])
    );
    providerConfigs.set('openai-compat', {
      id: 'openai-compat',
    ...
    const models = getProviderModels('openai-compat');
    expect(models).toHaveLength(1);
    expect(models[0].api).toBe('openai-compat-openai');
