 ...
 * Only populates accounts if none exist yet.
 *
 * Supported env vars:
...
export function applyEnvDefaults(): void {
  ...
  const provider = env.VITE_SLICC_PROVIDER as string | undefined;
  const apiKey = env.VITE_SLICC_API_KEY as string | undefined;
  const baseUrl = env.VITE_SLICC_BASE_URL as string | undefined;
  const model = env.VITE_SLICC_MODEL as string | undefined;

  if (apiKey && provider && getAccounts().length === 0) {
    addAccount(provider, apiKey, baseUrl || undefined);
  }
  if (model && !localStorage.getItem(MODEL_KEY)) {
    setSelectedModelId(provider ? `${provider}:${model}` : model);
  }
}
