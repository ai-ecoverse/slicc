...
export class LeaderSyncManager {
  ...
  private handleFollowerMessage(bootstrapId: string, message: FollowerToLeaderMessage): void {
    switch (message.type) {
      ...
      case 'cookie.teleport.request': {
        const { requestId, targetRuntimeId, url, catchPattern, catchNotPattern } = message;
        if (targetRuntimeId === 'leader') {
          ...
          this.forwardCookieTeleportRequest(requestId, targetRuntimeId, bootstrapId, url, catchPattern, catchNotPattern);
        }
        break;
      }
   ...
   */
  private forwardCookieTeleportRequest(requestId: string, targetRuntimeId: string, requesterBootstrapId: string, url?: string, catchPattern?: string, catchNotPattern?: string): void {
    const targetBootstrapId = this.runtimeToBootstrap.get(targetRuntimeId);
    const targetFollower = targetBootstrapId ? this.followers.get(targetBootstrapId) : undefined;
    const requester = this.followers.get(requesterBootstrapId);
    ...
    targetFollower.sync.send({ type: 'cookie.teleport.request', requestId, url, catchPattern, catchNotPattern });
  }
