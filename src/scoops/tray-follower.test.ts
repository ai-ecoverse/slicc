...
describe('tray-follower', () => {
  it('posts the follower join request and normalizes wait responses', async () => {
    ...
    expect(fetchImpl).toHaveBeenCalledWith('https://tray.example.com/join/token', expect.objectContaining({
      method: 'POST',
  ...
  it('preserves signal and fail outcomes without assuming a follower websocket', () => {
    expect(normalizeFollowerAttachResponse({
      trayId: 'tray-1',
    ...
    expect(normalizeFollowerAttachResponse({
      ...
      controllerId: 'follower-2',
      role: 'follower',
      leader: { controllerId: 'leader-1', connected: false, reconnectDeadline: '2026-03-11T01:00:00.000Z' },
      participantCount: 2,
      result: { action: 'fail', code: 'TRAY_EXPIRED', error: 'Tray expired because the leader did not reclaim it within one hour' },
    })).toEqual({
      trayId: 'tray-1',
      controllerId: 'follower-2',
      participantCount: 2,
  ...
  it('rejects malformed worker responses', async () => {
    ...
    await expect(attachTrayFollower({
      joinUrl: 'https://tray.example.com/join/token',
      fetchImpl,
  ...
  it('posts follower bootstrap poll, answer, and retry actions', async () => {
    const fetchImpl = vi.fn<typeof fetch>()
      .mockResolvedValueOnce(new Response(JSON.stringify({
        ...
        events: [{ sequence: 1, sentAt: '2026-03-11T00:00:01.000Z', type: 'bootstrap.offer', offer: { type: 'offer', sdp: 'v=0' } }],
      }), { status: 200, headers: { 'content-type': 'application/json' } }))
      .mockResolvedValueOnce(new Response(JSON.stringify({
        trayId: 'tray-1',
        controllerId: 'follower-1',
    ...
    expect(poll.events).toEqual([
      { sequence: 1, sentAt: '2026-03-11T00:00:01.000Z', type: 'bootstrap.offer', offer: { type: 'offer', sdp: 'v=0' } },
    ]);
    ...
    expect(retry.bootstrap).toMatchObject({ bootstrapId: 'bootstrap-2', attempt: 2, retriesRemaining: 2 });

    expect(fetchImpl).toHaveBeenNthCalledWith(1, 'https://tray.example.com/join/token', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({ action: 'poll', controllerId: 'follower-1', bootstrapId: 'bootstrap-1', cursor: 0 }),
    }));
    expect(fetchImpl).toHaveBeenNthCalledWith(2, 'https://tray.example.com/join/token', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({ action: 'answer', controllerId: 'follower-1', bootstrapId: 'bootstrap-1', answer: { type: 'answer', sdp: 'v=0' } }),
    }));
    expect(fetchImpl).toHaveBeenNthCalledWith(3, 'https://tray.example.com/join/token', expect.objectContaining({
      method: 'POST',
      body: JSON.stringify({ action: 'retry', controllerId: 'follower-1', bootstrapId: 'bootstrap-1', runtime: 'electron' }),
    }));
  });
});