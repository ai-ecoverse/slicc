...
describe('tray-url-shared', () => {
  ...
  it('parses tray join URLs and strips query/hash noise', () => {
    expect(parseTrayJoinUrl('https://tray.example.com/base/join/tray-join.secret?via=share#copied')).toEqual({
  ...
  it('builds canonical tray launch URLs and removes legacy params', () => {
    expect(buildCanonicalTrayLaunchUrl(
    ...
    )).toBe('http://localhost:3000/?scoop=cone&tray=https%3A%2F%2Ftray.example.com%2Fbase%2Fjoin%2Ftray-join.secret');
  });
});