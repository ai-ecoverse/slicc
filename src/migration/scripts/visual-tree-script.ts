/**
 * Visual tree extraction script.
 * Self-contained IIFE that runs in-page via CDP evaluate().
 * Captures DOM spatial hierarchy for LLM analysis.
 *
 * Ported from vibemigration:
 *   - css-selector.ts (Chrome DevTools algorithm)
 *   - layout-detection.ts (grid pattern detection)
 *   - visual-tree.ts (tree builder, optimizer, formatter, CIELAB color)
 *
