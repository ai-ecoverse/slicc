/**
 * In-page script that detects and dismisses overlays (cookie banners,
 * chat widgets, GDPR consent, etc.) using heuristic vendor patterns.
 *
 * Runs via BrowserAPI.evaluate(). Returns JSON with dismissal results.
 *
 * Ported from: vibemigration/playwright-worker/src/overlay-detection/heuristics.ts
 *              vibemigration/playwright-worker/src/overlay-detection/script-compiler.ts
 */
export const OVERLAY_DISMISS_SCRIPT = `(async () => {
