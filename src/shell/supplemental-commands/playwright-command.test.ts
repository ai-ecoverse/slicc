...
describe('playwright-cli cookie commands', () => {
  ...
  it('cookie-set sets a cookie with flags', async () => {
    (browser.evaluate as ReturnType<typeof vi.fn>).mockResolvedValue('https://example.com/page');
    const cmd = createPlaywrightCommand('playwright-cli', browser as BrowserAPI, fs as VirtualFS);
    await cmd.execute(['open', 'https://example.com', '--foreground'], {} as any);
...
describe('playwright-cli session history logging', () => {
  ...
  it('creates /.playwright/ directories', async () => {
    const cmd = createPlaywrightCommand('playwright-cli', browser as BrowserAPI, fs as VirtualFS);
    await cmd.execute(['open', 'https://example.com', '--foreground'], {} as any);
    expect(fs.mkdir).toHaveBeenCalledWith('/.playwright');
    expect(fs.mkdir).toHaveBeenCalledWith('/.playwright/snapshots');
    expect(fs.mkdir).toHaveBeenCalledWith('/.playwright/screenshots');
  });
