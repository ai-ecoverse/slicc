...
/** Commands that modify page state and should trigger auto-snapshot. */
...
function renderNode(
  ...
  if (needsRef) {
    ...
    if (role === 'button' && name) {
      ...
      if (name) {
        selector = `input[aria-label="${escapedName}"], textarea[aria-label="${escapedName}"], [contenteditable][aria-label="${escapedName}"], input[placeholder="${escapedName}"], textarea[placeholder="${escapedName}"]`;
      } else {
...
async function ensureTarget(
  ...
  const active = pages.find(
    (p) => p.active && !isAppTab(state, p.targetId),
  );
  if (active) {
    state.currentTarget = active.targetId;
    return active.targetId;
  }
  // Fall back to first non-app tab
  const first = pages.find((p) => !isAppTab(state, p.targetId));
  if (first) {
...
export function createPlaywrightCommand(
  ...
  return defineCommand(name, async (args): Promise<CmdResult> => {
    ...
    try {
      switch (subcommand) {
        ...
        case 'cookie-set': {
          ...
          // If no domain specified, get current page URL domain
          if (!params['domain']) {
            const urlStr = await browser.evaluate('location.href') as string;
            try {
    ...
    try {
      ...
      if (STATE_CHANGING_COMMANDS.has(subcommand) && result.exitCode === 0 && state.currentTarget) {
