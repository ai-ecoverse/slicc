...
async function runAll() {
  ...
  check(
    '1000 RPC reads under hot write loop succeed',
    rpcFail === 0,
    `ok=${rpcOk} fail=${rpcFail} writes=${st1.writes} lastErr=${st1.lastErr ?? 'none'}`
  );
  check(
    'sizes are monotonically non-decreasing (no corruption)',
  ...
  check(
    'page-direct-read coexists with open worker sync handle (Chrome behavior recorded)',
    true,
    pageDirectReadThrew
      ? 'getFile threw'
      : `getFile returned ${pageDirectReadLen} bytes (snapshot may be stale; do NOT rely on it)`
  );
  ...
  try {
    ...
    check(
      'page can read+write OPFS after worker releases handles',
      text === 'page-write-after-worker-reset'
    );
  } catch (e) {
  ...
  const results = await Promise.allSettled(
    Array.from({ length: 200 }, () => rpc('read', { name: 'hot.txt' }))
  );
  await rpc('stop');
...
document
  .getElementById('run')
  .addEventListener('click', () => runAll().catch((e) => log(`fatal: ${e.message}`, 'err')));
