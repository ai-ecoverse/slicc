...
- **Node / cloud**: `packages/node-server/`, `packages/cloudflare-worker/`, `packages/cloud-core/`, `packages/shared-ts/`, `packages/vfs-root/`, `packages/dev-tools/`, `packages/assets/`, `packages/github-workflow/`, `packages/bench/`
...
- **Biscotto** (pl. **biscotti**): a revocable guest seat on a cone. Holder gets a private `*.sliccy.now` URL showing the live transcript; its messages are untrusted by default and pass a message/tool gate. Restricted at the wire by `scoops/tray-leader/biscotto-gate.ts`. Approvers: `user`, `cone`, `agent` (a bounded per-request approver driven by `/etc/APPROVALS.md`), or a delegated `scoop`.
