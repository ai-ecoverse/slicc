...
- `teleport` — Teleport browser cookies from a remote tray runtime to the local browser. Auto-selects best follower (prefers standalone floats) or target a specific runtime. Usage: `teleport` (auto), `teleport <runtime-id>` (specific), `teleport --list` (list runtimes). Flags: `--no-reload` to skip page reload after applying cookies.
