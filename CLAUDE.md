...
- **Image preprocessing**: Images in tool results are automatically validated and resized before reaching the LLM (`src/core/image-processor.ts`). The 5MB limit is enforced on the **base64 string length** (not decoded bytes) since that's what the API checks. Images over 5MB base64, over 8000px on any dimension, or over 1568px long edge are compressed via ImageMagick WASM (`src/shell/supplemental-commands/magick-wasm.ts`). Dimensions are parsed from PNG/GIF/JPEG headers without full decode (`getImageDimensions()`). Unsupported formats or corrupt images are replaced with text placeholders. If the API still rejects an image, `recoverFromImageError()` in `scoop-context.ts` strips image blocks from context and re-prompts (same pattern as overflow recovery).
- **Overflow recovery** (`scoop-context.ts`): When the API returns a "prompt too long" error, `ScoopContext` catches it via `isContextOverflow()` from pi-ai, removes the error message, replaces oversized messages (>40K chars) with placeholders, and re-prompts the agent with an explanation. For assistant messages, ToolCall blocks are preserved during replacement (only text/image/thinking content is removed) — this maintains the required `tool_use` ↔ `toolResult` pairing that the API enforces. Limited to 1 retry to prevent infinite loops. Uses deep import from `@mariozechner/pi-ai/dist/utils/overflow.js` (Vite alias in both configs).
...
- **Tests are colocated**: foo.test.ts next to foo.ts. Vitest with globals: true, environment: node. New pure-logic code (utilities, adapters, data transformations) should always have tests. DOM-dependent code (UI panels, layout) and chrome.* API code (DebuggerClient) are acceptable to skip in Node tests but should be manually verified. Use `fake-indexeddb/auto` for tests that need VFS. Current count: 1513 tests across 84 files.
...
- **Model ID aliases**: When referencing Anthropic models in `getModelIds` or provider config, use pi-ai alias IDs (e.g., `claude-opus-4-6`) not dated snapshot IDs (e.g., `claude-opus-4-6-20250626`). In the browser bundle, pi-ai's `getModel()` returns `undefined` for unknown IDs (instead of throwing), and `{ ...undefined }` silently produces `{}` — losing all model metadata. `resolveCurrentModel` guards against this, but using aliases avoids the fallback path entirely.
