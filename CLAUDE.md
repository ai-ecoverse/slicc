...
- **Delegation**: The cone feeds work to scoops via the `feed_scoop` tool, providing complete self-contained prompts (scoops have no access to the cone's conversation). When a scoop finishes, the orchestrator automatically routes its response back to the cone's message queue. Delegation enriches prompts with sibling coordination context (active scoops, their tasks, statuses).
- **Structured Completion Reports**: When a scoop completes, the orchestrator generates a filesystem diff (files created/modified/deleted) by comparing a pre-delegation snapshot to the post-completion state. This replaces lossy prose summaries with verifiable change reports. Functions: `captureSnapshot()`, `buildDiffReport()`.
- **Coordination Directory**: `/shared/.coordination/{folder}.json` files are maintained by the orchestrator for sibling awareness. Written on delegation, updated on status changes, deleted on scoop removal. Scoops can read this directory to discover siblings. System prompt hints guide scoops to check it.
- **VFS Write Notifications**: `VirtualFS.onWrite` callback fires when any scoop writes to `/shared/` (excluding `/shared/.coordination/`). `RestrictedFS` passes the scoop name as the writer identity. The orchestrator routes `fs-notify` messages to the cone so it can react to shared file changes.
- **Delegation Waves**: Consecutive `delegateToScoop` calls within 1s are grouped into a `DelegationWave`. When all scoops in a wave complete, `runIntegrationCheck()` detects same-file modifications across scoops in `/shared/` and sends an `[integration-check]` report to the cone. Single-scoop waves skip the check.
