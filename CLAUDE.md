...
- **Dual-mode compatibility**: Features MUST work in both CLI and extension. Extension CSP blocks eval/CDN — sprinkles/dips → `sprinkle-sandbox.html`, bundled assets → `chrome.runtime.getURL()`; JS realms run in the kernel-worker `DedicatedWorker`, not `sandbox.html` (offscreen-era).
