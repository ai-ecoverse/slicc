...
- **Tests are colocated**: foo.test.ts next to foo.ts. Vitest with globals: true, environment: node. New pure-logic code (utilities, adapters, data transformations) should always have tests. DOM-dependent code (UI panels, layout) and chrome.* API code (DebuggerClient) are acceptable to skip in Node tests but should be manually verified. Use `fake-indexeddb/auto` for tests that need VFS. Current count: 1237 tests across 68 files.
...
- **Build-time provider composition**: Providers are auto-discovered from `src/providers/built-in/*.ts` (built-in, filtered by `providers.build.json`) and `/providers/*.ts` (external, gitignored, always included). Each provider module exports `config: ProviderConfig` and optionally `register(): void` for custom stream functions. The discovery and registration happens in `src/providers/index.ts`, imported as a single side-effect in both `main.ts` and `offscreen.ts`. To add an external provider, drop a `.ts` file into the root `providers/` directory. To filter built-ins for a custom build, edit `providers.build.json` (`include: ["*"]` = all, `exclude: ["*"]` = none). OAuth providers set `isOAuth: true` in their config and define `onOAuthLogin`/`onOAuthLogout` callbacks; the generic `OAuthLauncher` from `src/providers/oauth-service.ts` handles the transport (popup or chrome.identity).
