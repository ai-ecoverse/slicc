...
**Preview SW** (`packages/webapp/src/ui/preview-sw.ts`): Intercepts `/preview/*` requests, serves VFS content from the _local_ origin. Built as IIFE via esbuild (not rollup — avoids code-splitting issues in SWs). **Legacy path** (pre-Phase-3): kept alive for the `open <vfs-path>` shell command. `serve` no longer uses the SW.

**Unified Preview** (`packages/cloudflare-worker/src/preview-handler.ts` + `packages/webapp/src/scoops/preview-request-handler.ts`): `serve` mints `<token>.preview.sliccy.ai` URLs that route through the tray hub worker. The worker forwards reads to the leader over the existing controller WebSocket (`preview.request` → `preview.response` chunked), broadcasts `preview.open` to all connected followers (TS + iOS), and reassembles + streams the bytes back to the requesting tab. Two leader-side mint paths share the wire shape: in-realm `setPreviewMinter` hook (extension agent + panel terminal in offscreen) and `tray-open-preview` panel-RPC op (standalone kernel-worker → page-side `LeaderSyncManager`). Worker has no CORS (preview subdomains can't fetch each other); leader has `isPathWithinServedRoot` defending against `..`, `.`, URL-encoded traversal, sibling-prefix, and over-broad `serve /`. `--bridge` opts into leader-managed live updates (Phase 2); Cherry-attached followers default-on; `--no-bridge` always wins.
...
- **Preview URLs**: `toPreviewUrl(vfsPath)` builds the legacy local SW URL (still used by `open <vfs-path>` pre-Phase-3). For the unified worker-hosted variant use `mintPreviewViaWorker` from `preview-mint-client.ts`. `isPreviewUrl(url)` in `shared.ts` matches both forms (used by the BrowserAPI app-tab exclude).
