...
- **Tray sync protocol** (`tray-sync-protocol.ts`): Typed message protocol for leader↔follower communication over WebRTC data channels. Leader→follower messages: `snapshot`, `agent_event`, `user_message_echo`, `status`, `error`, `targets.registry`, `cdp.request`, `cdp.response`, `tab.open`, `tab.opened`, `tab.open.error`, `cookie.teleport.request`, `cookie.teleport.response`. Follower→leader messages: `user_message`, `abort`, `request_snapshot`, `targets.advertise`, `cdp.request`, `cdp.response`, `tab.open`, `tab.opened`, `tab.open.error`, `cookie.teleport.request`, `cookie.teleport.response`. Types `RemoteTargetInfo`, `TrayTargetEntry`, and `CookieTeleportCookie` define the target advertisement and cookie teleport contracts.
...
- **Cookie teleport protocol**: Transfers browser cookies between tray runtimes via `cookie.teleport.request` / `cookie.teleport.response` messages. Uses `Network.getCookies` on the source runtime and `Network.setCookies` on the destination. Follows the same leader-mediated routing pattern as CDP and FS — the leader tracks `PendingCookieTeleportRoute` by `requestId` and forwards requests/responses between followers. `LeaderSyncManager.sendCookieTeleportRequest()` and `FollowerSyncManager.sendCookieTeleportRequest()` provide the public API. `getBestFollowerForTeleport()` auto-selects the best follower (prefers standalone floats, sorted by `lastActivity`). When `cookie.teleport.request` includes an optional `url` field, the follower opens a browser tab for interactive human authentication via `executeTeleportAuth()` (`teleport-auth.ts`): opens tab → waits for auth completion (hostname redirect detection via `isAuthRedirect()`) or 120s timeout → captures cookies → closes tab. The `teleport` shell command (`teleport-command.ts`) wires it all together, with `--url <url>` flag for interactive auth.
