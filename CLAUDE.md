...
**Preview SW** (`packages/webapp/src/ui/preview-sw.ts`): Intercepts `/preview/*`, serves VFS content from local origin. Built as IIFE via esbuild. Legacy path kept for `open <vfs-path>`; `serve` now uses the worker-relayed unified preview.

**Unified Preview**: `serve <dir>` mints a capability-token URL on `*.preview.sliccy.ai`; the worker relays HTTP reads to the leader via the controller WebSocket, reassembles, and responds. `isPathWithinServedRoot` guards traversal; no CORS. See `packages/cloudflare-worker/CLAUDE.md`.
