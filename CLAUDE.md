...
- **Biscotto** (pl. **biscotti**): a revocable guest seat on a cone. Holder gets a private `*.sliccy.now` URL showing the live transcript; its messages are untrusted by default and pass a message/tool gate. Restricted at the wire by `scoops/tray-leader/biscotto-gate.ts`. Approvers: `user`, `cone`, `agent` (a bounded per-request approver driven by `/shared/APPROVALS.md`), or a delegated `scoop`.
