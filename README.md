...
[![58% Vibe_Coded](https://img.shields.io/badge/58%25-Vibe_Coded-ff69b4?style=for-the-badge&logo=claude&logoColor=white)](https://github.com/ai-ecoverse/vibe-coded-badge-action)
...
- On iPhone and iPad, Light and Dark follow the device. A theme chosen on the leader still colors the follower when the device is in that theme's appearance, and its accent shows either way. The terminal stays dark
- In Theme settings, select two themes and SLICC switches between the darker and the brighter one as Light and Dark change
...
- **A real shell environment.** Many browser agents are constrained by the tools provided to them. SLICC has an almost-too-real shell with commands like `git`, "`node`", `python`, `playwright`, built-in. Text files use UTF-8 by default; binary pipelines and explicit file encodings preserve their bytes. Browser files support serialized appends and persistent modes/timestamps.
...
- **Make targeted file changes with Pi's `edit` tool.** One call can apply multiple unique, non-overlapping replacements while preserving BOMs and line endings; replacement text is always literal, so `$` sequences cannot rewrite themselves through JavaScript replacement expansion.
...
- **Use a Ghostty-backed terminal.** wterm renders shell text and handles input; `imgcat photo.png` sends image bytes to its Kitty graphics preview. Other supported image and video types keep their existing media preview.
- **Watch computers next to browser tabs.** Registered screens (v86 guests, tabs, jsh backends, a Mac follower's desktop) appear as cards after browser tabs in the Browser overlay. Click a card for a live lightbox; when the computer allows input, clicks, scroll, and keys in that lightbox drive it (Escape releases). `computer` bash rows show a live or frozen frame — frozen stills never forward input. A Swift-launcher Mac advertises native capture; iOS is viewer-only.
...
- **Publish persistent previews.** `serve --ttl 30d <dir>` uploads an immutable snapshot that remains available without the tray leader for up to 30 days; `serve --stop <token>` revokes it early. Files are capped at 25 MiB; a tray holds up to 10 snapshots (live previews are unlimited and expire once the leader has been gone for 5 minutes) — see [`serve` limits](docs/shell-reference.md#limits-and-lifetime).
...
- **See saved chat while SLICC is still starting.** After a reload, each cone's thread is filled from its saved conversation as soon as that record can be read, before each agent is brought back. Until then the thread says it is restoring the session. Compaction markers in that history still name the `/sessions` snapshot they point at.
- **Never lose a conversation to compaction.** Whenever a cone's history is summarized to fit the context window, the full transcript is first saved to `/sessions` as a live snapshot (one per cone, extended on every round) and the summary tells the agent where to find it. **New chat** completes that snapshot into a normal frozen session (Save / New chat, fast) or deletes it (Discard). **Compact on idle**: a cone that has sat idle for 30 minutes with a context of at least 200k tokens compacts in the background, so you never wait for a summary at the start of your next turn; if you come back mid-round, or if the summary provider is rate-limited, out of quota, unavailable, or returns no valid summary, the round is thrown away and both the live and saved conversation stay byte-for-byte as they were. A later idle window can retry. Every adopted compaction leaves a small marker in the transcript naming what happened, with a link to the snapshot it replaced; a round that ends up keeping nothing takes its marker back out while retaining the full snapshot for recovery.
- **Keep terminal failures visible.** If a cone or delegated scoop exhausts its provider retries, the error card is stored outside the model-facing history and survives a reload. Retrying or rebuilding the transcript does not duplicate it.
- **Keep a gelatiere.** Turn on **Memory v2** under Experimental features and SLICC keeps a resident advisor: a persistent scoop no cone owns and nobody can prompt that reviews your archived sessions every night and after you end a chat, then comes back with a few concrete suggestions — a skill from the catalog that fits what you keep doing by hand, a use case you have not tried, a habit that would have avoided a detour. It always uses the current leading cone's model, including after a model switch or leadership change. Suggestions show up as cards in the suggestions sprinkle (Install / Try it / Dismiss) and every open cone gets a lick. `gelatiere run` asks for a pass now; the instructions, the nightly schedule and the budget live in `/shared/GELATIERE.md`.
- **Inspect experimental features from the avatar menu.** When centrally managed flags enable the surface for the current runtime, click your avatar and choose **Experimental features…** to open its standalone dialog. The dialog may honestly be empty: `experimental-settings` controls its availability but is not itself user-toggleable. Experimental features take effect when SLICC boots, so flipping one only matters on the next load. Changes are held until you confirm: the dialog says what is waiting and offers **Reload now** to save and apply them, or **Revert** to drop them. Nothing is written until you reload, and dismissing the dialog discards — so a flag can never end up reading `on` with nothing to show for it.
...
- **Delegate parallel work to scoops.** Split tasks into isolated sub-agents with their own sandboxes and context, then let the main agent coordinate the results. If a scoop hits a terminal model or provider failure, its owner gets a durable error card without spending another model call to acknowledge the outage; fatal state still bypasses mute and releases `scoop_wait`.
...
- **Recover from concurrent file edits.** When an edit invalidates a browser file snapshot, SLICC attempts the read up to three times. Persistent read errors are still reported.
...
- **Sprinkles** — everything is better with sprinkles: small, optional enhancements you can add on top of the core system. Select a newly discovered sprinkle's rail icon to open its panel. Panel actions return to the cone that opened them by default, including clicks on a synced follower or the iPhone/iPad app, and a panel can target a different cone or scoop for an individual action.
