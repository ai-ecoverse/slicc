...
- **Publish persistent previews.** `serve --ttl 30d <dir>` uploads an immutable snapshot that remains available without the tray leader for up to 30 days; `serve --stop <token>` revokes it early. Previews (live or persistent) are capped at 25 MiB per file and 10 per tray, and persist until stopped or expired — see [`serve` limits](docs/shell-reference.md#limits-and-lifetime).
