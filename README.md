...
- **Steer a running turn instead of waiting for it.** In the chat composer, `Enter` sends (queueing behind whatever the cone is already doing) and `Shift+Enter` inserts a newline. `Ctrl+Enter` (`Cmd+Enter` on macOS) steers: the message is injected as soon as the current step finishes, so you can redirect the agent mid-turn.
