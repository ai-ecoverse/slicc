...
[![41% Vibe_Coded](https://img.shields.io/badge/41%25-Vibe_Coded-ff69b4?style=for-the-badge&logo=claude&logoColor=white)](https://github.com/ai-ecoverse/vibe-coded-badge-action)
...
- **Run Linux commands in a WebAssembly sandbox.** `vpod run` executes commands in a real 64-bit Linux guest via [vpod](https://vpod.sh) — a genuine kernel + userland compiled to wasm, entirely client-side, with git, node, and python3 baked into the default snapshot. Ideal for isolating untrusted code from the workspace. Install with `ipk add @capsule-run/vpod`.
