...
- **Watch computers next to browser tabs.** Registered screens (v86 guests, tabs, jsh backends) appear as cards after browser tabs in the Browser overlay. Click a card for a live lightbox; when the computer allows input, clicks, scroll, and keys in that lightbox drive it (Escape releases). `computer` bash rows show a live or frozen frame — frozen stills never forward input.
