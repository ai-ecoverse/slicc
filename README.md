...
- **Never lose a conversation to compaction.** Whenever a cone's history is summarized to fit the context window, the full transcript is first saved to `/sessions` as a live snapshot (one per cone, extended on every round) and the summary tells the agent where to find it. **New chat** completes that snapshot into a normal frozen session (Save / New chat, fast) or deletes it (Discard). With the experimental **Compact on idle** flag on, a cone that has sat idle for 30 minutes with a context of at least 200k tokens compacts in the background; if you come back mid-round, the round is thrown away and your chat continues as it was. Every compaction — idle or not — leaves a small marker in the transcript naming what happened, with a link to the snapshot it replaced; a round that ends up keeping nothing takes its marker back out.
...
- **Inspect experimental features from the avatar menu.** When centrally managed flags enable the surface for the current runtime, click your avatar and choose **Experimental features…** to open its standalone dialog. The dialog may honestly be empty: `experimental-settings` controls its availability but is not itself user-toggleable. Experimental features are wired when SLICC boots, so flipping one only takes effect on the next load: once you change a switch the dialog says so and asks you to confirm with **Reload now**, or to **Revert** and stay put. Dismissing the dialog with a change pending reloads too — the setting is already saved, and leaving it half-applied is how a flag ends up reading `on` with nothing to show for it.
